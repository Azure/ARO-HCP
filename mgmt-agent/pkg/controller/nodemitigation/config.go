...
func (cfg Config) Validate() error {
	...
	if slices.Contains(cfg.Mitigators, "never-ready") && (cfg.Window.Duration <= 0 || cfg.ObservationMaxAge.Duration >= cfg.Window.Duration) {
		return fmt.Errorf("never-ready requires observationMaxAge < positive window")
	}
	...
	if slices.Contains(cfg.Mitigators, "never-ready") && (cfg.MaxUnavailableCluster < 1 || cfg.MaxUnavailablePool < 1 ||
