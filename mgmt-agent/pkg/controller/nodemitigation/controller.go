...
func (c *Controller) hasCandidates(cfg Config) (bool, error) {
	...
	for _, node := range nodes {
		...
		detections := nodeEvidence(node, pods, events, c.clock())
...
func nodeEvidence(node *corev1.Node, pods []*corev1.Pod, observed []corev1.Event, now time.Time) []detectors.Detection {
	...
	return detectors.CollectDetections(node, events, onNode, now)
}
...
func (c *Controller) reconcile(ctx context.Context) error {
	...
	for _, node := range snapshot.Nodes {
		...
		detections := nodeEvidence(node, snapshot.Pods, snapshot.Events, c.clock())
		for _, detection := range detections {
