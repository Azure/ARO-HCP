...
func TestPlacementReconsidersAssignments(t *testing.T) {
	for _, count := range []int{2, 3} {
		t.Run(fmt.Sprintf("pods=%d", count), func(t *testing.T) {
			snapshot := ClusterSnapshot{Nodes: []*corev1.Node{placementNode("source", "source")}}
			moving := make([]*corev1.Pod, count)
			for i := range count {
				name := fmt.Sprintf("node-%d", i)
				node := placementNode(name, name)
				node.Status.Allocatable[corev1.ResourcePods] = resource.MustParse("1")
				node.Status.Allocatable[corev1.ResourceCPU] = resource.MustParse("1")
				node.Status.Allocatable[corev1.ResourceMemory] = resource.MustParse("1Gi")
				node.Status.Allocatable[kuberesources.SwiftNICResourceName] = resource.MustParse("1")
				snapshot.Nodes = append(snapshot.Nodes, node)
				pod := placementPod(fmt.Sprintf("pod-%d", i), "source")
				snapshot.Pods = append(snapshot.Pods, pod)
				moving[count-i-1] = pod
				pod.Spec.Affinity = &corev1.Affinity{PodAntiAffinity: &corev1.PodAntiAffinity{
					RequiredDuringSchedulingIgnoredDuringExecution: []corev1.PodAffinityTerm{{
						TopologyKey: corev1.LabelHostname, LabelSelector: &metav1.LabelSelector{MatchLabels: pod.Labels},
					}},
				}}
			}
			moving[0].Spec.NodeSelector = map[string]string{corev1.LabelHostname: "node-0"}
			input := struct {
				Snapshot ClusterSnapshot
				Moving   []*corev1.Pod
			}{snapshot, moving}
			...
			if err := placement(snapshot, map[string]bool{"source": true}, moving); err != nil {
				t.Fatalf("constrained last Pod fits node-0 after reassignment: %v", err)
			}
			...
			if string(before) != string(after) {
				t.Fatal("placement mutated its snapshot or caller's moving slice")
			}
		})
	}
}

func TestPlacementReconsidersPendingDemand(t *testing.T) {
	a, b := placementNode("a", "a"), placementNode("b", "b")
	a.Status.Allocatable[corev1.ResourcePods] = resource.MustParse("1")
	b.Status.Allocatable[corev1.ResourcePods] = resource.MustParse("1")
	pending := placementPod("a-flexible", "")
	replacement := placementPod("z-constrained", "source")
	replacement.Spec.NodeSelector = map[string]string{corev1.LabelHostname: "a"}
	snapshot := ClusterSnapshot{Nodes: []*corev1.Node{a, b}, Pods: []*corev1.Pod{pending, replacement}}
	if err := placement(snapshot, nil, []*corev1.Pod{replacement}); err != nil {
		t.Fatalf("pending Pod fits B and replacement fits A: %v", err)
	}
	b.Status.Allocatable[corev1.ResourcePods] = resource.MustParse("0")
	if err := placement(snapshot, nil, []*corev1.Pod{replacement}); err == nil ||
		!strings.Contains(err.Error(), "no feasible replacement capacity") {
		t.Fatalf("insufficient joint capacity did not hold: %v", err)
	}
}

func TestPlacementSearchAttemptLimit(t *testing.T) {
	for _, test := range []struct {
		...
		{"last allowed attempt fits", maxPlacementAttempts, maxPlacementAttempts - 1, ""},
		{"next attempt exceeds limit", maxPlacementAttempts + 1, maxPlacementAttempts, "exhausted its 4096 pod/node attempts"},
		{"all allowed attempts fail", maxPlacementAttempts, maxPlacementAttempts, "no feasible replacement capacity"},
	} {
		t.Run(test.name, func(t *testing.T) {
			snapshot := ClusterSnapshot{}
			for i := range test.nodes {
				snapshot.Nodes = append(snapshot.Nodes, placementNode(fmt.Sprintf("node-%05d", i), "zone"))
			}
			pod := placementPod("moving", "source")
			pod.Spec.NodeSelector = map[string]string{corev1.LabelHostname: fmt.Sprintf("node-%05d", test.target)}
