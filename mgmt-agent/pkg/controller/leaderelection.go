...
// If the run callback returns an error, the parent context is cancelled so that the caller
// (e.g. an errgroup) can detect the failure and exit, allowing Kubernetes to restart the pod.
func RunWithLeaderElection(ctx context.Context, controllerName string, config *LeaderElectionConfig, run func(ctx context.Context) error) error {
		...
		Callbacks: leaderelection.LeaderCallbacks{
			OnStartedLeading: func(leaderCtx context.Context) {
				...
				if err := run(leaderCtx); err != nil {
					klog.ErrorS(err, "Controller failed, releasing leadership", "controllerName", controllerName)
				}
			},
