...
func resolveBackupContext(
	...
	if spc.Status.ManagementClusterResourceID == nil {
		return nil, fmt.Errorf("management cluster placement not resolved for cluster %s", resourceID.String())
	}
...
func GetBackup(
	...
	return func(writer http.ResponseWriter, request *http.Request) error {
		...
		if kaClient == nil {
			return fmt.Errorf("kube-applier client not available for management cluster")
		}
...
func (h *HCPGetBackupScheduleHandler) ServeHTTP(writer http.ResponseWriter, request *http.Request) error {
	...
	if spc.Status.ManagementClusterResourceID == nil {
		return fmt.Errorf("management cluster placement not resolved")
	}
...
func (h *HCPPatchBackupScheduleHandler) ServeHTTP(writer http.ResponseWriter, request *http.Request) error {
	...
	if state != api.BackupScheduleStateActive && state != api.BackupScheduleStatePaused {
		return arm.NewCloudError(http.StatusBadRequest, arm.CloudErrorCodeInvalidRequestContent, "", "invalid state %q: must be %q or %q", patch.State, api.BackupScheduleStateActive, api.BackupScheduleStatePaused)
	}
