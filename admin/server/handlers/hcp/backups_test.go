	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, expected := range tt.expectedSchedules {
				...
				if actual.BackupSchedulePhase != expected.BackupSchedulePhase {
					t.Errorf("schedule[%d] phase: expected %q, got %q", i, expected.BackupSchedulePhase, actual.BackupSchedulePhase)
				}
				if actual.Paused != expected.Paused {
					t.Errorf("schedule[%d] paused: expected %v, got %v", i, expected.Paused, actual.Paused)
				}
			}
		})
	}
