...
func NewCreateBackupCommand() (*cobra.Command, error) {
	opts := base.DefaultAuthOptions()
	var subscriptionID, resourceGroup, clusterName string

	cmd := &cobra.Command{
		Use:           "create-backup",
		Short:         "Create a backup for an HCP cluster",
		SilenceErrors: true,
		SilenceUsage:  true,
		RunE: func(cmd *cobra.Command, args []string) error {
			return execute(cmd.Context(), opts, subscriptionID, resourceGroup, clusterName)
		},
	}
	...
	for _, flag := range []string{"subscription-id", "resource-group", "cluster-name"} {
...
func execute(ctx context.Context, opts *base.RawAuthOptions, subscriptionID, resourceGroup, clusterName string) error {
	...
	logger.Info("Creating backup", "subscriptionID", subscriptionID, "resourceGroup", resourceGroup, "clusterName", clusterName)

	client := adminClient.NewClient(completed.Endpoint, completed.Host, completed.Token, completed.Insecure, false)
	err = client.CreateBackup(ctx, subscriptionID, resourceGroup, clusterName)
	if err != nil {
