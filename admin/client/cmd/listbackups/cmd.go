...
func NewListBackupsCommand() (*cobra.Command, error) {
	opts := base.DefaultAuthOptions()
	var subscriptionID, resourceGroup, clusterName string

	cmd := &cobra.Command{
		Use:           "list-backups",
		Short:         "List backups for an HCP cluster",
		SilenceErrors: true,
		SilenceUsage:  true,
...
func execute(ctx context.Context, opts *base.RawAuthOptions, subscriptionID, resourceGroup, clusterName string) error {
	...
	logger.Info("Listing backups", "subscriptionID", subscriptionID, "resourceGroup", resourceGroup, "clusterName", clusterName)

	client := adminClient.NewClient(completed.Endpoint, completed.Host, completed.Token, completed.Insecure, false)
	err = client.ListBackups(ctx, subscriptionID, resourceGroup, clusterName)
	if err != nil {
