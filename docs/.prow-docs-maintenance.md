...
- **Postsubmit Jobs**: https://github.com/openshift/release/blob/master/ci-operator/jobs/Azure/ARO-HCP/Azure-ARO-HCP-main-postsubmits.yaml
...
- **E2E Gating Config**: https://github.com/openshift/release/blob/master/ci-operator/config/Azure/ARO-HCP/Azure-ARO-HCP-main__e2e.yaml
  - Contains: EV2 gating E2E postsubmit job configs
  - Key fields: `tests[].as` (job name), `tests[].postsubmit` (must be true), `tests[].run_if_changed` (set to `^$` to prevent auto-triggering)
...
2. Add to appropriate subsection in `prow.md` (Image Updater Tooling, Cleanup Jobs, or Periodic E2E Tests)
...
- Postsubmit (EV2 gating) jobs: `branch-ci-Azure-ARO-HCP-main-e2e-<test-name>`
