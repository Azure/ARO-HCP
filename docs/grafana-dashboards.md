...
When your PR changes a file under `observability/grafana-dashboards/`, CI deploys the changed dashboards to the shared dev Grafana instance and leaves a comment on the PR with links to preview the dashboards before merging so that a reviewer can see the rendered dashboard against live dev data. The comment on the PR will update when new dashboard changes are pushed, and the dashboard links are also in the Prow job log in case of a failure with the GitHub bot.

Dashboards are placed in a folder named `PR-<number>` in the Azure Managed Grafana instance `arohcp-dev`. Dashboards generated by the CI step will never overwrite a dashboard that was deployed from `main`.

The `grafana-preview` presubmit is optional and will never block a merge. To run it again, push new dashboard changes or comment `/test grafana-preview` on the PR.
