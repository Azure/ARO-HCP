...
4. build only the downstream images needed by the requested target

That last point matters: ci-operator does **not** always build every image declared in `Azure-ARO-HCP-main.yaml`. It builds the transitive closure required by the target node it is trying to run.

So a local E2E target and a persistent-environment E2E target can start from the same config file and still build very different image graphs.
...
- ci-operator leases for the managed-identity container pool and the DEV MSI mock SP pool
