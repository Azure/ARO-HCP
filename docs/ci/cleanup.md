...
Because the `createdAt` tag is required for every `action: delete` rule, this policy is a hard per-subscription prerequisite: `rg-ordered` must not be enabled in a subscription (including the int, stg, and prod e2e subscriptions) until the `createdAt` policy is assigned there. Groups created before the policy was assigned have no tag and are never candidates. Until the policy is in place, only `shared-leftovers` is safe to run in that subscription, since it does not delete resource groups.
