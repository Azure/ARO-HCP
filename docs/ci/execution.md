...
For every environment, each test or spec creates its own **test resource group** in a testing subscription. The request path then diverges:
...
In all cases, the RP creates the ARO HCP cluster resource back into that test's resource group.
...
DEV local E2E also spreads ARM read traffic by leasing one MSI mock service principal from a Boskos-backed pool:
...
This pool exists so concurrent DEV jobs do not all consume ARM throttle budget through the same actor.
...
For how to identify a specific EV2-triggered run after it has started, see [CI Operations](operations.md#understanding-ev2-triggered-runs).
