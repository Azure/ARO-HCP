...
#### DataPlaneOIDCFederationDesiredState
...
| Actor | When |
|-------|------|
| [DataPlaneOIDCFederationDesiredState](#dataplaneoidcfederationdesiredstate) | Sets `Phase` to `PendingConfigure` for fully resolved data-plane identities in `ManagedIdentityDetails` (`MSIBasedDetails` false), and to `PendingDeconfigure` for identities that have left that data-plane set (including during cluster deletion, and even if the same UAMI is still present as an MSI-based CP or SMI entry). A first transition to `PendingDeconfigure` also stamps `DeconfigureTimestamp` to now (when deconfigure was requested). |
...
Two writers split by concern: the desired-state controller only marks phases from `ManagedIdentityDetails` and stamps `DeconfigureTimestamp` when deconfigure is requested; the executor is the only actor that calls Azure and writes the federated identity credential resource IDs.
