...
| Read | `ServiceProviderCluster` | <ul><li>`Status.ManagedIdentityDetails` (NeedsWork: `MetadataFromARMUserAssignedIdentitiesAPI` ClientID, PrincipalID, and TenantID for data-plane operator ResourceIDs become the desired federation set. Dataplane and hardcoded-identity metadata are ignored)</li><li>`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation` (NeedsWork: compared to the computed desired map; entries not in the resolved data-plane operator set are marked PendingDeconfigure)</li></ul> |
| **Write** | **`ServiceProviderCluster`** | <ul><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<key>].Phase`** = new fully resolved data-plane identities (and previously Deconfigured identities that are desired again) are set to `PendingConfigure`; Configured / PendingConfigure entries that are still resolved are left as-is; identities no longer in the resolved data-plane operator set (or during cluster deletion) are set to `PendingDeconfigure` unless already `Deconfigured` or `PendingDeconfigure`</li><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<key>].DeconfigureTimestamp`** = now on the first transition to `PendingDeconfigure` (when deconfigure was requested), including during cluster deletion; already `PendingDeconfigure` / `Deconfigured` entries keep their existing timestamp; cleared when flipping back to `PendingConfigure`</li></ul> |
...
Two writers split by concern: DataPlaneOIDCFederationIntent only marks phases from `ManagedIdentityDetails` ARM User Assigned Identities metadata and stamps `DeconfigureTimestamp` when deconfigure is requested; DataPlaneOIDCFederation is the only actor that calls Azure and writes the federated identity credential resource IDs for the federation map. Read by [ClusterChildResourcesCleanupController](#clusterchildresourcescleanupcontroller) to gate deletion of the `ServiceProviderCluster` document until no FIC IDs remain.
