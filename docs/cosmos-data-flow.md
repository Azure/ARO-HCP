...
| Read | `HCPOpenShiftCluster` (rollout-race only) | <ul><li>`ServiceProviderProperties.PendingClusterServiceID` (triggers the Cluster Service backfill when both Spec and Status are nil)</li></ul> |
...
| **Write** | **`ServiceProviderCluster`** | <ul><li>**`Spec.ManagementClusterResourceID`** = chosen (or backfilled) management cluster resource ID (conflict-retried)</li><li>**`Spec.ManagementClusterPlacementTime`** = current time, set once atomically with the intent and preserved across re-writes/backfills (conflict-retried)</li></ul> |
...
This is the *desired* placement (scheduler intent), owned solely by the PlacementController. It is read by `ClusterPendingClusterServiceIDAssign` (gate: must be non-nil before a pending CS ID is assigned) and `ClusterClusterServiceCreate` (resolves the placed management cluster to pin the CS provision shard).

### `ServiceProviderCluster.Spec.ManagementClusterPlacementTime`

| Actor | When |
|-------|------|
| [PlacementController](#placementcontroller) | Stamps the current time when placement intent is first recorded (atomically with `Spec.ManagementClusterResourceID`); preserved across later re-writes/backfills |

Marks the moment of placement (not the latest reconcile), owned solely by the PlacementController. Exposed kube-state-metrics style as the `backend_cluster_placement_time_seconds` gauge — a unix timestamp (seconds), not a duration. Time-to-placement is computed in PromQL against the cluster's creation timestamp (`placement_ts - creation_ts`).
