...
It records the deny assignment resource ID as `PendingAzureResource` and persists that intent **before** CreateOrUpdate ("set pending before Azure") when the ID is not already `AzureResource`. A write failure is returned so Azure is not called and the workqueue retries. `PendingDeconfigure` deletes IDs already tracked on `AzureResource` / `PendingAzureResource`. A second pass Gets each desired deny assignment and CreateOrUpdates only when it is missing or permissions / excluded principals drifted. Azure errors on one type do not skip the rest: remaining types still run, and a partial status is persisted before the error is returned so the workqueue retries only the unfinished work. Successful configure/deconfigure persists `Configured` / `Deconfigured` and a jittered 12h `EarliestRecheckTime` even when Azure was a no-op Get. After a successful PUT, `ExcludedIdentities` is rewritten to match Azure: live desired principals get `ObservedIdentity`, and wait-elapsed or LRU-dropped waiters are deleted.
