...
| Actor | When |
|-------|------|
| [DataPlaneOIDCFederationDesiredState](#dataplaneoidcfederationdesiredstate) | Sets `Phase` to `PendingConfigure` for fully resolved identities in `ManagedIdentityDetails`, and to `PendingDeconfigure` for identities that have left that set (including during cluster deletion) |
...
Two writers split by concern: the desired-state controller only marks phases from `ManagedIdentityDetails`; the executor is the only actor that calls Azure and writes the federated identity credential resource IDs.
