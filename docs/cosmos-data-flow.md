...
| Read | `ServiceProviderCluster` | <ul><li>`Status.ManagementClusterResourceID`</li><li>`Status.MaestroReadonlyBundles`</li><li>`Status.AzureResources.ManagedResourceGroup` (gate: ServiceProviderCluster is not deleted while `AzureResource` or `PendingAzureResource` is set)</li><li>`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>|<operatorName>].AzureResources` / `PendingAzureResources` (gate: ServiceProviderCluster is not deleted while any assignment still has tracked federated identity credential IDs)</li></ul> |
...
Marks data-plane OIDC federation entries from Cluster `DataPlaneOperators` and `Status.ManagedIdentityDetails` ARM User Assigned Identities metadata. The map is keyed by `(lowercased identity ResourceID, operator name)`. Does not call Azure.
...
| **Write** | **`ServiceProviderCluster`** | <ul><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].TargetIdentity`** = ARM ClientID/PrincipalID/TenantID for the identity instance, copied onto every assignment that shares that ResourceID</li><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>]`** = created for each desired operator on that identity; `DeconfigureTimestamp` is cleared when the operator is desired again</li><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].EnsuredIdentity` / `AzureResources` / `PendingAzureResources`** = cleared on an assignment when its TargetIdentity does not match the ARM snapshot for that ResourceID (identity recreation removes child Azure FederatedIdentityCredential resources), including draining neighbors whose TargetIdentity still lags. Assignments whose TargetIdentity already matches are left as-is</li><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].DeconfigureTimestamp`** = stamped on first transition when that operator leaves this identity (including when another operator on the same identity has unresolved ARM metadata; assignments with nothing tracked to delete are dropped). Does not write `Spec.EarliestRecheckTimesByController`</li></ul> |
...
**Trigger:** Cluster informer, 1-minute resync
**Gate (needsWork):**
- A desired operator (still in `DataPlaneOperators` for this identity, `DeconfigureTimestamp` nil) is not yet ensured (`EnsuredIdentity` is nil or does not equal `TargetIdentity`) and the cluster is not deleting: true (ignores `Spec.EarliestRecheckTimesByController["DataPlaneOIDCFederation"]`)
...
- An operator deconfigure is ready (`DeconfigureTimestamp` set and 24h elapsed, or cluster deleting): true (ignores that recheck time)
...
Creates and deletes Azure federated identity credentials per assignment under `Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation`. When needsWork is true, every assigned operator is reconciled this pass (Get/CreateOrUpdate/delete extras on that operator's tracked FIC IDs). Loop 1 only adds FIC IDs to that assignment's `PendingAzureResources`. Deconfigure still waits 24h on a live cluster. The recheck time is set to now + jittered 1h when Azure work this pass succeeded and remaining entries are idle (all desired operators ensured, or ensured plus deconfigure still inside the 24h wait). The assignment is deleted after a successful deconfigure. Immediate work remaining leaves any existing recheck time in place.
...
| Read | `ServiceProviderCluster` | <ul><li>`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].TargetIdentity` (NeedsWork: compared to EnsuredIdentity)</li><li>`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].EnsuredIdentity` (NeedsWork: ensure complete for this operator)</li><li>`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].AzureResources` (NeedsWork: compared to desired FIC set)</li><li>`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].PendingAzureResources`</li><li>`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].DeconfigureTimestamp` (NeedsWork: 24h wait, or immediate when deleting)</li><li>`Spec.EarliestRecheckTimesByController["DataPlaneOIDCFederation"]` (NeedsWork: honored only for ensured operators when the desired FIC set is unchanged)</li></ul> |
| Read | Azure (FederatedIdentityCredentialsClient) | <ul><li>`Get` / `CreateOrUpdate` / `Delete` per desired or tracked FIC</li></ul> |
| **Write** | **`ServiceProviderCluster`** | <ul><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].EnsuredIdentity`** = copy of TargetIdentity after every desired FIC for that operator is ensured; cleared when a desired FIC that was not already on AzureResources fails CreateOrUpdate; left unchanged when a failed recheck is only of already confirmed FICs; the assignment key is deleted after a successful deconfigure</li><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].PendingAzureResources`** = FIC IDs persisted before CreateOrUpdate (append-only until Azure Delete); cleared on full ensure success</li><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<identityResourceID>\|<operatorName>].AzureResources`** = confirmed FIC IDs for that operator</li><li>**`Spec.EarliestRecheckTimesByController["DataPlaneOIDCFederation"]`** = now + jittered 1h interval when remaining entries are idle after a successful pass; deleted when the federation map is empty; left unchanged when immediate work remains or Azure errors were accumulated</li></ul> |
...
| Actor | When |
|-------|------|
| [DataPlaneOIDCFederationIntent](#dataplaneoidcfederationintent) | Adds desired assignment keys with TargetIdentity from ARM metadata, copies that instance onto every assignment that shares the identity ResourceID, clears EnsuredIdentity and FIC lists on an assignment when its TargetIdentity does not match that instance, and stamps DeconfigureTimestamp on an operator that leaves this identity |
| [DataPlaneOIDCFederation](#dataplaneoidcfederation) | Persists per-assignment pending/confirmed FIC resource IDs, copies TargetIdentity into EnsuredIdentity after every desired FIC is ensured, clears EnsuredIdentity when a new desired FIC fails CreateOrUpdate, removes the assignment after a successful deconfigure. Sets `Spec.EarliestRecheckTimesByController["DataPlaneOIDCFederation"]` when remaining entries are idle |

Two writers. Intent owns TargetIdentity, assignment DeconfigureTimestamp, and clearing EnsuredIdentity plus FIC lists on an assignment when its TargetIdentity does not match the ARM snapshot for that ResourceID; the executor owns EnsuredIdentity after a successful ensure (and clears it when a new desired FIC fails CreateOrUpdate), Azure FIC tracking, and the controller-level recheck time.
