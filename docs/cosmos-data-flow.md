...
- Not deleting (reconcile): computes the expected role assignments by pairing each control-plane operator (`CustomerProperties...UserAssignedIdentities.ControlPlaneOperators`) and data-plane operator (`...DataPlaneOperators`) identity's resolved `PrincipalID` (from `Status.MSIManagedIdentities` / `Status.DataPlaneOperatorsManagedIdentities`) with that operator's role definitions (from the cluster-scoped identities config), scoped to the managed resource group. The role assignment name is generated with a deterministic UUIDv5 algorithm (see the `roleassignment` package). It then runs a **single loop** over the expected set, querying Azure once per assignment with `GetByID`:
  - **exists** → recorded as confirmed (`AzureResources`).
  - **not found** (`RoleAssignmentNotFound`) → recorded as pending (`PendingAzureResources`) and created (`PrincipalID`, `RoleDefinitionID`, `PrincipalType=ServicePrincipal`); a create that races with a concurrent creator surfaces as `RoleAssignmentExists` and is swallowed. A freshly-created assignment stays **pending** this pass and confirms on a later `GetByID`.
  - **other error** (on Get or Create) → the assignment is left pending (not confirmed) and the error is returned so the sync retries.
- After the loop `PendingAzureResources` and `AzureResources` are overwritten from this pass, so every expected assignment is tracked in exactly one of them and nothing else. When nothing is pending (every expected assignment confirmed) `EarliestRecheckTime` is set to a future time — the only thing that stops the loop re-running on every resync; while work remains it is cleared. A `nil` `EarliestRecheckTime` is treated as due, so an existing cluster does one recheck pass on rollout. Any previously-confirmed assignment that is no longer expected is logged (an accepted leak — there will be none until managed identity replacement is supported) but is neither tracked nor deleted.
...
| Read | Azure (RoleAssignmentsClient) | <ul><li>`GetByID` once per expected role assignment in the single reconcile loop (which also serves as the earliest-recheck verification) -> exists / `RoleAssignmentNotFound`</li></ul> |
| **Write** | Azure (RoleAssignmentsClient) | <ul><li>`Create` per expected role assignment Azure reports missing (`PrincipalID`, `RoleDefinitionID`, `PrincipalType=ServicePrincipal`); `RoleAssignmentExists` is treated as success</li></ul> |
| **Write** | **`ServiceProviderCluster`** | <ul><li>**`Status.AzureResources.RoleAssignments.PendingAzureResources`** = overwritten each pass with the expected role assignment IDs not confirmed this pass (freshly-created ones stay pending until a later pass confirms them); confirmed or no-longer-expected IDs drop out</li><li>**`Status.AzureResources.RoleAssignments.AzureResources`** = overwritten each pass with the expected role assignment IDs confirmed to exist in Azure (a previously-confirmed assignment no longer expected is logged and dropped, not tracked)</li><li>**`Status.AzureResources.RoleAssignments.EarliestRecheckTime`** = now + a jittered recheck interval when nothing is pending (every expected assignment confirmed); cleared while work remains</li></ul> |
...
| Actor | When |
|-------|------|
| [IdentityRoleAssignments](#identityroleassignments) | Manages (creates + observes): while the cluster is not being deleted and the managed resource group is confirmed, runs a single loop over the expected control-plane / data-plane operator role assignments, querying each with `GetByID` — an existing one is confirmed (`AzureResources`), a missing one is recorded pending and created (idempotent by deterministic name; `RoleAssignmentExists` treated as success) and stays pending until a later pass confirms it. `PendingAzureResources` / `AzureResources` are overwritten each pass; when nothing is pending `EarliestRecheckTime` is set to a jittered future time (the loop otherwise re-runs on that cadence, re-verifying the confirmed set). A previously-confirmed assignment no longer expected is logged (accepted leak) but not tracked. Deletion is a no-op (the managed resource group deletion cascade removes the role assignments). |
