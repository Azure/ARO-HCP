...
| Read | `ServiceProviderCluster` | <ul><li>`Status.ManagedIdentityDetails` (NeedsWork: identities with non-empty ClientID, PrincipalID, and TenantID become the desired federation set)</li><li>`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation` (NeedsWork: compared to the computed desired map; entries not in the resolved details set are marked PendingDeconfigure)</li></ul> |
| **Write** | **`ServiceProviderCluster`** | <ul><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<key>].Phase`** = new fully resolved identities (and previously Deconfigured identities that are desired again) are set to `PendingConfigure`; Configured / PendingConfigure entries that are still resolved are left as-is; identities no longer in the resolved details set (or during cluster deletion) are set to `PendingDeconfigure` unless already `Deconfigured` or `PendingDeconfigure`</li><li>**`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation[<key>].DeconfigureTimestamp`** = now on the first transition to `PendingDeconfigure` (when deconfigure was requested), including during cluster deletion; already `PendingDeconfigure` / `Deconfigured` entries keep their existing timestamp; cleared when flipping back to `PendingConfigure`</li></ul> |
...
| Actor | When |
|-------|------|
| [DataPlaneOIDCFederationDesiredState](#dataplaneoidcfederationdesiredstate) | Sets `Phase` to `PendingConfigure` for fully resolved identities in `ManagedIdentityDetails`, and to `PendingDeconfigure` for identities that have left that set (including during cluster deletion). A first transition to `PendingDeconfigure` also stamps `DeconfigureTimestamp` to now (when deconfigure was requested). |
