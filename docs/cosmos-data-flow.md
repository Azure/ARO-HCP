...
- Not deleting (reconcile): records the managed resource group as `PendingAzureResource` and persists that intent **before** querying Azure ("set pending before Get"), so a Get failure — or a resource group that does not exist yet — still leaves a durable pending marker (keeping the deletion gate closed) rather than an empty reference. It then queries Azure and switches on the result:
  - **not found** → creates the resource group via `CreateOrUpdate`, claiming ownership by setting `ManagedBy` to this cluster's ID, and validates that its provisioning state is `Succeeded`; on success it clears `PendingAzureResource` and records the resource group as `AzureResource`. A create failure (or a non-`Succeeded` state) returns an error so a later pass retries with the pending marker still in place.
