...
| Read | `ServiceProviderCluster` | <ul><li>`Status.ManagementClusterResourceID`</li><li>`Status.MaestroReadonlyBundles`</li><li>`Status.AzureResources.ManagedResourceGroup` (gate: ServiceProviderCluster is not deleted while `AzureResource` or `PendingAzureResource` is set)</li><li>`Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation` (gate: ServiceProviderCluster is not deleted while any entry still has confirmed or pending federated identity credential resource IDs)</li></ul> |
| Read | Child NodePools | <ul><li>list (must be empty)</li></ul> |
| Read | Child ExternalAuths | <ul><li>list (must be empty)</li></ul> |
| **Write** | Child Cosmos docs | <ul><li>**DELETES** ServiceProviderCluster (when the managed resource group is reflected as gone, data-plane OIDC federation has no remaining FIC IDs, MaestroReadonlyBundles empty, and kube-applier desires gone)</li><li>**DELETES** ManagementClusterContent docs</li><li>**DELETES** kube-applier desire documents</li></ul> |
...
**Trigger:** Cluster informer, 1-minute resync
**Gate (needsWork):** computed deny assignment map differs from `ServiceProviderCluster.Status.DenyAssignmentsV2`. The desired type set is the deny assignment definitions for the cluster. Types are added as `PendingConfigure` when the identities they exclude have resolved principal IDs on `Status.MSIManagedIdentities` / `Status.DataPlaneOperatorsManagedIdentities`. Nested `ExcludedIdentities` are added as `PendingConfigure` for those principals and `ObservedIdentity` is filled from resolved ClientID/TenantID/PrincipalID (`Status.MSIManagedIdentities` / `Status.DataPlaneOperatorsManagedIdentities`, overwritten from `Status.ManagedIdentityDetails` ARM metadata when that source is fully resolved and the ARM PrincipalID matches). Identities that left a type (or whose PrincipalID changed) are marked `PendingDeconfigure` with `DeconfigureTimestamp` and keep the previous `ObservedIdentity`. A ClientID or TenantID change on the same ResourceID+PrincipalID key updates `ObservedIdentity` and sets that identity to `PendingConfigure` without a 24h deconfigure. Types whose identities are not yet resolved are not added; already-tracked types that are still required are left as-is (unresolved ResourceIDs are not deconfigured). Types no longer in the definition set are marked `PendingDeconfigure`. Cluster deletion (`HCPOpenShiftCluster.ServiceProviderProperties.DeletionTimestamp` != nil) is a no-op: deny assignments are scoped to the managed resource group and Azure cascade-deletes them.
...
| Read | `ServiceProviderCluster` | <ul><li>`Status.MSIManagedIdentities.ControlPlaneOperatorsIdentities` / `ServiceManagedIdentity` (NeedsWork: PrincipalID must be resolved for every identity excluded by a required type before that type is added; ClientID is copied onto ObservedIdentity)</li><li>`Status.DataPlaneOperatorsManagedIdentities.Identities` (NeedsWork: PrincipalID must be resolved for every data-plane identity excluded by a required type before that type is added; ClientID is copied onto ObservedIdentity)</li><li>`Status.ManagedIdentityDetails[].MetadataFromARMUserAssignedIdentitiesAPI` (NeedsWork: when HasResolvedIdentityInformation is true and ARM PrincipalID matches the key, ClientID/TenantID/PrincipalID overwrite ObservedIdentity)</li><li>`Status.DenyAssignmentsV2` (NeedsWork: compared to the computed desired map; types not in the definition set are marked PendingDeconfigure; nested ObservedIdentity is compared so a ClientID/TenantID change is PendingConfigure)</li></ul> |
| **Write** | **`ServiceProviderCluster`** | <ul><li>**`Status.DenyAssignmentsV2[<type>].Phase`** = new fully resolved required types (and previously Deconfigured types that are required again) are set to `PendingConfigure`; Configured / PendingConfigure entries that are still required are left as-is; types no longer in the definition set are set to `PendingDeconfigure` unless already `Deconfigured` or `PendingDeconfigure`</li><li>**`Status.DenyAssignmentsV2[<type>].ExcludedIdentities[<key>].Phase`** = desired identities (ResourceID + PrincipalID) are set to `PendingConfigure`; leftover identities (including a PrincipalID change on the same ResourceID) are set to `PendingDeconfigure` unless already `Deconfigured` or `PendingDeconfigure`; a ClientID or TenantID change on a still-desired key is set to `PendingConfigure`</li><li>**`Status.DenyAssignmentsV2[<type>].ExcludedIdentities[<key>].DeconfigureTimestamp`** = now on the first transition to `PendingDeconfigure`; cleared when a previously deconfigured identity is desired again, and cleared when a still-desired key is set to `PendingConfigure` because ObservedIdentity changed</li><li>**`Status.DenyAssignmentsV2[<type>].ExcludedIdentities[<key>].ObservedIdentity`** = ClientID, TenantID, and PrincipalID for still-desired keys; PendingDeconfigure rows keep the snapshot from when the principal left</li></ul>
...
**Trigger:** Cluster informer, 1-minute resync
**Gate (needsWork):** at least one `Status.DenyAssignmentsV2` entry has phase `PendingConfigure` or `PendingDeconfigure` while `HCPOpenShiftCluster.ServiceProviderProperties.DeletionTimestamp` is nil; or `Configured` while `DeletionTimestamp` is nil and either that entry's `ExcludedIdentities` has `PendingConfigure` or a `PendingDeconfigure` wait that has elapsed, or `EarliestRecheckTime` is nil or already past. Cluster deletion skips all work so this controller does not create or delete deny assignments while Azure cascade-deletes them with the managed resource group.

It records the deny assignment resource ID as `PendingAzureResource` and persists that intent **before** CreateOrUpdate ("set pending before Azure") when the ID is not already `AzureResource`. A write failure is returned so Azure is not called and the workqueue retries. `PendingDeconfigure` deletes IDs already tracked on `AzureResource` / `PendingAzureResource`. A second pass Gets each desired deny assignment and CreateOrUpdates only when it is missing or permissions / excluded principals drifted. Azure errors on one type do not skip the rest: remaining types still run, and a partial status is persisted before the error is returned so the workqueue retries only the unfinished work. Successful configure/deconfigure persists `Configured` / `Deconfigured` and a jittered 12h `EarliestRecheckTime` even when Azure was a no-op Get.

| | Object | Fields |
|---|--------|--------|
| Read | `HCPOpenShiftCluster` | <ul><li>`ServiceProviderProperties.DeletionTimestamp` (NeedsWork / SyncOnce: when set, all phases are skipped)</li><li>`ServiceProviderProperties.ClusterServiceID` / `PendingClusterServiceID` (required for PendingConfigure and Configured: CS cluster ID used to generate the deny assignment UUID; PendingDeconfigure does not need it)</li><li>`CustomerProperties.Platform.ManagedResourceGroup` (scope and resource ID for PendingConfigure and Configured)</li><li>`CustomerProperties.Etcd.DataEncryption` (selects whether the KeyVault definition also excludes the KMS operator)</li><li>`ID` (subscription / resource group / name)</li></ul> |
| Read | `Subscription` | <ul><li>`Properties.TenantId` (to build the FPA deny assignment and generic resources clients)</li></ul> |
| Read | `ServiceProviderCluster` | <ul><li>`Status.DenyAssignmentsV2[].Phase` (NeedsWork: PendingConfigure, PendingDeconfigure, or Configured; all ignored while DeletionTimestamp is set)</li><li>`Status.DenyAssignmentsV2[].EarliestRecheckTime` (NeedsWork: ignored for PendingConfigure and PendingDeconfigure; Configured honors it unless ExcludedIdentities needs work)</li><li>`Status.DenyAssignmentsV2[].PendingAzureResource` / `AzureResource` (PendingDeconfigure deletes these tracked IDs; PendingConfigure persists PendingAzureResource before Azure; AzureResource is set when the deny assignment object exists)</li><li>`Status.DenyAssignmentsV2[].ExcludedIdentities` (NeedsWork: PendingConfigure, or PendingDeconfigure after 24h; SyncOnce: principal IDs for Azure ExcludePrincipals, including a 24h keep and LRU drop of older waiters at the 25-principal Azure limit)</li></ul> |
...
| **Write** | **`ServiceProviderCluster`** | <ul><li>**`Status.DenyAssignmentsV2[<type>].Phase`** = `Configured` after the desired deny assignment is ensured, or `Deconfigured` after tracked IDs are deleted; left unchanged when the Azure call fails</li><li>**`Status.DenyAssignmentsV2[<type>].PendingAzureResource`** = the deny assignment resource ID persisted **before** CreateOrUpdate ("set pending before Azure") when it is not already `AzureResource`; cleared after a successful ensure or delete</li><li>**`Status.DenyAssignmentsV2[<type>].AzureResource`** = the deny assignment resource ID after a fully successful ensure (object exists; not gated on every identity being Configured); left unchanged on a failed ensure; cleared after a fully successful delete; on a partial deconfigure, the ID Azure did not delete</li><li>**`Status.DenyAssignmentsV2[<type>].EarliestRecheckTime`** = now + jittered 12h interval after a fully successful configure (including a no-op Get) or deconfigure; not advanced when the Azure call fails</li><li>**`Status.DenyAssignmentsV2[<type>].ExcludedIdentities[<key>].Phase`** = `Configured` after a successful PUT that included that principal; `Deconfigured` after a successful PUT that omitted a wait-elapsed or LRU-evicted `PendingDeconfigure` principal. ObservedIdentity is not written by this controller.</li></ul>
...
**File:** [role_assignment_intent_controller.go](../backend/pkg/controllers/cluster/roleassignments/role_assignment_intent_controller.go)
**Trigger:** Cluster informer, 1-minute resync
**Gate (needsWork):** computed role assignment map differs from `ServiceProviderCluster.Status.RoleAssignmentsV2`. Desired keys are each control-plane and data-plane operator identity (`CustomerProperties.Platform.OperatorsAuthentication.UserAssignedIdentities.ControlPlaneOperators` / `DataPlaneOperators`) paired with that operator's role definitions from the cluster-scoped identities config. Keys are added as `PendingConfigure` when the identity has a resolved principal ID on `Status.MSIManagedIdentities` / `Status.DataPlaneOperatorsManagedIdentities`. `ObservedIdentity` is filled from resolved ClientID/TenantID/PrincipalID (`Status.MSIManagedIdentities` / `Status.DataPlaneOperatorsManagedIdentities`, overwritten from `Status.ManagedIdentityDetails` ARM metadata when that source is fully resolved and the ARM PrincipalID matches). Leftover keys (including a PrincipalID change on the same ResourceID and role definition) are marked `PendingDeconfigure` with `DeconfigureTimestamp` and keep the previous `ObservedIdentity`. A ClientID or TenantID change on the same ResourceID+PrincipalID+RoleDefinitionResourceID key updates `ObservedIdentity` and sets that entry to `PendingConfigure` without a 24h deconfigure. Unresolved ResourceIDs are not deconfigured. Cluster deletion (`HCPOpenShiftCluster.ServiceProviderProperties.DeletionTimestamp` != nil) is a no-op: role assignments are scoped to the managed resource group and Azure cascade-deletes them.

| | Object | Fields |
|---|--------|--------|
| Read | `HCPOpenShiftCluster` | <ul><li>`ServiceProviderProperties.DeletionTimestamp` (SyncOnce: must be nil)</li><li>`CustomerProperties.Platform.OperatorsAuthentication.UserAssignedIdentities.ControlPlaneOperators` / `DataPlaneOperators` (operator identities to enumerate; the service managed identity is not included)</li><li>`ID` (subscription / resource group / name)</li></ul> |
| Read | `ServiceProviderCluster` | <ul><li>`Status.MSIManagedIdentities.ControlPlaneOperatorsIdentities` (NeedsWork: PrincipalID must be resolved before that identity's keys are added; ClientID is copied onto ObservedIdentity)</li><li>`Status.DataPlaneOperatorsManagedIdentities.Identities` (NeedsWork: PrincipalID must be resolved before that identity's keys are added; ClientID is copied onto ObservedIdentity)</li><li>`Status.ManagedIdentityDetails[].MetadataFromARMUserAssignedIdentitiesAPI` (NeedsWork: when HasResolvedIdentityInformation is true and ARM PrincipalID matches the key, ClientID/TenantID/PrincipalID overwrite ObservedIdentity)</li><li>`Status.RoleAssignmentsV2` (NeedsWork: compared to the computed desired map; leftover keys are marked PendingDeconfigure; ObservedIdentity is compared so a ClientID/TenantID change is PendingConfigure)</li></ul> |
| **Write** | **`ServiceProviderCluster`** | <ul><li>**`Status.RoleAssignmentsV2[<key>].Phase`** = new fully resolved desired keys (and previously Deconfigured keys that are desired again) are set to `PendingConfigure`; Configured / PendingConfigure entries that are still desired are left as-is unless ObservedIdentity changed; leftover keys are set to `PendingDeconfigure` unless already `Deconfigured` or `PendingDeconfigure`</li><li>**`Status.RoleAssignmentsV2[<key>].DeconfigureTimestamp`** = now on the first transition to `PendingDeconfigure`; cleared when a previously deconfigured key is desired again, and cleared when a still-desired key is set to `PendingConfigure` because ObservedIdentity changed</li><li>**`Status.RoleAssignmentsV2[<key>].ObservedIdentity`** = ClientID, TenantID, and PrincipalID for still-desired keys; PendingDeconfigure rows keep the snapshot from when the key left</li></ul> |

#### ClusterRoleAssignmentV2

**File:** [role_assignment_v2_controller.go](../backend/pkg/controllers/cluster/roleassignments/role_assignment_v2_controller.go)
**Trigger:** Cluster informer, 1-minute resync
**Gate (needsWork):** `Status.AzureResources.ManagedResourceGroup.AzureResource` is set and at least one `Status.RoleAssignmentsV2` entry has phase `PendingConfigure` while `HCPOpenShiftCluster.ServiceProviderProperties.DeletionTimestamp` is nil; or `PendingDeconfigure` after the 24h wait; or `Configured` while `EarliestRecheckTime` is nil or already past. Azure work is skipped until the managed resource group is confirmed. Cluster deletion skips all work so this controller does not create or delete role assignments while Azure cascade-deletes them with the managed resource group.

It records the role assignment resource ID as `PendingAzureResource` and persists that intent **before** Create ("set pending before Azure") when the ID is not already `AzureResource`. A write failure is returned so Azure is not called and the workqueue retries. `PendingDeconfigure` deletes IDs already tracked on `AzureResource` / `PendingAzureResource` after 24 hours. A second pass Gets each desired role assignment and Creates only when it is missing. If Get finds a role assignment whose principal or role definition drifted, it Deletes then Creates. Azure errors on one key do not skip the rest: remaining keys still run, and a partial status is persisted before the error is returned so the workqueue retries only the unfinished work. Successful configure/deconfigure persists `Configured` / `Deconfigured` and a jittered 12h `EarliestRecheckTime` even when Azure was a no-op Get. A `RoleAssignmentExists` result from Create is treated as success. This controller does not write `ObservedIdentity`.

| | Object | Fields |
|---|--------|--------|
| Read | `HCPOpenShiftCluster` | <ul><li>`ServiceProviderProperties.DeletionTimestamp` (NeedsWork / SyncOnce: when set, all phases are skipped)</li><li>`CustomerProperties.Platform.ManagedResourceGroup` (scope and resource ID for PendingConfigure and Configured)</li><li>`ID` (subscription / resource group / name)</li></ul> |
| Read | `Subscription` | <ul><li>`Properties.TenantId` (to build the FPA RoleAssignments client)</li></ul> |
| Read | `ServiceProviderCluster` | <ul><li>`Status.AzureResources.ManagedResourceGroup.AzureResource` (NeedsWork: must be confirmed before any Azure calls)</li><li>`Status.RoleAssignmentsV2[].Phase` (NeedsWork: PendingConfigure, PendingDeconfigure after 24h, or Configured; all ignored while DeletionTimestamp is set)</li><li>`Status.RoleAssignmentsV2[].EarliestRecheckTime` (NeedsWork: ignored for PendingConfigure and PendingDeconfigure; Configured honors it)</li><li>`Status.RoleAssignmentsV2[].DeconfigureTimestamp` (NeedsWork: PendingDeconfigure waits 24h)</li><li>`Status.RoleAssignmentsV2[].PendingAzureResource` / `AzureResource` (PendingDeconfigure deletes these tracked IDs; PendingConfigure persists PendingAzureResource before Azure; AzureResource is set when the role assignment exists with the expected principal and role definition)</li></ul> |
| Read | Azure (RoleAssignmentsClient) | <ul><li>`GetByID` then `Create` for PendingConfigure and Configured (`Create` only when Get is not found; Delete then Create when principal or role definition drifted); skipped when the managed resource group is unconfirmed or when DeletionTimestamp is set</li><li>`DeleteByID` for PendingDeconfigure after 24h (`RoleAssignmentNotFound` is success)</li></ul> |
| **Write** | **`ServiceProviderCluster`** | <ul><li>**`Status.RoleAssignmentsV2[<key>].Phase`** = `Configured` after the desired role assignment is ensured, or `Deconfigured` after tracked IDs are deleted; left unchanged when the Azure call fails</li><li>**`Status.RoleAssignmentsV2[<key>].PendingAzureResource`** = the role assignment resource ID persisted **before** Create ("set pending before Azure") when it is not already `AzureResource`; cleared after a successful ensure or delete</li><li>**`Status.RoleAssignmentsV2[<key>].AzureResource`** = the role assignment resource ID after a fully successful ensure; left unchanged on a failed ensure; cleared after a fully successful delete; on a partial deconfigure, the ID Azure did not delete</li><li>**`Status.RoleAssignmentsV2[<key>].EarliestRecheckTime`** = now + jittered 12h interval after a fully successful configure (including a no-op Get) or deconfigure; not advanced when the Azure call fails</li></ul> |
...
| Actor | When |
|-------|------|
| [ClusterDenyAssignmentIntent](#clusterdenyassignmentintent) | Sets type `Phase` to `PendingConfigure` for deny assignment types in the cluster's definition set once excluded identities have resolved principal IDs, and to `PendingDeconfigure` for types that have left that definition set. Nested `ExcludedIdentities` are set to `PendingConfigure` for desired ResourceID+PrincipalID keys, `ObservedIdentity` is written for those keys, and leftover identities (or a PrincipalID change) are set to `PendingDeconfigure` (stamping `DeconfigureTimestamp`) while keeping the previous `ObservedIdentity`. A ClientID or TenantID change on the same key updates `ObservedIdentity` and sets `PendingConfigure` without a 24h deconfigure. Identities that are temporarily unresolved leave still-required types as-is. Cluster deletion is a no-op. |
| [ClusterDenyAssignmentV2](#clusterdenyassignmentv2) | Persists `PendingAzureResource` before CreateOrUpdate, Gets the deny assignment and CreateOrUpdates only when missing or permissions / excluded principals drifted, deletes tracked resource IDs for type `PendingDeconfigure`, then sets type `Phase` to `Configured` or `Deconfigured` and writes `AzureResource` and `EarliestRecheckTime` when the Azure call for that type succeeds. ExcludePrincipals is the union of live identity phases plus `PendingDeconfigure` principals still inside 24h, capped at 25 (oldest waiters dropped). Successful PUTs advance identity phases to `Configured` or `Deconfigured`. Cluster deletion is a no-op: Azure cascade-deletes deny assignments with the managed resource group. |

Two writers split by concern: ClusterDenyAssignmentIntent only marks type and nested identity phases from the required definition set and resolved identities, writes `ObservedIdentity` for still-desired keys, and stamps `DeconfigureTimestamp` when an identity deconfigure is requested; ClusterDenyAssignmentV2 is the only actor that calls Azure and writes the deny assignment resource IDs and ExcludePrincipals. It does not write `ObservedIdentity`. Not read by [ClusterChildResourcesCleanupController](#clusterchildresourcescleanupcontroller): deny assignments are scoped to the managed resource group and are cascade-deleted with it.

### `ServiceProviderCluster.Status.RoleAssignmentsV2`

| Actor | When |
|-------|------|
| [ClusterRoleAssignmentIntent](#clusterroleassignmentintent) | Sets `Phase` to `PendingConfigure` for each control-plane / data-plane operator identity and role definition once the principal ID is resolved, and to `PendingDeconfigure` for keys that have left that desired set. `ObservedIdentity` is written for still-desired keys. Leftover keys (or a PrincipalID change) are set to `PendingDeconfigure` (stamping `DeconfigureTimestamp`) while keeping the previous `ObservedIdentity`. A ClientID or TenantID change on the same key updates `ObservedIdentity` and sets `PendingConfigure` without a 24h deconfigure. Unresolved ResourceIDs are not deconfigured. Cluster deletion is a no-op. |
| [ClusterRoleAssignmentV2](#clusterroleassignmentv2) | Persists `PendingAzureResource` before Create, Gets the role assignment and Creates only when missing, Deletes then Creates when principal or role definition drifted, deletes tracked resource IDs for `PendingDeconfigure` after 24h, then sets `Phase` to `Configured` or `Deconfigured` and writes `AzureResource` and `EarliestRecheckTime` when the Azure call for that key succeeds. Cluster deletion is a no-op: Azure cascade-deletes role assignments with the managed resource group. |

Two writers split by concern: ClusterRoleAssignmentIntent only marks phases from the cluster operators and cluster-scoped identities config and resolved identities, writes `ObservedIdentity` for still-desired keys, and stamps `DeconfigureTimestamp` when a deconfigure is requested; ClusterRoleAssignmentV2 is the only actor that calls Azure and writes the role assignment resource IDs. It does not write `ObservedIdentity`. Not read by [ClusterChildResourcesCleanupController](#clusterchildresourcescleanupcontroller): role assignments are scoped to the managed resource group and are cascade-deleted with it. The create-completion gate still reads [ObserveRoleAssignments](#observeroleassignments) (`Status.AzureResources.RoleAssignments`) until Cluster Service hands off creation.
