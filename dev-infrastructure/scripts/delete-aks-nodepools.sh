...
#   POOL_NAME_PATTERN - Regex pattern to match nodepool names (for example: "infra[0-9]+")

echo "Listing user-mode nodepools matching '${POOL_NAME_PATTERN}' on cluster '${CLUSTER_NAME}' in RG '${RESOURCE_GROUP}'..."
...
if [ -z "${POOLS}" ]; then
  echo "No user-mode nodepools found matching '${POOL_NAME_PATTERN}'. Nothing to do."
  exit 0
fi
...
echo "All matching user-mode nodepools have been processed."
