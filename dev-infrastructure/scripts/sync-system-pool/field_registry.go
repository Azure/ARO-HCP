...
var agentPoolFieldReasons = map[string]string{
	...
	"NetworkProfile":             "ignored: AKS always populates this with its own defaults even when omitted from the request; not config-driven, comparing it produces guaranteed false-positive drift on every pool",
	...
	"ScaleDownMode":              "ignored: AKS always populates this with its own default (Delete) even when omitted from the request; not config-driven, comparing it produces guaranteed false-positive drift on every pool",
