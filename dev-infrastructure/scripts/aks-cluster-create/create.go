...
func run(ctx context.Context, o *completedOptions, logger logr.Logger) error {
	...
	if existing != nil && !hasProvisioningTag(existing.Tags) {
		...
		resolved, err := computeDesiredPools(ctx, logger, o)
		if err != nil {
			return fmt.Errorf("computing desired pools: %w", err)
		}
		systemPools, infraPools, workerPools := splitPoolsByRole(resolved.Pools)
		...
		logger.Info("cluster already fully provisioned, capacity baseline initialized, no pool changes applied")
		return nil
	}
	...
	managedCluster := buildManagedCluster(o.validatedOptions, bootstrap, networkConfig, provisioningTags(o.validatedOptions, existing))
	...
	if err := finalizeClusterTags(ctx, o); err != nil {
		return err
	}
...
// computeDesiredPools resolves the full profile through the same allocation
// path as the nodepool controller, including whether every tier was satisfied.
func computeDesiredPools(ctx context.Context, logger logr.Logger, o *completedOptions) (compute.DesiredPoolsResult, error) {
	ctx = utils.ContextWithLogger(ctx, logger)
	return compute.ResolveDesiredPools(ctx, o.skuCache, o.subscriptionID, o.profile, o.zones, o.fetchUsage)
}
...
func buildManagedCluster(o *validatedOptions, bootstrap compute.Pool, networkConfig compute.NetworkConfig, tags map[string]*string) armcontainerservice.ManagedCluster {
