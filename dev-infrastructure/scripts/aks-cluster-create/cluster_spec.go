...
// desiredClusterSpec defines the configuration once for both creation and updates.
...
func (o *validatedOptions) desiredClusterSpec(existing *armcontainerservice.ManagedCluster, bootstrap *compute.Pool) (armcontainerservice.ManagedCluster, []string, error) {
	...
	if existing == nil {
		...
		desired.Tags = initialClusterTags(o.clusterTags)
	...
	if len(o.metricLabelsAllowlist) == 0 && (liveMetrics == nil || liveMetrics.MetricLabelsAllowlist == nil) {
		metrics.MetricLabelsAllowlist = nil
	}
	if len(o.metricAnnotationsAllowlist) == 0 && (liveMetrics == nil || liveMetrics.MetricAnnotationsAllowList == nil) {
	...
	// Pools are deployed as ARM child resources, not through the cluster resource.
	// Omitting agentPoolProfiles leaves separately managed pools untouched.
	// KubernetesVersion is owned by a separate upgrade path.
	// Ref: https://learn.microsoft.com/en-us/rest/api/aks/managed-clusters/create-or-update
...
// desired fields are unowned, not requests to clear server defaults.
// Changed paths identify configuration fields, never their values.
func mergeClusterConfiguration(current, desired map[string]any, prefix string) []string {
	...
	for key, value := range desired {
		...
		if object, ok := value.(map[string]any); ok {
			if currentObject, ok := current[key].(map[string]any); ok {
				changed = append(changed, mergeClusterConfiguration(currentObject, object, path)...)
				continue
			}
		}
		if currentValue, exists := current[key]; !exists || !reflect.DeepEqual(currentValue, value) {
			current[key] = value
