...
func newRawOptionsFromEnv(env func(string) string) *rawOptions {
	o := &rawOptions{
		...
		profile: env("PROFILE"),
		// Mirrors fleet's controller default (resolveNodePoolProfile in
		// fleet/cmd/controller/options.go) so an empty ZONES config value behaves
		// the same at cluster-creation time as it does at steady state.
		zones: orDefault(env("ZONES"), "1,2,3"),
...
func (o *rawOptions) Validate(_ context.Context) (*validatedOptions, error) {
	...
	zones := parseCSVList(o.zones)
	if len(zones) != 3 {
		return nil, fmt.Errorf("ZONES: exactly 3 zones required, got %d", len(zones))
	}
...
func (o *rawOptions) validateRequired() error {
...
// orDefault returns val, or def if val is empty.
