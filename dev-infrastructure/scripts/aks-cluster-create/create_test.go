...
func TestTryEnsurePool(t *testing.T) {
	pool := compute.Pool{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err = tryEnsurePool(context.Background(), client, "rg1", "cluster1", pool, networkConfig, logr.Discard())
...
func TestEnsurePool(t *testing.T) {
	...
	newTestClient := func(t *testing.T, srv *armcontainerservicefake.AgentPoolsServer) *armcontainerservice.AgentPoolsClient {
		t.Helper()
		transport := armcontainerservicefake.NewAgentPoolsServerTransport(srv)
		client, err := armcontainerservice.NewAgentPoolsClient("sub1", &azfake.TokenCredential{}, &azcorearm.ClientOptions{
			ClientOptions: policy.ClientOptions{Transport: transport},
	...
	t.Run("succeeds on first attempt without retrying", func(t *testing.T) {
		getCalls := 0
		client := newTestClient(t, &armcontainerservicefake.AgentPoolsServer{
			Get: func(ctx context.Context, resourceGroupName, resourceName, agentPoolName string, options *armcontainerservice.AgentPoolsClientGetOptions) (resp azfake.Responder[armcontainerservice.AgentPoolsClientGetResponse], errResp azfake.ErrorResponder) {
				getCalls++
				errResp.SetResponseError(http.StatusNotFound, "PoolNotFound")
				return
		...
		require.NoError(t, err)
		assert.Equal(t, 1, getCalls, "should not retry once the pool is successfully ensured")
	})

	t.Run("expired context stops retrying after the first failure", func(t *testing.T) {
		getCalls := 0
		client := newTestClient(t, &armcontainerservicefake.AgentPoolsServer{
			Get: func(ctx context.Context, resourceGroupName, resourceName, agentPoolName string, options *armcontainerservice.AgentPoolsClientGetOptions) (resp azfake.Responder[armcontainerservice.AgentPoolsClientGetResponse], errResp azfake.ErrorResponder) {
				getCalls++
		...
		// A short-but-nonzero timeout lets the first (fast, in-memory) fake
		// call complete normally, then expires while ensurePool is waiting
		// between retries, so the retry loop's ctx.Done() branch fires well
		// before the real 30s inter-attempt delay would.
		ctx, cancel := context.WithTimeout(context.Background(), 20*time.Millisecond)
		defer cancel()

		err := ensurePool(ctx, client, "rg1", "cluster1", pool, networkConfig, logr.Discard())

		require.Error(t, err)
		assert.Equal(t, 1, getCalls, "should give up after the first attempt once the context expires")
	})
}
