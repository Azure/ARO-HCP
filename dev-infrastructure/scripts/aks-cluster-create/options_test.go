...
func TestNewRawOptionsFromEnvDefaults(t *testing.T) {
	...
	assert.Equal(t, "1,2,3", o.zones, "ZONES should default like fleet's controller (resolveNodePoolProfile)")
}
...
func TestRawOptionsValidate(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "whitespace only zones",
			overrides: map[string]string{"ZONES": " , "},
			wantErr:   "ZONES: exactly 3 zones required",
		},
		{
			name:      "too few zones",
			overrides: map[string]string{"ZONES": "1,2"},
			wantErr:   "ZONES: exactly 3 zones required, got 2",
		},
		{
