...
@description('The name of the azure cloud to which AFD instance is deployed')
...
// In public cloud we don't add Managed Identity to AFD as the custom domains use managed certificates where FairFax don't support managed Certificate.
var isPublicCloud = (azureCloudName == 'AzureCloud')
...
resource keyVault 'Microsoft.KeyVault/vaults@2022-07-01' existing = if (!isPublicCloud) {
  name: KeyVaultName
...
resource secret 'Microsoft.Cdn/profiles/secrets@2023-05-01' = if (!isPublicCloud) {
...
resource customDomain 'Microsoft.Cdn/profiles/customDomains@2023-05-01' = {
  ...
  properties: {
    ...
    tlsSettings: {
      ...
      certificateType: isPublicCloud ? 'ManagedCertificate' : 'CustomerCertificate'
      secret: isPublicCloud ? null : {
        id: secret.id
      }
    }
    azureDnsZone: {
...
resource validationTxtRecord 'Microsoft.Network/dnsZones/TXT@2018-05-01' = if (isPublicCloud) {
  parent: zone
