    ...
    rules: [
      {
        ...
        annotations: {
          ...
          description: 'etcd gRPC P99 latency in namespace {{ $labels.namespace }} on cluster {{ $labels.cluster }} has exceeded 200ms in both the 1h and 5m windows, indicating a fast burn-rate degradation affecting hosted cluster operations.'
          info: 'etcd gRPC P99 latency in namespace {{ $labels.namespace }} on cluster {{ $labels.cluster }} has exceeded 200ms in both the 1h and 5m windows, indicating a fast burn-rate degradation affecting hosted cluster operations.'
          runbook_url: 'https://aka.ms/arohcp-runbook-etcd'
          summary: 'etcd gRPC P99 latency exceeds 200ms in {{ $labels.namespace }} on {{ $labels.cluster }} (fast burn 1h/5m)'
          title: 'etcd gRPC P99 latency exceeds 200ms in {{ $labels.namespace }} on {{ $labels.cluster }} (fast burn 1h/5m)'
        }
      ...
      }
      {
        ...
        annotations: {
          ...
          description: 'etcd gRPC P99 latency in namespace {{ $labels.namespace }} on cluster {{ $labels.cluster }} has exceeded 200ms in both the 6h and 30m windows, indicating a sustained medium burn-rate degradation affecting hosted cluster operations.'
          info: 'etcd gRPC P99 latency in namespace {{ $labels.namespace }} on cluster {{ $labels.cluster }} has exceeded 200ms in both the 6h and 30m windows, indicating a sustained medium burn-rate degradation affecting hosted cluster operations.'
          runbook_url: 'https://aka.ms/arohcp-runbook-etcd'
          summary: 'etcd gRPC P99 latency exceeds 200ms in {{ $labels.namespace }} on {{ $labels.cluster }} (medium burn 6h/30m)'
          title: 'etcd gRPC P99 latency exceeds 200ms in {{ $labels.namespace }} on {{ $labels.cluster }} (medium burn 6h/30m)'
        }
      ...
      }
      {
        ...
        annotations: {
          ...
          description: 'etcd gRPC P99 latency in namespace {{ $labels.namespace }} on cluster {{ $labels.cluster }} has exceeded 200ms over a 3d window, indicating a slow but persistent latency degradation. This generates a ticket for investigation.'
          info: 'etcd gRPC P99 latency in namespace {{ $labels.namespace }} on cluster {{ $labels.cluster }} has exceeded 200ms over a 3d window, indicating a slow but persistent latency degradation. This generates a ticket for investigation.'
          runbook_url: 'https://aka.ms/arohcp-runbook-etcd'
          summary: 'etcd gRPC P99 latency exceeds 200ms in {{ $labels.namespace }} on {{ $labels.cluster }} (slow burn 3d)'
          title: 'etcd gRPC P99 latency exceeds 200ms in {{ $labels.namespace }} on {{ $labels.cluster }} (slow burn 3d)'
        }
