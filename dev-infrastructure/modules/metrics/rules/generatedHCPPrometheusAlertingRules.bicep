...
resource hcpKmsAvailabilityRules 'Microsoft.AlertsManagement/prometheusRuleGroups@2023-03-01' = {
    ...
    rules: [
      {
        ...
        annotations: {
          ...
          info: 'HostedCluster {{ $labels.name }} (ID: {{ $labels._id }}) ValidAzureKMSConfig availability is burning error budget at 14.4x rate. The customer\'s KMS key may be inaccessible.'
          runbook_url: 'https://aka.ms/arohcp-runbook-kms'
          summary: 'KMS config invalid for HostedCluster {{ $labels.name }}'
          title: 'KMS config invalid for HostedCluster {{ $labels.name }}'
        }
      ...
      }
      {
        ...
        annotations: {
          ...
          info: 'HostedCluster {{ $labels.name }} (ID: {{ $labels._id }}) ValidAzureKMSConfig availability is burning error budget at 6x rate.'
          runbook_url: 'https://aka.ms/arohcp-runbook-kms'
          summary: 'KMS config invalid for HostedCluster {{ $labels.name }}'
          title: 'KMS config invalid for HostedCluster {{ $labels.name }}'
        }
      ...
      }
      {
        ...
        annotations: {
          ...
          info: 'HostedCluster {{ $labels.name }} (ID: {{ $labels._id }}) ValidAzureKMSConfig is slowly burning error budget.'
          runbook_url: 'https://aka.ms/arohcp-runbook-kms'
          summary: 'Slow KMS availability budget burn for HostedCluster {{ $labels.name }}'
          title: 'Slow KMS availability budget burn for HostedCluster {{ $labels.name }}'
        }
