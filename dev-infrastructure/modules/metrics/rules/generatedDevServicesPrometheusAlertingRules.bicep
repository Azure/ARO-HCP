  ...
  properties: {
    ...
    rules: [
      ...
      }
      {
        ...
        expression: '(sum by (controller, cluster, region) (max without (prometheus_replica) (rate(backend_controller_reconcile_errors_total{namespace="aro-hcp"}[10m]))) / clamp_min((sum by (controller, cluster, region) (max without (prometheus_replica) (rate(backend_controller_reconcile_total{namespace="aro-hcp"}[10m]))) - (sum by (controller, cluster, region) (max without (prometheus_replica) (rate(backend_controller_reconcile_cooldown_skips_total{namespace="aro-hcp"}[10m]))) or (sum by (controller, cluster, region) (max without (prometheus_replica) (rate(backend_controller_reconcile_total{namespace="aro-hcp"}[10m]))) * 0))), 0)) > 0.5'
        for: 'PT10M'
        severity: severityCeiling > 0 ? max(3, severityCeiling) : 3
      }
    ]
