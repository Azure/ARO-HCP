    ...
    rules: [
      {
        ...
        expression: 'histogram_quantile(0.99, sum by (cluster, le, region) (max without (prometheus_replica) (rate(ip_assignment_latency_seconds_bucket{cluster=~".*-mgmt(-[0-9]+)?$"}[5m]))))'
      }
    ]
  }
}
    ...
    rules: [
      {
        ...
        expression: 'max without (prometheus_replica) (kube_daemonset_status_number_ready{cluster=~".*-mgmt(-[0-9]+)?$",daemonset="azure-cns",job="kube-state-metrics",namespace="kube-system"}) / clamp_min(max without (prometheus_replica) (kube_daemonset_status_desired_number_scheduled{cluster=~".*-mgmt(-[0-9]+)?$",daemonset="azure-cns",job="kube-state-metrics",namespace="kube-system"}), 1)'
      }
    ]
  }
}
