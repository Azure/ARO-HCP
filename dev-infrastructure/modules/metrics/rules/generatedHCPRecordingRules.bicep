    ...
    rules: [
      {
        ...
        expression: 'sum by (cluster, region) (max without (prometheus_replica) (kube_deployment_status_replicas_available{deployment="router",job="kube-state-metrics",namespace=~"ocm-.*"}))'
      }
      {
        record: 'swift_router_replicas:configured'
        expression: 'sum by (cluster, region) (max without (prometheus_replica) (kube_deployment_spec_replicas{deployment="router",job="kube-state-metrics",namespace=~"ocm-.*"}))'
      }
      {
        record: 'router:startup_latency:seconds'
        expression: '(time() - max without (prometheus_replica) (kube_pod_created{job="kube-state-metrics",namespace=~"ocm-.*"})) * on (cluster, namespace, pod) max without (prometheus_replica) (kube_pod_owner{job="kube-state-metrics",owner_kind="ReplicaSet",owner_name=~"router-.*"}) * on (cluster, namespace, pod) (max without (prometheus_replica) (kube_pod_status_phase{job="kube-state-metrics",phase="Pending"}) == 1)'
      }
      {
