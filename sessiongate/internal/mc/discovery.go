...
type HCPInfo struct {
	...
	APIServerDNSName  string `json:"apiServerDNSName" yaml:"apiServerDNSName"`   // API server DNS name from spec.networking.apiServer.advertiseAddress
}
...
func NewDiscovery(hypershiftClient hypershiftclientset.Interface) *Discovery {
	return &Discovery{
...
func (d *Discovery) ListAllClusters(ctx context.Context) ([]HCPInfo, error) {
	// List all HostedClusters across all namespaces using the hypershift client
	hcList, err := d.hypershiftClient.HypershiftV1beta1().HostedClusters(metav1.NamespaceAll).List(ctx, metav1.ListOptions{})
	if err != nil {
...
func (d *Discovery) DiscoverClusterByResourceID(ctx context.Context, resourceID *azcorearm.ResourceID) (HCPInfo, error) {
	...
	hcList, err := d.hypershiftClient.HypershiftV1beta1().HostedClusters(metav1.NamespaceAll).List(ctx, metav1.ListOptions{LabelSelector: fmt.Sprintf("api.openshift.com/name=%s", clusterName)})
	if err != nil {
...
func findHCPInfoByPredicate(ctx context.Context, items []hypershiftv1beta1.HostedCluster, predicate func(HCPInfo) bool) (HCPInfo, error) {
	...
	for _, item := range items {
		clusterInfo, err := hostedClusterToHCPInfo(&item)
		fmt.Printf("clusterInfo!!!: %s %+v\n", item.Namespace, clusterInfo)
		if err != nil {
...
func hostedClusterToHCPInfo(hostedCluster *hypershiftv1beta1.HostedCluster) (HCPInfo, error) {
	...
	// Get API server DNS name from networking configuration
	apiServerDNSName := hostedCluster.Spec.KubeAPIServerDNSName
	if apiServerDNSName == "" {
		return HCPInfo{}, fmt.Errorf("HostedCluster %s/%s does not have API server DNS name configured", baseNamespace, clusterName)
	}

	// todo make sure the cluster is ready
	...
	return HCPInfo{
		...
		APIServerDNSName:  apiServerDNSName,
	}, nil
}
