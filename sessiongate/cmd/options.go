...
func (o *RawControllerOptions) BindFlags(cmd *cobra.Command) error {
	cmd.Flags().StringVar(&o.ServerAddress, "server-address", o.ServerAddress, "The local bind address for the HTTP server (e.g., ':8080' or 'localhost:8080')")
	...
	cmd.Flags().IntVar(&o.Workers, "workers", o.Workers, "Number of workers to run")
	cmd.Flags().IntVar(&o.LogVerbosity, "log-verbosity", o.LogVerbosity, "Verbosity level")
...
func (o *RawControllerOptions) Validate(ctx context.Context) (*ValidatedControllerOptions, error) {
	...
	if o.IngressBaseURL == "" {
		return nil, fmt.Errorf("session-ingress-hostname is required")
	}
...
func (o *ValidatedControllerOptions) Complete(ctx context.Context) (*ControllerOptions, error) {
	...
	// Build kubeconfig - try in-cluster first, then fall back to out-of-cluster
	kubeConfig, err := o.buildKubeConfig()
	if err != nil {
		return nil, fmt.Errorf("failed to build kubeconfig: %w", err)
	}

	// Create clientsets and informer factory
	...
	// Use label selector to only cache policies managed by this controller (memory efficient)
	...
	// Use label selector to only cache session-related secrets (memory efficient)
	...
	logger.Info("Successfully built kubeconfig and clientsets")

	// Create server
	srv := server.NewServer(o.ServerAddress, o.IngressBaseURL, prometheus.DefaultRegisterer)
	...
	ctrl, err := controller.NewController(
		ctx, kubeClientset, sessiongateClientset, istioClientset.SecurityV1beta1(),
...
func (o *ControllerOptions) Run(ctx context.Context) error {
	...
	// First component to fail will cancel the context for the other
	...
	// Run controller
	g.Go(func() error {
		if err := o.controller.Run(ctx, o.workers); err != nil {
	...
	// Wait for both to complete - first error cancels context for the other
