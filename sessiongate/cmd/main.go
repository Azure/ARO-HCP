...
import (
	...
	"github.com/Azure/ARO-HCP/sessiongate/internal/controller"
...
func init() {
	...
	utilruntime.Must(sessiongatev1beta1.AddToScheme(scheme))
...
func main() {
	...
	var sessionServerAddr string
	var tlsOpts []func(*tls.Config)
	flag.StringVar(&metricsAddr, "metrics-bind-address", "0", "The address the metrics endpoint binds to. "+
		"Use :8443 for HTTPS or :8080 for HTTP, or leave as 0 to disable the metrics service.")
	flag.StringVar(&probeAddr, "health-probe-bind-address", ":8081", "The address the probe endpoint binds to.")
	flag.StringVar(&sessionServerAddr, "session-server-address", "127.0.0.1:8082", "The address the session server binds to.")
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "d32ca3d4.aro-hcp.azure.com",
	...
	// Create Azure credential for kubeconfig access
	...
	// Create and start session webserver
	sessionServer := server.NewServer(sessionServerAddr)
	ctx := ctrl.SetupSignalHandler()
	if err := sessionServer.Start(ctx); err != nil {
		setupLog.Error(err, "unable to start session server")
		os.Exit(1)
	}
	setupLog.Info("session server started", "address", sessionServerAddr)

	if err := (&controller.SessionReconciler{
		Client:     mgr.GetClient(),
		Scheme:     mgr.GetScheme(),
		WebServer:  sessionServer,
