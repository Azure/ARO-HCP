...
$(APPLYCONFIGURATION_GEN): $(BINGO_DIR)/applyconfiguration-gen.mod
	...
	@cd $(BINGO_DIR) && GOWORK=off $(GO) build -mod=mod -modfile=applyconfiguration-gen.mod -o=$(GOBIN)/applyconfiguration-gen-v0.34.3 "k8s.io/code-generator/cmd/applyconfiguration-gen"

BINGO := $(GOBIN)/bingo-v0.10.0
$(BINGO): $(BINGO_DIR)/bingo.mod
	@# Install binary/ries using Go 1.14+ build command. This is using bwplotka/bingo-controlled, separate go module with pinned dependencies.
	@echo "(re)installing $(GOBIN)/bingo-v0.10.0"
	@cd $(BINGO_DIR) && GOWORK=off $(GO) build -mod=mod -modfile=bingo.mod -o=$(GOBIN)/bingo-v0.10.0 "github.com/bwplotka/bingo"

CLIENT_GEN := $(GOBIN)/client-gen-v0.34.3
$(CLIENT_GEN): $(BINGO_DIR)/client-gen.mod
	@# Install binary/ries using Go 1.14+ build command. This is using bwplotka/bingo-controlled, separate go module with pinned dependencies.
	@echo "(re)installing $(GOBIN)/client-gen-v0.34.3"
	@cd $(BINGO_DIR) && GOWORK=off $(GO) build -mod=mod -modfile=client-gen.mod -o=$(GOBIN)/client-gen-v0.34.3 "k8s.io/code-generator/cmd/client-gen"
...
$(DEEPCOPY_GEN): $(BINGO_DIR)/deepcopy-gen.mod
	...
	@cd $(BINGO_DIR) && GOWORK=off $(GO) build -mod=mod -modfile=deepcopy-gen.mod -o=$(GOBIN)/deepcopy-gen-v0.34.3 "k8s.io/code-generator/cmd/deepcopy-gen"
...
$(INFORMER_GEN): $(BINGO_DIR)/informer-gen.mod
	...
	@cd $(BINGO_DIR) && GOWORK=off $(GO) build -mod=mod -modfile=informer-gen.mod -o=$(GOBIN)/informer-gen-v0.34.3 "k8s.io/code-generator/cmd/informer-gen"

LISTER_GEN := $(GOBIN)/lister-gen-v0.34.3
$(LISTER_GEN): $(BINGO_DIR)/lister-gen.mod
	@# Install binary/ries using Go 1.14+ build command. This is using bwplotka/bingo-controlled, separate go module with pinned dependencies.
	@echo "(re)installing $(GOBIN)/lister-gen-v0.34.3"
	@cd $(BINGO_DIR) && GOWORK=off $(GO) build -mod=mod -modfile=lister-gen.mod -o=$(GOBIN)/lister-gen-v0.34.3 "k8s.io/code-generator/cmd/lister-gen"
...
$(PROMTOOL): $(BINGO_DIR)/promtool.mod
	...
	@cd $(BINGO_DIR) && GOWORK=off $(GO) build -mod=mod -modfile=promtool.mod -o=$(GOBIN)/promtool-v0.302.1 "github.com/prometheus/prometheus/cmd/promtool"
