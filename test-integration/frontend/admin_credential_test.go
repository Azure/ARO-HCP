...
func testRequestAdminCredentialStoresCSR(t *testing.T, withMock bool) {
	...
	clusterInternalID := metadataapi.Must(metadataapi.NewInternalID("/api/clusters_mgmt/v1/clusters/test-cs-id"))

	// Register the subscription.
	sub := &coreapi.Subscription{
		CosmosMetadata: coreapi.CosmosMetadata{
			ResourceID:   metadataapi.Must(azcorearm.ParseResourceID("/subscriptions/" + subscriptionID)),
	...
	clientFactory := integrationutils.Get20260901ClientFactory(testInfo.FrontendURL, subscriptionID)
	...
	var found *coreapi.Operation
	for _, op := range operationsIter.Items(ctx) {
		if op.Request == cosmosstorageutils.OperationRequestSystemAdminCredentialRequest {
			found = op
			break
		}
	}
	...
	require.NotNil(t, found.SystemAdminCredentialRequest, "SystemAdminCredentialRequest should be set on the operation")
	require.Equal(t, string(csrPEM), found.SystemAdminCredentialRequest.CertificateSigningRequest, "CertificateSigningRequest should be stored in the operation")
}
