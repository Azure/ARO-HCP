...
// serviceProviderClusterForClusterServiceCreate returns the ServiceProviderCluster
// document BuildCSCluster expects on the cluster create path. When DesiredVersion
// is unset (integration tests do not run ControlPlaneDesiredVersion), it is
// derived from the cluster customer version.
func serviceProviderClusterForClusterServiceCreate(
	ctx context.Context,
	resourcesDBClient database.ResourcesDBClient,
	cluster *api.HCPOpenShiftCluster,
) (*api.ServiceProviderCluster, error) {
	serviceProviderCluster, err := database.GetOrCreateServiceProviderCluster(ctx, resourcesDBClient, cluster.ID)
	if err != nil {
	...
	if versionID == "" {
		return nil, utils.TrackError(fmt.Errorf("cluster customer version ID is not set"))
	}
	desiredVersion, err := semver.ParseTolerant(versionID)
	if err != nil {
		return nil, utils.TrackError(fmt.Errorf("failed to parse cluster version %q: %w", versionID, err))
	}

	serviceProviderCluster = serviceProviderCluster.DeepCopy()
...
// PostCluster + ClusterServiceID persistence.
func SimulateBackendClusterServiceCreate(
	...
	serviceProviderCluster, err := serviceProviderClusterForClusterServiceCreate(ctx, resourcesDBClient, cluster)
	if err != nil {
		return utils.TrackError(err)
	}
