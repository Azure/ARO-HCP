...
func ResourceInstanceEquals(t *testing.T, expected, actual any) (string, bool) {
	...
	for _, currMap := range []map[string]any{expectedMap, actualMap} {
		...
		for _, possiblePrepend := range []string{"", "properties"} {
			...
			unstructured.RemoveNestedField(currMap, prepend(possiblePrepend, "serviceProviderProperties", "deleteOperationCompletionDeadline")...) // cluster - deadline computed from DeletionTimestamp - varies on every run on delete
			...
			// for controllers
			expectedConditions, found, err := unstructured.NestedSlice(currMap, prepend(possiblePrepend, "status", "conditions")...)
			if found && err == nil {
				for i := range expectedConditions {
					delete(expectedConditions[i].(map[string]any), "lastTransitionTime")
				}
				if err := unstructured.SetNestedSlice(currMap, expectedConditions, prepend(possiblePrepend, "status", "conditions")...); err != nil {
					panic(err)
				}
			}
