	...
	testCases := []controllertesthelpers.BasicControllerTest{
		{
			...
			ArtifactDir: resourcesapi.Must(fs.Sub(artifacts, path.Join("artifacts/externalauth"))),
			...
			ControllerVerifierFn: func(ctx context.Context, t *testing.T, controller controllerutils.Controller, testMemory map[string]any, input *controllertesthelpers.ControllerInitializationInput) {
				clusterResourceID := resourcesapi.Must(azcorearm.ParseResourceID(strings.ToLower("/subscriptions/a433a095-1277-44f1-8453-8d61a4d848c2/resourceGroups/unimportantPostponement/providers/Microsoft.RedHatOpenShift/hcpOpenShiftClusters/monstrousPrecinct/externalAuths/default")))
