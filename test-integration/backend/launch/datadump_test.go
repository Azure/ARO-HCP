...
func TestDataDumpControllerWithClusterAndCredentialRequest(t *testing.T) {
	...
	integrationutils.WithAndWithoutCosmos(t, func(t *testing.T, withMock bool) {
		...
		backendInformers := coreinformers.NewBackendInformersWithRelistDuration(ctx, resourcesDBClient.ResourcesGlobalListers(), resourcesDBClient, testInfo.BillingDBClient().BillingGlobalListers(), nil)

		_, activeOperationLister := backendInformers.ActiveOperations()
		dataDumpController := datadump.NewClusterRecursiveDataDumpController(
		...
		credResourceID := metadataapi.Must(coreapihelpers.ToSystemAdminCredentialRequestResourceID(
			clusterResourceID.SubscriptionID, clusterResourceID.ResourceGroupName, clusterResourceID.Name, credentialName))
		credRequest := &coreapi.SystemAdminCredentialRequest{
		...
		credCRUD := resourcesDBClient.HCPClusters(clusterResourceID.SubscriptionID, clusterResourceID.ResourceGroupName).SystemAdminCredentialRequests(clusterResourceID.Name)
