...
if [ "${USE_GOTESTSUM}" = "true" ]; then
    ...
    gotestsum --junitfile "${ARTIFACT_DIR}/junit-frontend-simulation-junit.xml" -- -race github.com/Azure/ARO-HCP/frontend/test/simulate/...
    sed -i 's/\(<testsuite.*\)name=""/\1 name="go-unit"/' "${ARTIFACT_DIR}/junit-frontend-simulation-junit.xml"
else
    echo "Running tests with go test (local mode)..."
    go test -race github.com/Azure/ARO-HCP/frontend/test/simulate/...
fi