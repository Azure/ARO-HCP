...
func (f *Frontend) GetHCPCluster(writer http.ResponseWriter, request *http.Request) {
	...
	if database.IsResponseError(err, http.StatusNotFound) {
		...
		arm.WriteCloudError(writer, ocm.CSErrorToCloudError(reportingErr, resourceID))
		return
	}
	if err != nil {
		logger.Error(err.Error())
	...
	if err != nil {
		logger.Error(err.Error())
		arm.WriteCloudError(writer, ocm.CSErrorToCloudError(err, hcpCluster.ResourceID))
		return
	}
