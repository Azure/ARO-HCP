...
type KeyVaultSecretReader interface {
	GetSecret(ctx context.Context, name string, version string, options *azsecrets.GetSecretOptions) (azsecrets.GetSecretResponse, error)
}

var _ KeyVaultSecretReader = (*azsecrets.Client)(nil)

type KeyVaultSecretClientFactory interface {
	KeyVaultSecretClient(managedIdentityClientID string, vaultURL string) (KeyVaultSecretReader, error)
}
