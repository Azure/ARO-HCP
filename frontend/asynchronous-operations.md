...
3. The backend pod now requests a [ClusterStatus](https://pkg.go.dev/github.com/openshift-online/ocm-sdk-go/arohcp/v1alpha1#ClusterStatus) object from Cluster Service, which is a child of the cluster's REST endpoint / "internal ID" (`/api/aro_hcp/v1alpha1/cluster/{cluster_id}/status`).
