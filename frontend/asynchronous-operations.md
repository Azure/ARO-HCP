...
The primary purpose of a hosted control plane cluster or node pool document is to serve as a cross-reference from Azure's resource identifier to OpenShift Cluster Manager (OCM)'s resource identifier.
...
When the pod that created the lock document completes its critical section, it releases the lock by deleting the lock document, again with the lock's entity tag (`_etag`) system field as the `if-match` precondition. The precondition ensures the lock document is only deleted if it is truly owned by the pod attempting the deletion.

The short time-to-live of lock documents acts as a safety fallback. In the event the pod that created the lock document crashes or becomes blocked, the lock document will quickly expire and be deleted by the Cosmos DB service, thereby freeing up the logical partition for another pod to create a lock document.
