...
`
	...
	assert.Equal(t, []string{`Deployment/my-app container "web" has resource limits set`}, violations)
}
...
`
	violations := checkPolicyViolations(manifest, emptyAllowlist)
	assert.Equal(t, []string{`Deployment/my-app container "init" has resource limits set`}, violations)
}
...
`
	violations := checkPolicyViolations(manifest, emptyAllowlist)
	assert.Equal(t, []string{`CronJob/cleanup container "cleaner" has resource limits set`}, violations)
}
...
`
	violations := checkPolicyViolations(manifest, emptyAllowlist)
	assert.Equal(t, []string{`DaemonSet/agent container "collector" has resource limits set`}, violations)
}
...
`
	violations := checkPolicyViolations(manifest, testAllowlist)
	assert.Equal(t, []string{`DaemonSet/test-allowlisted-ds container "not-allowlisted" has resource limits set`}, violations)
}
