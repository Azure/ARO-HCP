...
func runTest(testPath string, testCase testCase) (string, error) {
	settings := cli.New()
	cfg := action.Configuration{}
	cfg.Init(settings.RESTClientGetter(), testCase.Namespace, "memory")
	...
	rel, err := in.RunWithContext(context.TODO(), chart, cval)
	if err != nil {
...
func TestHelmTemplate(t *testing.T) {
	...
	for _, testCasePath := range testCases {
		...
		t.Run(testCase.Name, func(t *testing.T) {
			stdout, err := runTest(filepath.Dir(testCasePath), testCase)
			assert.NoError(t, err)
			CompareWithFixture(t, stdout, WithGoldenDir(filepath.Dir(testCasePath)))
		})
