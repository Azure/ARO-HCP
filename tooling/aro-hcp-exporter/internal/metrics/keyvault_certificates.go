...
func (c *KeyVaultCertificateCollector) cacheCertificateMetrics(certificateName string, response azcertificates.GetCertificateResponse) error {
	...
	if attributes == nil {
		return fmt.Errorf("certificate attributes are missing")
	}
	if attributes.NotBefore == nil {
		return fmt.Errorf("certificate not-before timestamp is missing")
	}
	if attributes.Expires == nil {
		return fmt.Errorf("certificate expiry timestamp is missing")
	}
	if attributes.Enabled == nil {
		return fmt.Errorf("certificate enabled attribute is missing")
	}
