...
// Exercise real SDK paging and latest GET / soft-delete HTTP paths without Azure.
func TestSDKHTTP(t *testing.T) {
	for _, dryRun := range []bool{true, false} {
		t.Run(fmt.Sprintf("dryRun=%t", dryRun), func(t *testing.T) {
			...
			want := []string{"GET /certificates", "GET /certificates"}
			if !dryRun {
				want = append(want, "GET /certificates/"+name+"/", "DELETE /certificates/"+name)
			}
