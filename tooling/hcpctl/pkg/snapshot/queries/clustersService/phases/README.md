...
- reaches `uninstalling` but never completes, review `logs/clustersService/logs.md` for repeated `Not continuing to the next destructor` messages, and check `conditions/acm/managedClusterConditions.md` for the ManagedCluster stuck in Detaching state. Review `events/acm/klusterletEvents.md` for addon pre-delete pod evictions or scheduling failures that may be blocking the destruct chain.
