...
- reaches `uninstalling` but never completes, review `logs/clustersService/logs.md` for repeated `Not continuing to the next destructor` messages, and check `events/mgmtAgent/podEvictions.md` for addon pre-delete pod evictions that may be blocking the destruct chain.
