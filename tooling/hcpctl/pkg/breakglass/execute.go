...
func Execute(ctx context.Context, params *ExecutionParams) error {
	...
	logger.Info("Starting breakglass workflow", "clusterId", params.ClusterID, "user", params.User, "privileged", params.Privileged)
	...
	if params.EnablePortForward {
		...
		// Step 4: Handle shell or wait mode
...
func setupPortForwarding(ctx context.Context, params *ExecutionParams, localPort int) (chan struct{}, *sync.Once, error) {
	...
	forwarder, err := portforward.New(params.RestConfig, params.Namespace, kasServiceName, localPort, KubeAPIServerPort)
	if err != nil {
	...
	select {
	case <-readyCh:
		logger.Info("Port forwarding established", "service", kasServiceName, "localPort", localPort, "remotePort", KubeAPIServerPort, "namespace", params.Namespace)
	case <-ctx.Done():
