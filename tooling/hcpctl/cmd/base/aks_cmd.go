...
func runBreakglass(ctx context.Context, opts *RawBreakglassAKSOptions, config ClusterConfig) error {
	...
	kubeconfigPath, err := aks.GetAKSKubeconfig(ctx, completed.SubscriptionID, completed.ResourceGroup, completed.ClusterName, completed.AzureCredential)
	if err != nil {
	...
	if completed.OutputPath != "" {
		if err := utils.CopyFile(kubeconfigPath, completed.OutputPath); err != nil {
