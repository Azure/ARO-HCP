		...
		for _, group := range irf.Rules.Spec.Groups {
			...
			if len(armGroup.Properties.Rules) > 0 {
				...
				if isRecordingRulesFile {
					...
					if err := writeAlertingGroups(armGroup, output); err != nil {
						return err
					}
				}
			}
...
func writeAlertingGroups(groups armalertsmanagement.PrometheusRuleGroupResource, into io.Writer) error {
