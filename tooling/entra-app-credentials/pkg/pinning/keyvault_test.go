...
func (f *fakeAzureCertificateClient) GetCertificatePolicy(_ context.Context, _ string, _ *azcertificates.GetCertificatePolicyOptions) (azcertificates.GetCertificatePolicyResponse, error) {
	return azcertificates.GetCertificatePolicyResponse{CertificatePolicy: f.policy}, nil
}
...
func TestCreateCertificateUsesEstablishedPolicy(t *testing.T) {
	...
	certificate, err := certificates.CreateCertificate(context.Background(), "cert", "cert.example.com", nil)
	...
	assert.Equal(t, "cert.example.com", *policy.X509CertificateProperties.SubjectAlternativeNames.DNSNames[0])
...
func TestRotateCertificateReusesExistingPolicy(t *testing.T) {
	oldInterval := certificatePollInterval
	certificatePollInterval = time.Millisecond
	t.Cleanup(func() { certificatePollInterval = oldInterval })

	existingPolicy := azcertificates.CertificatePolicy{
		IssuerParameters: &azcertificates.IssuerParameters{Name: stringPointer("ExistingIssuer")},
		Attributes:       &azcertificates.CertificateAttributes{Enabled: boolPointer(true)},
	}
	client := &fakeAzureCertificateClient{
	...
	certificate, err := NewCertificateClient(client).CreateCertificate(context.Background(), "cert", "ignored.example.com", []byte("old"))
	require.NoError(t, err)
	assert.Equal(t, []byte("new"), certificate)
	require.Len(t, client.createParameters, 1)
	assert.Same(t, existingPolicy.IssuerParameters, client.createParameters[0].CertificatePolicy.IssuerParameters)
	assert.True(t, *client.createParameters[0].CertificatePolicy.Attributes.Enabled)
	assert.Nil(t, client.createParameters[0].CertificatePolicy.ID)
}
...
func TestCreateCertificateReportsOperationFailure(t *testing.T) {
	...
	_, err := NewCertificateClient(client).CreateCertificate(context.Background(), "cert", "cert.example.com", nil)
