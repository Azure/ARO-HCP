...
# Resolve REGION_RG, SVC_WORKSPACE_NAME, HCP_WORKSPACE_NAME, GRAFANA_NAME, GLOBAL_RG
# and GLOBAL_SUBSCRIPTION from config via templatize using the same dev-settings and
# dev-environment the make targets use.
resolve_config() {
    local templatize
...
TEMPLATE
    ...
    if [[ -z "${REGION_RG:-}" || -z "${SVC_WORKSPACE_NAME:-}" || -z "${HCP_WORKSPACE_NAME:-}" \
        || -z "${GRAFANA_NAME:-}" || -z "${GLOBAL_RG:-}" || -z "${GLOBAL_SUBSCRIPTION:-}" ]]; then
