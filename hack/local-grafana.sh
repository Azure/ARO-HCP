...
# When GRAFANA_VERSION is empty, the script reads the version from the Azure Managed
# Grafana instance that config.yaml points at so that the local container matches dev.
...
# Resolve variables from config via templatize using the same dev-settings and dev-environment the make targets use.
resolve_config() {
    local templatize
    ...
    template="$(mktemp)"
    rendered="$(mktemp)"
    # Delete the tmp files (and self-clear the trap) after a RETURN or EXIT
    # shellcheck disable=SC2064
    trap "rm -f '${template}' '${rendered}'; trap - RETURN EXIT" RETURN EXIT
...
GLOBAL_RG={{ .global.rg }}
GLOBAL_SUBSCRIPTION={{ .global.subscription.key }}
SVC_SUBSCRIPTION={{ .svc.subscription.key }}
TEMPLATE
    ...
    # Read the rendered output of the env vars as data since running `source "${rendered}"` could execute commands.
    local key value
    while IFS='=' read -r key value; do
        case "${key}" in
            REGION_RG|SVC_WORKSPACE_NAME|HCP_WORKSPACE_NAME|GRAFANA_NAME|GLOBAL_RG|GLOBAL_SUBSCRIPTION|SVC_SUBSCRIPTION)
    ...
    if [[ -z "${REGION_RG:-}" || -z "${SVC_WORKSPACE_NAME:-}" || -z "${HCP_WORKSPACE_NAME:-}" \
        || -z "${GRAFANA_NAME:-}" || -z "${GLOBAL_RG:-}" || -z "${GLOBAL_SUBSCRIPTION:-}" \
        || -z "${SVC_SUBSCRIPTION:-}" ]]; then
    ...
    echo "  GLOBAL_RG=${GLOBAL_RG}"
    echo "  GLOBAL_SUBSCRIPTION=${GLOBAL_SUBSCRIPTION}"
    echo "  SVC_SUBSCRIPTION=${SVC_SUBSCRIPTION}"
}

# Set GRAFANA_VERSION to the version that the Azure Managed Grafana (AMG) instance runs.
# The local container then renders the dashboards in observability/grafana-dashboards the same way AMG does.
resolve_grafana_version() {
    if [[ -n "${GRAFANA_VERSION}" ]]; then
        echo "GRAFANA_VERSION is set. Using Grafana ${GRAFANA_VERSION}."
        return 0
    fi

    local version
    version="$(az resource show \
        --name "${GRAFANA_NAME}" \
        --resource-group "${GLOBAL_RG}" \
        --subscription "${GLOBAL_SUBSCRIPTION}" \
        --resource-type Microsoft.Dashboard/grafana \
        --api-version 2024-10-01 \
        --query "properties.grafanaVersion" -o tsv || true)"

    if [[ ! "${version}" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        err "Could not read the version of AMG instance '${GRAFANA_NAME}' in resource group '${GLOBAL_RG}'."
        err "Run 'az login' and make sure you have read access to the instance."
        err "To skip this lookup, set GRAFANA_VERSION to a Grafana image tag."
        exit 1
    fi

    GRAFANA_VERSION="${version}"
    echo "local Grafana version set to ${GRAFANA_VERSION} to match the Dev Azure Managed Grafana instance '${GRAFANA_NAME}'."
}
...
get_prometheus_endpoint() {
    ...
    az monitor account show \
        --name "${name}" \
        --resource-group "${rg}" \
        --subscription "${SVC_SUBSCRIPTION}" \
        --query "metrics.prometheusQueryEndpoint" -o tsv || true
}
...
check_amw_access() {
    ...
    # Pass the bearer token through a curl config file instead of a command line header since header arguments are visible to
    # every local process through /proc, and a config file is not.
    local curl_config
    curl_config="$(mktemp)" # the 'umask 077' above makes this readable only by the current user
    # Delete the tmp file (and self-clear the trap) after a RETURN or EXIT
    # shellcheck disable=SC2064
    trap "rm -f '${curl_config}'; trap - RETURN EXIT" RETURN EXIT
...
append_azure_monitor_datasource() {
    ...
    if [[ -z "${tenant_id}" ]]; then
        tenant_id="$(az account show --subscription "${SVC_SUBSCRIPTION}" --query tenantId -o tsv || true)"
    fi
    if [[ -z "${subscription_id}" ]]; then
        subscription_id="$(az account show --subscription "${SVC_SUBSCRIPTION}" --query id -o tsv || true)"
    fi
    if [[ -z "${tenant_id}" || -z "${subscription_id}" ]]; then
        err "Azure Monitor datasource requested but the tenant/subscription of '${SVC_SUBSCRIPTION}' could not be resolved."
        err "Set AZURE_TENANT_ID and AZURE_SUBSCRIPTION_ID manually, or run 'az login' with an account that can read that subscription."
        return 1
    fi
...
start_grafana() {
    ...
    echo "=== Resolving the Grafana version ==="
    resolve_grafana_version
...
Prerequisites:
  - podman or docker
  - az CLI, logged in with an account that can read the "ARO Hosted Control Planes
    (EA Subscription 1)" subscription (it does not need to be your active one):
        az login
...
Environment variables:
  ...
  GRAFANA_VERSION            Grafana image tag (default: the Azure Managed Grafana instance version in config.yaml)
  ...
  AZURE_TENANT_ID            Tenant for the Azure Monitor datasource (default: tenant of the personal-dev svc subscription in config.yaml)
  AZURE_SUBSCRIPTION_ID      Subscription for the Azure Monitor datasource (default: the personal-dev svc subscription in config.yaml)
==============================================================================
EOF
}
