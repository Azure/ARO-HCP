...
type BackendOptions struct {
	...
	// ClusterOIDCIssuerBaseURL is the OIDC issuer base URL that clusters use to specify their OIDC Issuer URL. The
	// OIDC issuer URL configured for them use this base url as a base prefix of its full URL. The full OIDC issuer URL is then used as the issuer
	// in all K8s ServiceAccount tokens generated by the control plane's K8s API server via --service-account-isuer flag.
	// The full OIDC issuer URL is also used as the Issuer when configuring OIDC Federation on the cluster's data plane operator identities.
	ClusterOIDCIssuerBaseURL string
}
...
func (b *Backend) runBackendControllersUnderLeaderElection(ctx context.Context, electionChecker *leaderelection.HealthzAdaptor) error {
	...
	dataPlaneWorkloadsOIDCFederationController := clusterdataplaneworkloads.NewDataPlaneOIDCFederationController(
		...
		b.options.FPAMIDataplaneClientBuilder,
		b.options.ClusterScopedIdentitiesConfig,
		b.options.ClusterOIDCIssuerBaseURL,
	)
