...
func (c *operationClusterCreate) createBillingDocument(ctx context.Context, billingDocID, resourceGroupName string, clusterCreationTime time.Time, op *api.Operation) error {
	logger := utils.LoggerFromContext(ctx)
	...
	doc := database.NewBillingDocument(billingDocID, op.ExternalID)
