...
func TestDispatchClusterCreate_SynchronizeOperation(t *testing.T) {
	...
	tests := []struct {
		...
		desiredVersion *semver.Version // nil: rely on GetOrCreate (empty DesiredVersion)
		setupMockCS    func(ctrl *gomock.Controller, fixture *clusterTestFixture) ocm.ClusterServiceClientSpec
		expectError    bool
	...
	}{
		{
			...
			setupMockCS: func(ctrl *gomock.Controller, _ *clusterTestFixture) ocm.ClusterServiceClientSpec {
		...
		},
		{
			...
			expectError: false,
			verify: func(t *testing.T, ctx context.Context, db *databasetesting.MockResourcesDBClient, _ *clusterTestFixture) {
				op, err := db.Operations(testSubscriptionID).Get(ctx, testOperationName)
				require.NoError(t, err)
				assert.Equal(t, testClusterServiceIDStr, op.InternalID.String())
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockResourcesDBClient, err := databasetesting.NewMockResourcesDBClientWithResources(ctx, []any{subscription, cluster, operation})
			require.NoError(t, err)

			if tt.desiredVersion != nil {
				addTestServiceProviderCluster(t, ctx, mockResourcesDBClient, fixture, tt.desiredVersion)
			}

			mockCS := tt.setupMockCS(ctrl, fixture)
...
func addTestServiceProviderCluster(
	t *testing.T,
	ctx context.Context,
	db *databasetesting.MockResourcesDBClient,
	fixture *clusterTestFixture,
