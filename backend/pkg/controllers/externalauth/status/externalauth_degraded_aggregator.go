...
func (c *externalAuthDegradedAggregator) SyncOnce(ctx context.Context, key controllerutils.HCPExternalAuthKey) error {
	...
	// observed is true when the external auth has controllers, so an
	// all-healthy external auth reports Degraded=False/AsExpected instead of
	// Unknown/NoData (the latter is reserved for one with no controllers at all).
