...
type managementClusterSyncController struct {
	...
	fleetClient             database.FleetDBClient
...
func (c *managementClusterSyncController) syncProvisionShard(ctx context.Context, csShard *arohcpv1alpha1.ProvisionShard) error {
	...
	rid := convertedManagementCluster.ResourceID
	crud := c.fleetClient.ManagementClusters(rid.SubscriptionID, rid.ResourceGroupName)

	existing, err := c.managementClusterLister.Get(ctx, rid.SubscriptionID, rid.ResourceGroupName, rid.Name)
	if err != nil && !database.IsNotFoundError(err) {
		return fmt.Errorf("management cluster %s: %w", rid, err)
	}
	...
	if database.IsNotFoundError(err) {
		...
		created, err := crud.Create(ctx, convertedManagementCluster, nil)
		if err != nil {
	...
	if _, err = crud.Replace(ctx, managementClusterToWrite, nil); err != nil {
