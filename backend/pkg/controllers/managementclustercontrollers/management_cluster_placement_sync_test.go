...
func testMgmtClusterResourceID() *azcorearm.ResourceID {
	return api.Must(fleet.ToManagementClusterResourceID(testClusterSubscriptionID, testClusterResourceGroup, testMgmtClusterName))
}
		...
		},
		{
			...
			managementClusters: []*fleet.ManagementCluster{
				...
				func() *fleet.ManagementCluster {
					...
					mc.ResourceID = api.Must(fleet.ToManagementClusterResourceID(testClusterSubscriptionID, testClusterResourceGroup, "mgmt-cluster-2"))
					return mc
				}(),
			},
			expectCSCall:                true,
