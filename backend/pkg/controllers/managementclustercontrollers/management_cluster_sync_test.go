...
func getManagementCluster(t *testing.T, ctx context.Context, fleetClient database.FleetDBClient, name string) *fleet.ManagementCluster {
	t.Helper()
	mc, err := fleetClient.ManagementClusters(testSubscriptionID, testResourceGroup).Get(ctx, name)
...
// mockFleetDBClientWithCRUD wraps a gomock ManagementClusterCRUD into a database.FleetDBClient
// for tests that need to assert specific CRUD method expectations.
type mockFleetDBClientWithCRUD struct {
	database.FleetDBClient
	crud database.ManagementClusterCRUD
}

func (m *mockFleetDBClientWithCRUD) ManagementClusters(_, _ string) database.ManagementClusterCRUD {
	return m.crud
}
...
func (e *errorManagementClusterLister) Get(_ context.Context, _, _, _ string) (*fleet.ManagementCluster, error) {
	return nil, e.err
}
...
func TestSyncOnce(t *testing.T) {
	tests := []struct {
		name                string
		setup               func(t *testing.T, ctrl *gomock.Controller) (ocm.ClusterServiceClientSpec, database.FleetDBClient, dblisters.ManagementClusterLister)
		expectedErrorSubstr string
		validate            func(t *testing.T, ctx context.Context, fleetClient database.FleetDBClient)
	}{
		{
		...
		},
		{
			...
			validate: func(t *testing.T, ctx context.Context, fleetClient database.FleetDBClient) {
				t.Helper()
				doc := getManagementCluster(t, ctx, fleetClient, "test-westus3-mgmt-1")
		...
		},
		{
			...
			setup: func(t *testing.T, ctrl *gomock.Controller) (ocm.ClusterServiceClientSpec, database.FleetDBClient, dblisters.ManagementClusterLister) {
				...
				require.NoError(t, err)
				existingResourceID := api.Must(fleet.ToManagementClusterResourceID(testSubscriptionID, testResourceGroup, "test-westus3-mgmt-1"))
				...
				_, err = fleetClient.ManagementClusters(testSubscriptionID, testResourceGroup).Create(t.Context(), existing, nil)
				require.NoError(t, err)
		...
		},
		{
			...
			setup: func(t *testing.T, ctrl *gomock.Controller) (ocm.ClusterServiceClientSpec, database.FleetDBClient, dblisters.ManagementClusterLister) {
				...
				return mockCS, &mockFleetDBClientWithCRUD{crud: mockCRUD}, &listertesting.SliceManagementClusterLister{ManagementClusters: []*fleet.ManagementCluster{existing}}
			},
		},
		{
		...
		},
		{
			...
			validate: func(t *testing.T, ctx context.Context, fleetClient database.FleetDBClient) {
				t.Helper()
				doc1 := getManagementCluster(t, ctx, fleetClient, "test-westus3-mgmt-1")
				...
				doc2 := getManagementCluster(t, ctx, fleetClient, "test-eastus-mgmt-1")
		...
		},
		{
			...
			setup: func(t *testing.T, ctrl *gomock.Controller) (ocm.ClusterServiceClientSpec, database.FleetDBClient, dblisters.ManagementClusterLister) {
				...
				return mockCS, &mockFleetDBClientWithCRUD{crud: mockCRUD}, &listertesting.SliceManagementClusterLister{}
			},
