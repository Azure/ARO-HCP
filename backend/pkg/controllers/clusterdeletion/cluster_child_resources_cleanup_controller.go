...
func (c *clusterChildResourcesCleanupController) ensureClusterScopedKubeApplierResourcesDeleted(ctx context.Context, clusterResourceID *azcorearm.ResourceID) error {
	...
	// Backup schedule desires are managed by ensureBackupScheduleKubeObjectsDeleted
	// and must not be swept here while that cleanup is in progress.  On demand backup
	// desires can be deleted without cleanup on the management cluster, velero will
	// delete the backups when TTL expires.
...
func (c *clusterChildResourcesCleanupController) ensureBackupScheduleKubeObjectsDeleted(ctx context.Context, clusterResourceID *azcorearm.ResourceID) error {
	...
	for _, ad := range adIterator.Items(ctx) {
		if !strings.HasPrefix(ad.ResourceID.Name, backup.BackupScheduleDesireNamePrefix) {
			continue
		}
