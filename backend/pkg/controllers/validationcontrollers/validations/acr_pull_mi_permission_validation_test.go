...
func testCluster(containerRegistryMIResourceID, capzResourceID string) *api.HCPOpenShiftCluster {
	...
	if containerRegistryMIResourceID != "" {
		cluster.CustomerProperties.Platform.ContainerRegistry = &api.ContainerRegistryProfile{
			Credentials: api.ContainerRegistryCredentialProfile{
				...
				ManagedIdentity: &api.UserAssignedManagedIdentity{
					ResourceID: mustParseResourceID(containerRegistryMIResourceID),
				},
			},
		}
	}
...
func TestAcrPullManagedIdentityPermissionValidation(t *testing.T) {
	...
	acrPullMIResourceID := "/subscriptions/00000000-0000-0000-0000-000000000002/resourceGroups/customer-rg/providers/Microsoft.ManagedIdentity/userAssignedIdentities/acr-pull"
	...
	tests := []struct {
		...
		},
		{
			...
			cluster: testCluster(acrPullMIResourceID, capzResourceID),
			smiBuilder: &fakeSMIClientBuilder{
				uaisClient: uaisClient,
			},
			caBuilder: &fakeCheckAccessV2ClientBuilder{
		...
		},
		{
			...
			smiBuilder: &fakeSMIClientBuilder{
				uaisClient: &fakeUserAssignedIdentitiesClient{
					getFunc: func(context.Context, string, string, *armmsi.UserAssignedIdentitiesClientGetOptions) (armmsi.UserAssignedIdentitiesClientGetResponse, error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			v := NewAcrPullManagedIdentityPermissionValidation(tt.smiBuilder, tt.caBuilder)
