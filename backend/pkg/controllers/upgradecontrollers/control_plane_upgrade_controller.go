...
func (c *controlPlaneUpgradeSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error { // Get the customer's desired cluster configuration
	...
	oldActiveVersions := existingServiceProviderCluster.Status.Version.ActiveVersions
	existingServiceProviderCluster.Status.Version.ActiveVersions = c.prependActiveVersionIfChanged(
		oldActiveVersions,
		actualVersion,
	)
	...
	if !slices.Equal(oldActiveVersions, existingServiceProviderCluster.Status.Version.ActiveVersions) {
		logger := utils.LoggerFromContext(ctx)
		logger.Info("Active versions changed",
			"oldActiveVersions", oldActiveVersions,
			"newActiveVersions", existingServiceProviderCluster.Status.Version.ActiveVersions,
		)
	...
	activeVersions := existingServiceProviderCluster.Status.Version.ActiveVersions
	...
	previousDesiredVersion := existingServiceProviderCluster.Spec.Version.DesiredVersion
	...
	existingServiceProviderCluster.Spec.Version.DesiredVersion = desiredVersion
...
func (c *controlPlaneUpgradeSyncer) prependActiveVersionIfChanged(currentVersions []api.HCPClusterActiveVersion, newVersion string) []api.HCPClusterActiveVersion {
	...
	newVer := semver.MustParse(newVersion)

	// Create new list with at most 2 versions: new version + most recent old version
	newVersions := []api.HCPClusterActiveVersion{{Version: &newVer}}
...
func (c *controlPlaneUpgradeSyncer) desiredControlPlaneZVersion(ctx context.Context, cincinnatiClient cincinatti.Client, customerDesiredMinor string, channelGroup string, activeVersions []api.HCPClusterActiveVersion) (*semver.Version, error) {
	...
	if len(activeVersions) == 0 {
		...
		initialDesiredVersion, err := c.findLatestVersionInMinor(ctx, cincinnatiClient, channelGroup, customerDesiredMinor, []semver.Version{seedVersion})
		if err != nil {
			return nil, err
		}
	...
	// Extract semver versions from active versions
	...
	actualLatestVersion := activeVersionList[0]
	...
	if actualMinor == customerDesiredMinor { // we need to do a z-stream upgrade (actual minor == desired minor)
		...
		return c.findLatestVersionInMinor(ctx, cincinnatiClient, channelGroup, customerDesiredMinor, activeVersionList)
	}
	...
	if !isValidNextYStreamUpgradePath(actualMinor, customerDesiredMinor) {
		return nil, fmt.Errorf("invalid next y-stream upgrade path from %s to %s: only upgrades to the next minor version are allowed, no downgrades or skipping minor versions", actualMinor, customerDesiredMinor)
	}
	...
	latestVersion, err := c.findLatestVersionInMinor(ctx, cincinnatiClient, channelGroup, customerDesiredMinor, activeVersionList)
	if err != nil {
		return nil, err
	}
	...
	if latestVersion == nil {
		...
		return c.findLatestVersionInMinor(ctx, cincinnatiClient, channelGroup, actualMinor, activeVersionList)
	}
...
func (c *controlPlaneUpgradeSyncer) findLatestVersionInMinor(ctx context.Context, cincinnatiClient cincinatti.Client, channelGroup string, targetMinor string, activeVersions []semver.Version) (*semver.Version, error) {
	...
	// Parse target minor version for filtering
	targetMinorVersion, err := semver.ParseTolerant(targetMinor)
	if err != nil {
		return nil, fmt.Errorf("invalid target minor %s: %w", targetMinor, err)
	}
	...
	for _, activeVersion := range activeVersions {
		...
		for _, candidate := range candidateReleases {
			ver, err := semver.Parse(candidate.Version)
			...
			entry := candidatesByVersion[versionStr]
			entry.version = ver
			entry.count++
			candidatesByVersion[versionStr] = entry
		}
	}
	...
	for _, entry := range candidatesByVersion {
		if entry.count == len(activeVersions) {
			commonCandidates = append(commonCandidates, entry.version)
		}
	}

	// Use the most recent active version for additional validation logic
	return c.selectBestVersionFromCandidates(ctx, cincinnatiClient, channelGroup, targetMinorVersion, commonCandidates, activeVersions[0])
}
...
//
// Returns nil if no suitable version is found.
func (c *controlPlaneUpgradeSyncer) selectBestVersionFromCandidates(ctx context.Context, cincinnatiClient cincinatti.Client, channelGroup string, targetMinorVersion semver.Version, candidates []semver.Version, actualLatestVersion semver.Version) (*semver.Version, error) {
	if len(candidates) == 0 {
		return nil, nil
	}
	...
	slices.SortFunc(candidates, func(a, b semver.Version) int {
		return b.Compare(a) // descending: latest first
	})

	// Check if next minor channel exists before checking for gateways
	actualMinor := fmt.Sprintf("%d.%d", actualLatestVersion.Major, actualLatestVersion.Minor)
	targetMinor := fmt.Sprintf("%d.%d", targetMinorVersion.Major, targetMinorVersion.Minor)

	versionToCheckNextMinor := actualLatestVersion
	if targetMinor != actualMinor {
		// Y-stream upgrade: use latest candidate version
		versionToCheckNextMinor = candidates[0]
	}

	nextMinor := fmt.Sprintf("%d.%d", targetMinorVersion.Major, targetMinorVersion.Minor+1)
	// Here we are sure that the version to check next minor existence is in the graph,
	...
	_, _, _, nextMinorExistsErr := cincinnatiClient.GetUpdates(
		ctx,
		cincinnatiURI,
		"multi",
		"multi",
		fmt.Sprintf("%s-%s", channelGroup, nextMinor),
		versionToCheckNextMinor,
	)
