...
func TestTriggerControlPlaneUpgradeSyncer_CreateUpgradePolicyIfNeeded(t *testing.T) {
	testClusterServiceID, _ := resourcesapi.NewInternalID("/api/aro_hcp/v1alpha1/clusters/test-cluster-id")
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(mc *ocm.MockClusterServiceClientSpec) {
				latestPolicy := resourcesapi.Must(arohcpv1alpha1.NewControlPlaneUpgradePolicy().Version("4.19.20").Build())
				olderPolicy := resourcesapi.Must(arohcpv1alpha1.NewControlPlaneUpgradePolicy().Version("4.19.15").Build())
		...
		},
		{
			...
			clusterServiceID: testClusterServiceID,
			mockSetup: func(mc *ocm.MockClusterServiceClientSpec) {
				latestPolicy := resourcesapi.Must(arohcpv1alpha1.NewControlPlaneUpgradePolicy().Version("4.19.18").Build())
				olderPolicy := resourcesapi.Must(arohcpv1alpha1.NewControlPlaneUpgradePolicy().Version("4.19.15").Build())
