...
func (c *clusterResourcesController) fetchAndProcessClusterResources(ctx context.Context,
	...
	if err != nil {
		// Temporary workaround: skip timestamp parsing errors from OCM SDK
		if strings.Contains(err.Error(), "parsing time") && strings.Contains(err.Error(), "creation_timestamp") {
			logger.Info("Skipping cluster resources sync due to OCM SDK timestamp parsing issue", "error", err.Error())
			return nil
		}
		return err
	}
...
func (c *clusterResourcesController) createApplyDesireFromResource(
	...
	existing, err := getExistingApplyDesire(ctx, kubeApplierCRUD, resourceKey)
	if err != nil {
		return err
	}
...
func getExistingApplyDesire(
	ctx context.Context, crud database.ResourceCRUD[kubeapplier.ApplyDesire, *kubeapplier.ApplyDesire], resourceName string,
...
func applyDesireNeedsWork(existing, desired *kubeapplier.ApplyDesire) bool {
	if existing == nil {
		return true
	}
