...
func TestDesiredDataPlaneOIDCFederationStatus(t *testing.T) {
	...
	keyA, ok := (&dataPlaneOIDCFederationIntentSyncer{}).dataplaneOIDCFederationKeyFromARMUserAssignedIdentities(resolvedA)
	require.True(t, ok)

	resolvedB := resolvedARMManagedIdentityMetadata(identityB, "client-b", "principal-b", "tenant-b")
	keyB, ok := (&dataPlaneOIDCFederationIntentSyncer{}).dataplaneOIDCFederationKeyFromARMUserAssignedIdentities(resolvedB)
	require.True(t, ok)
...
func TestDesiredDataPlaneOIDCFederationStatusNilEntryErrors(t *testing.T) {
	...
	t.Run("nil federation status", func(t *testing.T) {
		...
		}).desiredDataPlaneOIDCFederationStatus(nil, nil, map[coreapi.ManagedIdentityDataplaneOIDCFederationKey]*coreapi.ManagedIdentityDataplaneOIDCFederationStatus{
			keyA: nil,
		})
		require.Error(t, err)
...
func TestDataplaneOIDCFederationKeyFromARMUserAssignedIdentities(t *testing.T) {
	t.Parallel()
	...
	t.Run("resolved ARM user assigned identities metadata", func(t *testing.T) {
		...
		key, ok := syncer.dataplaneOIDCFederationKeyFromARMUserAssignedIdentities(metadata)
		require.True(t, ok)
		assert.Equal(t, strings.ToLower(identity.String()), key.ResourceID)
		assert.Equal(t, "arm-client", key.ClientID)
		assert.Equal(t, "arm-principal", key.PrincipalID)
		assert.Equal(t, "arm-tenant", key.TenantID)
	})
	...
	t.Run("missing ARM user assigned identities metadata", func(t *testing.T) {
		...
		_, ok := syncer.dataplaneOIDCFederationKeyFromARMUserAssignedIdentities(metadata)
		assert.False(t, ok)
	})
