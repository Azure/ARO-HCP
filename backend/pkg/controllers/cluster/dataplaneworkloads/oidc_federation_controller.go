...
func (s *dataPlaneOIDCFederationSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error {
	...
	// Configured only when every Azure call for that identity succeeds.
	// Successful deconfigure removes the identity from the map. PendingConfigure
	// and Configured Get each desired FIC and
	...
	for federationKey := range replacement.Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation {
		...
		case coreapi.ManagedIdentityDataplaneOIDCFederationPhasePendingDeconfigure:
			...
			delete(replacement.Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation, federationKey)
		}
	}
	if len(replacement.Status.ManagedIdentitiesWithDataPlaneWorkloadsOIDCFederation) == 0 {
...
func (s *dataPlaneOIDCFederationSyncer) ensureFederation(
	...
	// ResourceID stays Configured or becomes PendingConfigure; it does not
	// deconfigure. This loop runs while the identity stays Configured because
...
func (s *dataPlaneOIDCFederationSyncer) parseManagedIdentityResourceID(federationKey string) (*azcorearm.ResourceID, error) {
	identityResourceID, err := azcorearm.ParseResourceID(federationKey)
	if err != nil {
		return nil, utils.TrackError(fmt.Errorf("failed to parse managed identity ResourceID %s: %w", federationKey, err))
	}
	return identityResourceID, nil
}

// clusterTenantID returns the tenant ID of the cluster's subscription. The OIDC
// issuer URL is <oidcIssuerBaseURL><clusterTenantID>/<csClusterID>, matching
// Clusters Service. Identity TenantID from ObservedIdentity is not used.
func (s *dataPlaneOIDCFederationSyncer) clusterTenantID(ctx context.Context, subscriptionID string) (string, error) {
	...
	if subscription.Properties == nil || len(ptr.Deref(subscription.Properties.TenantId, "")) == 0 {
		return "", utils.TrackError(fmt.Errorf("subscription %s has no tenantId", subscriptionID))
	}
