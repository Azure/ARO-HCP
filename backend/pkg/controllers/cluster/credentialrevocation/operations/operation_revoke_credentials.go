...
func (c *operationRevokeCredentialsPoll) ShouldProcess(ctx context.Context, operation *api.Operation) bool {
	...
	if operation.Request != api.OperationRequestSystemAdminCredentialRevocation {
		return false
	}
...
func (c *operationRevokeCredentialsPoll) SynchronizeOperation(ctx context.Context, key controllerutils.OperationKey) error {
	...
	if err := operationbase.UpdateOperationStatus(ctx, c.clock, c.resourcesDBClient, operation, arm.ProvisioningStateSucceeded, nil, notifyFn); err != nil {
		return utils.TrackError(err)
	}
