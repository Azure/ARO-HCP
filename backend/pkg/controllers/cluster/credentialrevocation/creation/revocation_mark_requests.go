...
func (c *revocationMarkRequests) SyncOnce(ctx context.Context, key controllerutils.SystemAdminCredentialRevocationKey) error {
	...
	if revocation.Status.DeletionTimestamp != nil || meta.IsStatusConditionTrue(revocation.Status.Conditions, api.SystemAdminCredentialRevocationConditionCredentialsMarkedForDeletion) {
		return nil
	}
