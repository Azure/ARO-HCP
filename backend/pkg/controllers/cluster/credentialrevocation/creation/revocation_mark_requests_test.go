	...
	tests := []struct {
		...
		},
		{
			...
			setupDB: func(db *corecosmosstoragetesting.MockResourcesDBClient) {
				...
				createTestRevocation(t, db, testRevocationName, func(r *api.SystemAdminCredentialRevocation) {
		...
		},
		{
			...
			verify: func(t *testing.T, ctx context.Context, db *corecosmosstoragetesting.MockResourcesDBClient) {
				...
				require.NoError(t, err)
				assert.True(t, meta.IsStatusConditionTrue(revocation.Status.Conditions, api.SystemAdminCredentialRevocationConditionCredentialsMarkedForDeletion),
