...
func TestClusterDeletionCleanup_SyncOnce(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			credName: testCredentialName,
			setupDB: func(db *databasetesting.MockResourcesDBClient) {
				createTestCredentialRequest(t, db, testCredentialName, func(cred *api.SystemAdminCredentialRequest) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			syncer := &credentialRequestDeletion{
				...
				serviceProviderClusterLister: &listertesting.SliceServiceProviderClusterLister{ServiceProviderClusters: tc.serviceProviderClusters},
			}
