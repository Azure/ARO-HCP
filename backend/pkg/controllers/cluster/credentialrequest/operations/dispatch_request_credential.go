...
func (c *dispatchRequestCredential) ShouldProcess(ctx context.Context, operation *api.Operation) bool {
	...
	if operation.Request != api.OperationRequestSystemAdminCredentialRequest {
		return false
	}
	...
	for _, cred := range iter.Items(ctx) {
		if cred.Spec.OperationID == operationIDStr {
			...
			if replacement.SystemAdminCredentialRequest == nil {
				replacement.SystemAdminCredentialRequest = &api.OperationSystemAdminCredentialRequest{}
			}
