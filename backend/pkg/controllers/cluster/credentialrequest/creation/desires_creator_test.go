...
import (
	...
	dblisertesting "github.com/Azure/ARO-HCP/internal/database/listertesting"
...
var testManagementClusterResourceID = api.Must(azcorearm.ParseResourceID(testManagementClusterResourceIDStr))
...
func newTestCluster() *api.HCPOpenShiftCluster {
	...
	csID := api.Must(api.NewInternalID("/api/clusters_mgmt/v1/clusters/abc123"))
...
func newTestServiceProviderCluster() *api.ServiceProviderCluster {
...
func TestDesiresCreator_SyncOnce(t *testing.T) {
	...
	tests := []struct {
		...
		expectError               bool
		verify                    func(t *testing.T, ctx context.Context, db *databasetesting.MockResourcesDBClient, kubeApplierClients *databasetesting.MockKubeApplierDBClients)
	}{
		{
		...
		},
		{
			...
			spcLister: &listertesting.SliceServiceProviderClusterLister{
				ServiceProviderClusters: []*api.ServiceProviderCluster{
					func() *api.ServiceProviderCluster {
		...
		},
		{
			...
			spcLister: &listertesting.SliceServiceProviderClusterLister{
				ServiceProviderClusters: []*api.ServiceProviderCluster{newTestServiceProviderCluster()},
			},
			expectError: false,
		},
		{
			...
			verify: func(t *testing.T, ctx context.Context, db *databasetesting.MockResourcesDBClient, kubeApplierClients *databasetesting.MockKubeApplierDBClients) {
				...
				require.NoError(t, err)
				var applyDesires []*kubeapplier.ApplyDesire
				...
				for _, ad := range applyDesires {
					assert.Equal(t, kubeapplier.ApplyDesireTypeServerSideApply, ad.Spec.Type, "ApplyDesire type")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.registerKubeApplierClient {
				mockKubeApplierClient := databasetesting.NewMockKubeApplierDBClient()
			...
			syncer := &desiresCreator{
				...
				applyDesireLister:            &dblisertesting.SliceApplyDesireLister{},
				readDesireLister:             &dblisertesting.SliceReadDesireLister{},
			}
