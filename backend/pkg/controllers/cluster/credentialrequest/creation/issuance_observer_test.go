	...
	tests := []struct {
	...
	}{
		{
			...
			verify: func(t *testing.T, ctx context.Context, db *databasetesting.MockResourcesDBClient) {
				...
				assert.True(t, meta.IsStatusConditionTrue(cred.Status.Conditions, coreapi.SystemAdminCredentialRequestConditionIssued), "credential should be in Issued state")
		...
		},
		{
			...
			verify: func(t *testing.T, ctx context.Context, db *databasetesting.MockResourcesDBClient) {
				...
				cred, err := credCRUD.Get(ctx, testCredentialName)
				require.NoError(t, err)
				assert.True(t, meta.IsStatusConditionTrue(cred.Status.Conditions, coreapi.SystemAdminCredentialRequestConditionFailed), "credential should be in Failed state")
			},
		},
		{
			...
			verify: func(t *testing.T, ctx context.Context, db *databasetesting.MockResourcesDBClient) {
				...
				cred, err := credCRUD.Get(ctx, testCredentialName)
				require.NoError(t, err)
				assert.True(t, credentialrequest.IsCredentialRequestPending(cred), "credential should remain in Pending state")
			},
		},
		{
			...
			verify: func(t *testing.T, ctx context.Context, db *databasetesting.MockResourcesDBClient) {
				...
				cred, err := credCRUD.Get(ctx, testCredentialName)
				require.NoError(t, err)
				assert.True(t, meta.IsStatusConditionTrue(cred.Status.Conditions, coreapi.SystemAdminCredentialRequestConditionIssued), "credential should remain in Issued state")
			},
		},
		{
