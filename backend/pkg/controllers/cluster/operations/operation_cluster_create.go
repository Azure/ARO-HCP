...
func (c *operationClusterCreate) SynchronizeOperation(ctx context.Context, key controllerutils.OperationKey) error {
	...
	newOperationStatus, opError, err := sharedops.ConvertClusterStatus(ctx, c.clusterServiceClient, operation, clusterStatus)
	if err != nil {
		return utils.TrackError(err)
	}
	...
	err = sharedops.UpdateOperationStatus(ctx, c.clock, c.resourcesDBClient, operation, newOperationStatus, opError, sharedops.PostAsyncNotificationFn(c.notificationClient))
	if err != nil {
		return utils.TrackError(err)
	}
...
func (c *operationClusterCreate) determineOperationStatus(ctx context.Context, operation *api.Operation) (*sharedops.OperationState, error) {
	logger := utils.LoggerFromContext(ctx)

	errs := []error{}
	operationStates := []*sharedops.OperationState{}
	...
	slices.SortStableFunc(operationStates, sharedops.CompareOperationState)
	...
	picked, err := sharedops.PickWorstOperationState(operationStates)
	if err != nil {
...
func (c *operationClusterCreate) clusterOperationStatus(ctx context.Context, operation *api.Operation) (*sharedops.OperationState, error) {
	...
	if database.IsNotFoundError(err) {
		...
		return sharedops.NewOperationState(arm.ProvisioningStateProvisioning, "cluster state not cached yet"), nil
	}
	if err != nil {
		return nil, utils.TrackError(err)
	}
	...
	if len(cluster.ServiceProviderProperties.API.URL) == 0 {
		...
		return sharedops.NewOperationState(arm.ProvisioningStateProvisioning, message), nil
	}

	return sharedops.NewOperationState(arm.ProvisioningStateSucceeded, ""), nil
}
...
func (c *operationClusterCreate) hostedClusterOperationStatus(ctx context.Context, operation *api.Operation) (*sharedops.OperationState, error) {
	logger := utils.LoggerFromContext(ctx)
	...
	if database.IsNotFoundError(err) {
		return sharedops.NewOperationState(arm.ProvisioningStateProvisioning, "hosted cluster state not cached yet"), nil
	}
	if err != nil {
		return nil, utils.TrackError(err)
	}
	...
	if readDesire.Status.KubeContent == nil || len(readDesire.Status.KubeContent.Raw) == 0 {
		return sharedops.NewOperationState(arm.ProvisioningStateProvisioning, "ReadDesire has no kube content"), nil
	}
	...
	if anyVersionWithValidSuccessCondition {
		...
		if !meta.IsStatusConditionTrue(hostedCluster.Status.Conditions, string(v1beta1.HostedClusterAvailable)) {
			...
			return sharedops.NewOperationState(arm.ProvisioningStateProvisioning, withDegradedSuffix(message, hostedCluster)), nil
		}
		...
		if !anyVersionInstalled {
			...
			return sharedops.NewOperationState(arm.ProvisioningStateProvisioning, withDegradedSuffix("hosted cluster has no installed version", hostedCluster)), nil
		}
	}

	if len(hostedCluster.Status.ControlPlaneEndpoint.Host) == 0 {
		return sharedops.NewOperationState(arm.ProvisioningStateProvisioning, withDegradedSuffix("hosted cluster has no control plane endpoint host", hostedCluster)), nil
	}
	if hostedCluster.Status.ControlPlaneEndpoint.Port == 0 {
		return sharedops.NewOperationState(arm.ProvisioningStateProvisioning, withDegradedSuffix("hosted cluster has no control plane endpoint port", hostedCluster)), nil
	}
