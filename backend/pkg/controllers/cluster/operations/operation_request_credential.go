...
func (opsync *operationRequestCredential) SynchronizeOperation(ctx context.Context, key controllerutils.OperationKey) error {
	...
	if !sharedops.NeedToPatchOperation(oldOperation, newOperationStatus, newOperationError) {
		return nil
	}

	err = sharedops.PatchOperation(ctx, opsync.clock, opsync.resourcesDBClient, oldOperation, newOperationStatus, newOperationError, sharedops.PostAsyncNotificationFn(opsync.notificationClient))
	if err != nil {
		return utils.TrackError(err)
	}
