...
func newTestCluster(opts ...func(*api.HCPOpenShiftCluster)) *api.HCPOpenShiftCluster {
	...
	csID := api.Must(api.NewInternalID("/api/aro_hcp/v1alpha1/clusters/abc123"))
	...
	allIdentities := make(map[string]*arm.UserAssignedIdentity)
	for _, id := range cpOps {
		allIdentities[strings.ToLower(id.String())] = &arm.UserAssignedIdentity{PrincipalID: ptr.To("principal-" + id.Name)}
	}
	for _, id := range dpOps {
		allIdentities[strings.ToLower(id.String())] = &arm.UserAssignedIdentity{PrincipalID: ptr.To("principal-" + id.Name)}
	}
	allIdentities[strings.ToLower(serviceManagedID.String())] = &arm.UserAssignedIdentity{PrincipalID: ptr.To("principal-service-managed")}
...
func matchingGetResponseForAllTypes(cluster *api.HCPOpenShiftCluster) func(ctx context.Context, scope string, denyAssignmentID string, opts *armauthorization.DenyAssignmentsClientGetOptions) (armauthorization.DenyAssignmentsClientGetResponse, error) {
...
func TestSyncOnce(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var clusters []*api.HCPOpenShiftCluster
			if tt.cluster != nil {
				clusters = []*api.HCPOpenShiftCluster{tt.cluster}
			}
			syncer := &clusterDenyAssignmentSyncer{
				clock:         clocktesting.NewFakeClock(time.Now()),
				clusterLister: &listertesting.SliceClusterLister{Clusters: clusters},
			}
...
func TestSyncDenyAssignmentNeedsWork(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			spc: newTestSPC(func(spc *api.ServiceProviderCluster) {
		...
		},
		{
			...
			cluster: newTestCluster(),
			spc: newTestSPC(func(spc *api.ServiceProviderCluster) {
				spc.Status.AzureResources.DenyAssignments.PendingAzureResources = []api.DenyAssignmentReference{
	...
	tests := []struct {
				...
				spc.Status.AzureResources.DenyAssignments.AzureResources = []api.DenyAssignmentReference{
					{
						...
						DenyAssignmentResourceID: api.Must(azcorearm.ParseResourceID("/subscriptions/" + testSubscriptionID + "/resourceGroups/" + testManagedRG + "/providers/Microsoft.Authorization/denyAssignments/stale-uuid")),
					},
				}
			}),
			mockDenyAssignments: &azuremockclient.DenyAssignmentsClientFunc{
		...
		},
		{
			...
			existingSPC: func() *api.ServiceProviderCluster {
				cluster := newTestCluster()
				refs, _ := allDenyAssignmentReferences(cluster)
				return newTestSPC(func(spc *api.ServiceProviderCluster) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			syncer := &clusterDenyAssignmentSyncer{
				...
				clusterLister:         &listertesting.SliceClusterLister{Clusters: []*api.HCPOpenShiftCluster{tt.cluster}},
				subscriptionLister:    &listertesting.SliceSubscriptionLister{Subscriptions: []*arm.Subscription{testSubscription()}},
...
func TestEnsureDenyAssignmentReferences(t *testing.T) {
	...
	var resourcesRef api.DenyAssignmentReference
	...
	tests := []struct {
		name                 string
		refs                 []api.DenyAssignmentReference
	...
	}{
		{
			...
			refs:       []api.DenyAssignmentReference{resourcesRef},
		...
		{
			name: "unknown type goes to failed",
			refs: []api.DenyAssignmentReference{
				{
					DenyAssignmentType:       "unknown-type",
					DenyAssignmentResourceID: api.Must(azcorearm.ParseResourceID("/subscriptions/" + testSubscriptionID + "/resourceGroups/" + testManagedRG + "/providers/Microsoft.Authorization/denyAssignments/test-uuid")),
				},
			},
			defsByType:           map[string]denyAssignmentDefinition{},
			mockDenyAssignments:  &azuremockclient.DenyAssignmentsClientFunc{},
...
func TestDeleteDenyAssignment(t *testing.T) {
	rid := api.Must(azcorearm.ParseResourceID("/subscriptions/" + testSubscriptionID + "/resourceGroups/" + testManagedRG + "/providers/Microsoft.Authorization/denyAssignments/test-uuid"))
...
func TestReplaceServiceProviderClusterIfChanged(t *testing.T) {
	tests := []struct {
		name              string
		modifyReplacement func(spc *api.ServiceProviderCluster)
		priorErrs         []error
		expectError       bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			original, err := spcCRUD.Get(ctx, api.ServiceProviderClusterResourceName)
			require.NoError(t, err)
...
func TestAppendDenyAssignmentReference(t *testing.T) {
	ref1 := api.DenyAssignmentReference{DenyAssignmentType: "type-a"}
	ref2 := api.DenyAssignmentReference{DenyAssignmentType: "type-b"}
	ref3 := api.DenyAssignmentReference{DenyAssignmentType: "type-a"}
	...
	result = appendDenyAssignmentReference([]api.DenyAssignmentReference{ref1}, ref2, ref3)
	assert.Len(t, result, 2)
}
