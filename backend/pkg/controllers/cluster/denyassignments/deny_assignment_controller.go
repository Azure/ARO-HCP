...
func (c *clusterDenyAssignmentSyncer) syncDenyAssignmentNeedsWork(cluster *api.HCPOpenShiftCluster, serviceProviderCluster *api.ServiceProviderCluster) bool {
...
func (c *clusterDenyAssignmentSyncer) syncDenyAssignmentUpsert(ctx context.Context, key controllerutils.HCPClusterKey, cluster *api.HCPOpenShiftCluster) error {
	...
	requiredDenyAssignmentReferenceByType := make(map[string]api.DenyAssignmentReference, len(requiredDenyAssignmentReferences))
...
func replaceServiceProviderClusterIfChanged(
	ctx context.Context,
	serviceProviderClusterCRUD database.ResourceCRUD[api.ServiceProviderCluster, *api.ServiceProviderCluster],
	serviceProviderCluster *api.ServiceProviderCluster,
	replacement *api.ServiceProviderCluster,
	priorErrs []error,
) (*api.ServiceProviderCluster, *api.ServiceProviderCluster, error) {
...
func (c *clusterDenyAssignmentSyncer) ensureDenyAssignmentReferences(
...
) (succeeded, failed []api.DenyAssignmentReference, err error) {
	logger := utils.LoggerFromContext(ctx)
	var errs []error
	...
	for _, ref := range refs {
		...
		if !ok {
			logger.Error(nil, "Skipping deny assignment type with no definition", "denyAssignmentType", ref.DenyAssignmentType)
...
func appendDenyAssignmentReference(slice []api.DenyAssignmentReference, refs ...api.DenyAssignmentReference) []api.DenyAssignmentReference {
