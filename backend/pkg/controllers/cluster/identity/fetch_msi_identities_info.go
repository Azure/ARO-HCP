...
// queried for MSI identity metadata. Spec.EarliestRecheckTimesByController[FetchMSIIdentitiesInfoControllerName]
// is honored only when the identities stored on the ServiceProviderCluster still match
...
func (c *fetchMSIIdentitiesInfoSyncer) needsWork(existingServiceProviderCluster *coreapi.ServiceProviderCluster, desiredIdentitiesToFetch *msiBasedIdentitiesToFetch) bool {
	// Only honor Spec.EarliestRecheckTimesByController[FetchMSIIdentitiesInfoControllerName] when the
	// desired identity set still matches the ServiceProviderCluster. Any mismatch (or future "must work now"
	...
	if c.desiredMSIResourceIDsMatchServiceProviderCluster(desiredIdentitiesToFetch, existingServiceProviderCluster) {
		...
		// Spec.EarliestRecheckTimesByController[FetchMSIIdentitiesInfoControllerName] so we do not repeatedly
		// query the Managed Identities Data Plane for the same identities. Nil means recheck immediately; a
