...
func TestDesiredMSIResourceIDsMatchServiceProviderCluster(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			serviceProviderCluster: matchingServiceProviderCluster,
			want:                   true,
		},
		{
		...
		},
		{
			...
			serviceProviderCluster: func() *coreapi.ServiceProviderCluster {
				...
				serviceProviderCluster.Status.MSIManagedIdentities.ControlPlaneOperatorsIdentities[otherLower] = &coreapi.ServiceProviderClusterControlPlaneOperatorIdentity{
		...
		},
		{
			...
			serviceProviderCluster: func() *coreapi.ServiceProviderCluster {
				_, serviceProviderCluster := newMatchingClusterAndServiceProviderCluster()
				serviceProviderCluster.Status.MSIManagedIdentities.ServiceManagedIdentity = nil
				return serviceProviderCluster
			}(),
			want: false,
		},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, tc.want, syncer.desiredMSIResourceIDsMatchServiceProviderCluster(tc.toFetch, tc.serviceProviderCluster))
		})
	}
}
...
func TestNeedsWorkIgnoresEarliestRecheckWhenIdentitiesDiverge(t *testing.T) {
	...
	divergedCluster, _ := newMatchingClusterAndServiceProviderCluster()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, tc.want, syncer.needsWork(tc.serviceProviderCluster, tc.toFetch))
		})
	}
}
...
func newMatchingClusterAndServiceProviderCluster() (*coreapi.HCPOpenShiftCluster, *coreapi.ServiceProviderCluster) {
	...
	return cluster, serviceProviderCluster
}
