...
const (
	...
	// AFECOwnershipTargetAFECs requires subscriptions to have a specific AFEC flag (INT/STAGING environment).
	AFECOwnershipTargetAFECs AFECOwnershipMode = "TargetAFECs"
	// AFECOwnershipExcludedAFECs requires subscriptions to NOT have any of the specified AFEC flags (PROD environment).
	AFECOwnershipExcludedAFECs AFECOwnershipMode = "ExcludedAFECs"
)
...
type AFECOwnershipConfig struct {
	...
	// TargetAFECs is the AFEC flag that identifies subscriptions owned by this environment.
	// Used when Mode is AFECOwnershipTargetAFECs.
	TargetAFECs string
	// ExcludedAFECs is a set of AFEC flags that identify subscriptions owned by OTHER environments.
	// Used when Mode is AFECOwnershipExcludedAFECs.
...
// - targetAFECs: AFEC flag identifying subscriptions owned by this environment (e.g., "Microsoft.RedHatOpenShift/STAGING-APPROVED")
// - excludedAFECs: Comma-separated list of AFEC flags identifying subscriptions owned by OTHER environments
// - runningMode: "delete" to actually delete orphaned MRGs, "dry-run" for read-only mode.
//
// PROD: Set excludedAFECs to skip INT/STAGING subscriptions
// STAGING/INT: Set targetAFECs to only clean subscriptions with that AFEC
...
func NewOrphanedManagedResourceGroupController(
	...
	afecOwnership := parseAFECOwnership(targetAFECs, excludedAFECs)

	return &orphanedManagedResourceGroupController{
		location:              location,
		readOnly:              runningMode != "delete",
...
// The caller must ensure that targetAFECs and excludedAFECsStr are not both set (mutually exclusive).
func parseAFECOwnership(targetAFECs, excludedAFECsStr string) AFECOwnershipConfig {
	// Parse excludedAFECs
	excludedAFECs := make(map[string]struct{})
	if len(excludedAFECsStr) != 0 {
		for _, afec := range strings.Split(excludedAFECsStr, ",") {
			trimmed := strings.TrimSpace(afec)
			if len(trimmed) != 0 {
				excludedAFECs[trimmed] = struct{}{}
			}
		}
	}

	// Determine mode
	if len(targetAFECs) == 0 && len(excludedAFECs) == 0 {
		return AFECOwnershipConfig{Mode: AFECOwnershipNone}
	}
	if len(targetAFECs) != 0 {
		return AFECOwnershipConfig{
			Mode:        AFECOwnershipTargetAFECs,
			TargetAFECs: targetAFECs,
		}
	}
	return AFECOwnershipConfig{
		Mode:          AFECOwnershipExcludedAFECs,
		ExcludedAFECs: excludedAFECs,
	}
}
...
// - Mode TargetAFECs: subscription MUST have the specified AFEC flag (INT/STAGING)
// - Mode ExcludedAFECs: subscription MUST NOT have any of the specified AFEC flags (PROD)
//
// Fail-closed: if the subscription's AFEC data is missing (nil Properties or RegisteredFeatures),
// the subscription is treated as not owned to prevent accidental deletions.
func (c *orphanedManagedResourceGroupController) isSubscriptionOwnedByThisEnvironment(subscription *coreapi.Subscription) bool {
	if subscription.Properties == nil || subscription.Properties.RegisteredFeatures == nil {
		return false
	}
	...
	case AFECOwnershipNone:
		return false
	case AFECOwnershipTargetAFECs:
		return subscription.HasRegisteredFeature(c.afecOwnership.TargetAFECs)
	case AFECOwnershipExcludedAFECs:
		for excludedAFEC := range c.afecOwnership.ExcludedAFECs {
			if subscription.HasRegisteredFeature(excludedAFEC) {
...
func (c *orphanedManagedResourceGroupController) needsWork(ctx context.Context, key controllerutils.ManagedResourceGroupKey) (bool, *coreapi.Subscription, error) {
	...
	if !c.isSubscriptionOwnedByThisEnvironment(subscription) {
		logger.V(1).Info("Subscription not owned by this environment, skipping MRG",
			...
			"targetAFECs", c.afecOwnership.TargetAFECs)
		return false, nil, nil
	}
...
func (c *orphanedManagedResourceGroupController) SyncOnce(ctx context.Context, key controllerutils.ManagedResourceGroupKey) error {
	...
	orphanedMRGsFound.WithLabelValues(c.location, strings.ToLower(key.SubscriptionID), strings.ToLower(key.ResourceGroupName)).Inc()
...
func (c *orphanedManagedResourceGroupController) deleteOrphanedManagedResourceGroup(
	...
	if err != nil {
		...
		orphanedMRGsDeletionFailed.WithLabelValues(c.location, strings.ToLower(key.SubscriptionID), strings.ToLower(key.ResourceGroupName)).Inc()
		return err
	}
