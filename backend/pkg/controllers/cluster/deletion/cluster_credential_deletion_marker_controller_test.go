...
func newTestCredentialRequest(t *testing.T, credName string, opts ...func(*api.SystemAdminCredentialRequest)) *api.SystemAdminCredentialRequest {
	t.Helper()
...
func newTestCredentialRevocation(t *testing.T, revocationName string, opts ...func(*api.SystemAdminCredentialRevocation)) *api.SystemAdminCredentialRevocation {
	t.Helper()
	resourceID := api.Must(azcorearm.ParseResourceID(
...
func TestClusterCredentialDeletionMarkerController_SyncOnce(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			existingCluster: readyForDeletionCluster(t),
			extraResources: []any{
				newTestCredentialRequest(t, "cred-1", func(cred *api.SystemAdminCredentialRequest) {
		...
		},
		{
			...
			existingCluster: readyForDeletionCluster(t),
			extraResources: []any{
				newTestCredentialRevocation(t, "revoke-1", func(rev *api.SystemAdminCredentialRevocation) {
					earlier := metav1.NewTime(fixedClockTime.Add(-2 * time.Hour))
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			credsForLister := []*api.SystemAdminCredentialRequest{}
			revocationsForLister := []*api.SystemAdminCredentialRevocation{}
