...
func (c *deleteOrphanedBackupManifestWorks) getAllServiceProviderClusters(ctx context.Context) ([]*api.ServiceProviderCluster, error) {
	...
	for {
		iterator, err := c.cosmosClient.ResourcesGlobalListers().ServiceProviderClusters().List(ctx, listOptions)
		if err != nil {
...
func (c *deleteOrphanedBackupManifestWorks) buildProvisionShardToServiceProviderClustersIndex(
	...
	mcByStampID := map[string]*fleet.ManagementCluster{}
	for {
		mcIterator, err := c.fleetDBClient.GlobalListers().ManagementClusters().List(ctx, listOptions)
		if err != nil {
			return index, utils.TrackError(fmt.Errorf("failed to list management clusters: %w", err))
		}
		for _, mc := range mcIterator.Items(ctx) {
			if mc.Status.ClusterServiceProvisionShardID == nil || mc.ResourceID == nil {
				continue
			}
			shardID := mc.Status.ClusterServiceProvisionShardID.ID()
			maestroClientCtx, cancel := context.WithCancel(ctx)
			maestroClient, err := createMaestroClientFromManagementCluster(
				maestroClientCtx, mc, c.maestroSourceEnvironmentIdentifier, c.maestroClientBuilder,
			)
			if err != nil {
				cancel()
				return index, utils.TrackError(fmt.Errorf("failed to create Maestro client: %w", err))
			}
			index[shardID] = &backupShardServiceProviderClusters{
				maestroClient:           maestroClient,
				maestroClientCancelFunc: cancel,
				serviceProviderClusters: []*api.ServiceProviderCluster{},
			}
			stampID := mc.ResourceID.Parent.Name
			mcByStampID[stampID] = mc
		}
		if err := mcIterator.GetError(); err != nil {
			return index, utils.TrackError(fmt.Errorf("failed iterating management clusters: %w", err))
		}
		continuationToken := mcIterator.GetContinuationToken()
		if continuationToken == "" {
			break
		}
		listOptions.ContinuationToken = &continuationToken
	...
	for _, spc := range allServiceProviderClusters {
		...
		mc, ok := mcByStampID[stampID]
		if !ok || mc.Status.ClusterServiceProvisionShardID == nil {
			continue
		}
		shardID := mc.Status.ClusterServiceProvisionShardID.ID()
		entry, ok := index[shardID]
		if !ok {
			continue
		}
...
func (c *deleteOrphanedBackupManifestWorks) ensureOrphanedBackupManifestWorksAreDeleted(
	...
	for csShardID, shardToSPCs := range index {
		...
		logger.Info("processing cluster service provision shard", "csProvisionShardID", csShardID, "spcCount", len(shardToSPCs.serviceProviderClusters))
