	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(t *testing.T, adCrud database.ResourceCRUD[kubeapplier.ApplyDesire, *kubeapplier.ApplyDesire], rdCrud database.ResourceCRUD[kubeapplier.ReadDesire, *kubeapplier.ReadDesire]) {
				...
				rd := makeOnDemandRD(t, mcResourceID, adName, &runtime.RawExtension{Raw: []byte(`{}`)})
				rd.Status.Conditions = []metav1.Condition{
					{Type: kubeapplier.ConditionTypeSuccessful, Status: metav1.ConditionTrue},
				}
				_, _ = rdCrud.Create(context.Background(), rd, nil)
			},
			checkADName: adName,
			wantADGone:  true,
			checkRDName: adName,
		...
		{
			name: "pass1: does not delete on-demand AD until RD has observed KubeContent",
			setup: func(t *testing.T, adCrud database.ResourceCRUD[kubeapplier.ApplyDesire, *kubeapplier.ApplyDesire], rdCrud database.ResourceCRUD[kubeapplier.ReadDesire, *kubeapplier.ReadDesire]) {
				ad := makeOnDemandAD(t, mcResourceID, adName)
				ad.Status.Conditions = []metav1.Condition{{Type: kubeapplier.ConditionTypeSuccessful, Status: metav1.ConditionTrue}}
				_, _ = adCrud.Create(context.Background(), ad, nil)
				rd := makeOnDemandRD(t, mcResourceID, adName, nil)
				rd.Status.Conditions = []metav1.Condition{{Type: kubeapplier.ConditionTypeSuccessful, Status: metav1.ConditionTrue}}
				_, _ = rdCrud.Create(context.Background(), rd, nil)
			},
			checkADName: adName,
			wantADGone:  false,
		...
		{
			name: "DB error on AD Get returns error",
			setup: func(t *testing.T, adCrud database.ResourceCRUD[kubeapplier.ApplyDesire, *kubeapplier.ApplyDesire], rdCrud database.ResourceCRUD[kubeapplier.ReadDesire, *kubeapplier.ReadDesire]) {
				_, _ = rdCrud.Create(context.Background(), makeOnDemandRD(t, mcResourceID, adName, nil), nil)
			},
