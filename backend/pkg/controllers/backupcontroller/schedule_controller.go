...
func (c *backupScheduleSyncer) ensureDesireUpdated(
	...
	for _, desired := range desiredADs {
		...
		if existing.Spec.ServerSideApply != nil &&
			!internalcontrollerutils.NeedsUpdate(existing.Spec.ServerSideApply.KubeContent, desired.Spec.ServerSideApply.KubeContent) {
			continue
		}
