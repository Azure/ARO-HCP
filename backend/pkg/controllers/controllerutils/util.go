...
// HCPSystemAdminRevocationKey is for driving workqueues keyed for system admin revocations.
type HCPSystemAdminRevocationKey struct {
	SubscriptionID               string `json:"subscriptionID"`
	ResourceGroupName            string `json:"resourceGroupName"`
	HCPClusterName               string `json:"hcpClusterName"`
	HCPSystemAdminRevocationName string `json:"hcpSystemAdminRevocationName"`
}

func (k *HCPSystemAdminRevocationKey) GetResourceID() *azcorearm.ResourceID {
	return api.Must(api.ToSystemAdminRevocationResourceID(k.SubscriptionID, k.ResourceGroupName, k.HCPClusterName, k.HCPSystemAdminRevocationName))
}

func (k *HCPSystemAdminRevocationKey) AddLoggerValues(logger logr.Logger) logr.Logger {
	return logger.WithValues(utils.LogValues{}.AddLogValuesForResourceID(k.GetResourceID())...)
}

func (k *HCPSystemAdminRevocationKey) InitialController(controllerName string) *api.Controller {
	resourceID := api.Must(azcorearm.ParseResourceID(k.GetResourceID().String() + "/" + api.ControllerResourceTypeName + "/" + controllerName))
	return &api.Controller{
		CosmosMetadata: api.CosmosMetadata{
			ResourceID: resourceID,
