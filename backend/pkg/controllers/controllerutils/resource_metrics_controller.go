...
// the resource ID string. The input is lowercased to ensure consistent hashes
// regardless of input casing. This anonymizes the resource identity in metric
// labels while remaining deterministic for correlation.
func ResourceIDHash(resourceID string) string {
	h := sha256.Sum256([]byte(strings.ToLower(resourceID)))
