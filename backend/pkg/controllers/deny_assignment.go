...
func (s *denyAssignmentSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error {
	...
	if err != nil {
		return fmt.Errorf("failed to reconcile deny assignments: %w", err)
	}
...
func (c *denyAssignmentReconciler) ReconcileDenyAssignments(ctx context.Context, cluster *api.HCPOpenShiftCluster) (bool, error) {
	...
	if cluster.CustomerProperties.Platform.ManagedResourceGroup == "" {
		return false, fmt.Errorf("managed resource group is not set")
	}
	...
	if err != nil {
		return false, fmt.Errorf("failed to fetch principal IDs: %w", err)
	}

	// Create resources client
	resourcesClient, err := armresources.NewClient(subscriptionID, c.credential, nil)
	if err != nil {
		return false, fmt.Errorf("failed to create resources client: %w", err)
	}
	...
	for _, da := range denyAssignmentReconcilers {
		...
		if err != nil {
			errs = append(errs, fmt.Errorf("failed to create %s: %w", da.suffix, err))
		}
...
func (c *denyAssignmentReconciler) fetchPrincipalIDs(ctx context.Context, cluster *api.HCPOpenShiftCluster, subscriptionID string) (map[string]string, error) {
	...
	if operatorsAuth.UserAssignedIdentities.ControlPlaneOperators == nil {
		return nil, fmt.Errorf("control plane operators not configured")
	}
...
func (c *denyAssignmentReconciler) fetchPrincipalID(ctx context.Context, client *armmsi.UserAssignedIdentitiesClient, resourceID *azcorearm.ResourceID) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to get managed identity %s: %w", resourceID.String(), err)
	}

	if resp.Properties == nil || resp.Properties.PrincipalID == nil {
		return "", fmt.Errorf("principal ID not found for managed identity %s", resourceID.String())
	}
...
func (c *denyAssignmentReconciler) reconcileDenyAssignment(
	...
	if errors.As(err, &respErr) && respErr.StatusCode != http.StatusNotFound {
		return false, fmt.Errorf("failed to check deny assignment: %w", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("failed to collect principal IDs: %w", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("failed to create deny assignment: %w", err)
	}

	// Poll for completion
	pollerRes, err := poller.Poll(ctx)
	if err != nil {
		return false, fmt.Errorf("failed to poll deny assignment creation: %w", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("deny assignment creation failed: %w", err)
	}
...
func (c *denyAssignmentReconciler) collectPrincipalIDs(
	...
	for _, opName := range controlPlaneOperators {
		...
		if !ok {
			return nil, fmt.Errorf("principal ID not found for control plane operator: %s", opName)
		}
	...
	for _, opName := range dataPlaneOperators {
		...
		if !ok {
			return nil, fmt.Errorf("principal ID not found for data plane operator: %s", opName)
		}
		principalIDs = append(principalIDs, principalID)
	}
	...
	if addServiceMI {
		...
		if !ok {
			return nil, fmt.Errorf("principal ID not found for service managed identity")
		}
		principalIDs = append(principalIDs, principalID)
	}
