...
func (c *createNodePoolScopedReadDesiresSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPNodePoolKey) error {
	...
	desired := clustermaestro.BuildReadDesire(
	...
	existing, err := clustermaestro.GetExistingReadDesire(ctx, crud, readDesireNameReadonlyNodePool)
	if err != nil {
		return err
	}
	if !clustermaestro.ReadDesireNeedsWork(existing, desired) {
		return nil
	}
	if existing == nil {
...
func nodePoolTarget(envIdentifier, csClusterID, csClusterDomainPrefix, csNodePoolID string) kubeapplier.ResourceReference {
	return kubeapplier.ResourceReference{
		...
		Resource:  "nodepools",
		Namespace: sharedmaestro.HostedClusterNamespace(envIdentifier, csClusterID),
