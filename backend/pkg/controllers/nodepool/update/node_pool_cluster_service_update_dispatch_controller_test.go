...
func TestNodePoolUpdateDispatchSyncer_SyncOnce(t *testing.T) {
	...
	newNodePoolWithConfigDiff := func() *coreapi.NodePool {
		return newTestNodePool(func(np *coreapi.NodePool) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			existingNodePool: newTestNodePool(func(np *coreapi.NodePool) {
		...
		},
		{
			...
			existingCSNodePool: mustBuildCSNodePoolFromRP(t, newTestNodePool(func(np *coreapi.NodePool) {
				np.Properties.Replicas = 2
			})),
			setupMockCSClient: func(mock *ocm.MockClusterServiceClientSpec) {
				mock.EXPECT().
					GetNodePool(gomock.Any(), csID).
					Return(mustBuildCSNodePoolFromRP(t, newTestNodePool(func(np *coreapi.NodePool) {
						np.Properties.Replicas = 2
					})), nil)
			},
			minimumReconcileTimeCooldownChecker: &alwaysSyncCooldownChecker{},
...
func mustBuildCSNodePoolFromRP(t *testing.T, hcpNodePool *coreapi.NodePool) *arohcpv1alpha1.NodePool {
	t.Helper()
...
func newTestNodePool(opts ...func(*coreapi.NodePool)) *coreapi.NodePool {
