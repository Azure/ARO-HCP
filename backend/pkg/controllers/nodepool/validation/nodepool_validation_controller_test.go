...
func TestNodePoolValidationSyncer_ConsecutiveUnknownSuppression(t *testing.T) {
	...
	for i := 1; i <= maxConsecutiveUnknownsBeforeWrite; i++ {
		// Advance the clock between attempts so each SyncOnce is independent of the previous
		// attempt's EarliestRetryAfter enqueue-time cooldown.
...
func findStoredNodePoolCondition(t *testing.T, ctx context.Context, mockDB *corecosmosstoragetesting.MockResourcesDBClient) *metav1.Condition {
	t.Helper()
	spnp, err := mockDB.ServiceProviderNodePools(
		testSubscriptionID, testResourceGroup, testClusterName, testNodePoolName,
	).Get(ctx, coreapi.ServiceProviderNodePoolResourceName)
	require.NoError(t, err)
	return meta.FindStatusCondition(spnp.Status.Validations, testValidationName)
}

// TestNodePoolValidationSyncer_CooldownDoesNotShortCircuitSyncOnce verifies that a
...
func TestNodePoolValidationSyncer_CooldownDoesNotShortCircuitSyncOnce(t *testing.T) {
	ctx := utils.ContextWithLogger(context.Background(), testr.New(t))
	...
	cond := findStoredNodePoolCondition(t, ctx, mockDB)
	require.NotNil(t, cond)
...
func TestNodePoolValidationSyncer_CooldownChecker(t *testing.T) {
	ctx := context.Background()
	fakeClock := clocktesting.NewFakePassiveClock(fixedNow)
	syncer, _ := newTestSyncer(corecosmosstoragetesting.NewMockResourcesDBClient(), NewMockNodePoolValidation(testValidationName), fakeClock)
