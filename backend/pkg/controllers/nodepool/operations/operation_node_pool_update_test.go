...
func TestOperationNodePoolUpdate_SynchronizeOperation(t *testing.T) {
	...
	newNodePoolWithVersion := func(version string, mutate ...func(*coreapi.ClusterNodePool)) *coreapi.ClusterNodePool {
	...
	newPassingCachedNodePoolReadDesire := func(nodePool *coreapi.ClusterNodePool) *kubeapplierapi.ReadDesire {
		return newNodePoolReadDesire(t, nodePool, fixture.NewCluster())
	}

	setupMockCSClientForNodePoolState := func(state operationbase.NodePoolStateValue, statusMessage ...string) func(*ocm.MockClusterServiceClientSpec) {
	...
	setupMockCSClientForNodePoolReadyWithSpec := func(replicas int, nodeDrainMinutes int32) func(*ocm.MockClusterServiceClientSpec) {
		return func(mock *ocm.MockClusterServiceClientSpec) {
			stateBuilder := arohcpv1alpha1.NewNodePoolState().NodePoolStateValue(string(operationbase.NodePoolStateReady))
			statusBuilder := arohcpv1alpha1.NewNodePoolStatus().State(stateBuilder)
			csNodePool, err := arohcpv1alpha1.NewNodePool().
				Replicas(replicas).
				NodeDrainGracePeriod(arohcpv1alpha1.NewValue().
	...
	testCases := []struct {
		...
		},
		{
			...
			existingNodePool: newNodePoolWithVersion("4.19.0", func(nodePool *coreapi.ClusterNodePool) {
...
func newNodePoolReadDesire(t *testing.T, nodePool *coreapi.ClusterNodePool, cluster *coreapi.Cluster) *kubeapplierapi.ReadDesire {
	t.Helper()
...
func nodePoolToHypershiftNodePool(nodePool *coreapi.ClusterNodePool, cluster *coreapi.Cluster) *v1beta1.NodePool {
