...
func (c *operationNodePoolUpdate) SynchronizeOperation(ctx context.Context, key controllerutils.OperationKey) error {
	...
	if err := sharedops.UpdateOperationStatus(ctx, c.clock, c.resourcesDBClient, operation, operationalState.ProvisioningState, persistErr, sharedops.PostAsyncNotificationFn(c.notificationClient)); err != nil {
		return utils.TrackError(err)
	}
	return nil
}

func (c *operationNodePoolUpdate) determineOperationState(ctx context.Context, operation *api.Operation) (*sharedops.OperationState, error) {
	logger := utils.LoggerFromContext(ctx)
	errs := []error{}
...
func (c *operationNodePoolUpdate) desiredVersionResolutionOperationState(ctx context.Context, operation *api.Operation) (*sharedops.OperationState, error) {
	...
	controllerDoc, getControllerErr := controllerCRUD.Get(ctx, version.NodepoolVersionControllerName)
	...
	if intentFailedCondition == nil {
		return sharedops.NewOperationState(arm.ProvisioningStateAccepted, "customer desired version not yet calculated"), nil
	}
	...
	if intentFailedCondition.Status != metav1.ConditionTrue || intentFailedCondition.Reason != api.VersionUpgradeNotAcceptedReason {
		pending := sharedops.NewOperationState(arm.ProvisioningStateAccepted, "customer desired version does not match resolved desired version")
		...
		c.desiredVersionMismatchFirstSeen.Remove(operationID)
		return sharedops.NewOperationState(arm.ProvisioningStateFailed, msg), nil
	}
	c.desiredVersionMismatchFirstSeen.Remove(operationID)
	return sharedops.NewOperationState(arm.ProvisioningStateFailed, intentFailedCondition.Message), nil
}

func (c *operationNodePoolUpdate) nodePoolServiceUpdateOperationState(ctx context.Context, operation *api.Operation) (*sharedops.OperationState, error) {
	logger := utils.LoggerFromContext(ctx)
	nodePoolStatus, err := c.clusterServiceClient.GetNodePoolStatus(ctx, operation.InternalID)
	if err != nil {
		return nil, utils.TrackError(err)
	}
	newOperationStatus, opError, err := sharedops.ConvertNodePoolStatus(operation, nodePoolStatus)
	if err != nil {
		return nil, utils.TrackError(err)
	}
	...
	return sharedops.NewOperationState(newOperationStatus, msg), nil
}
