...
// newTestNodePoolForAggregator builds a minimal NodePool
// suitable for the aggregator tests.
func newTestNodePoolForAggregator(opts ...func(*coreapi.NodePool)) *coreapi.NodePool {
...
func TestNodePoolDegradedAggregator_SyncOnce(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "healthy node-pool desire omitted; condition-less node-pool desire synthesized alongside a degraded controller",
			controllers: []*coreapi.Controller{
			...
			inertia:      thirtySecondInertia,
			expectStatus: metav1.ConditionTrue,
			// The degraded controller (elder, past inertia) drives the flip; the bad
			// path enumerates all bad sources, so the synthesized condition-less desire
			// is reported too. The healthy np-ok desire is omitted. Sorted by source
			// name: "AController" < "applydesire/...".
			expectReason:  "AController_Failed::" + npNoneName + "_MissingDegradedCondition",
			expectMessage: "AController: boom\n" + npNoneName + ": has not reported a Degraded condition",
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			existing := newTestNodePoolForAggregator(func(np *coreapi.NodePool) {
