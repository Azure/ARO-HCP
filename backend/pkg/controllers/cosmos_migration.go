...
func migrateDesireDocuments[T any](ctx context.Context, logger logr.Logger, crud database.ResourceCRUD[T], resourceDesc string, getName func(*T) string) error {
	...
	for _, item := range iter.Items(ctx) {
		...
		migrated++
		if err := replaceWithRetry(ctx, logger, crud, name, resourceDesc); err != nil {
			migrationErrors = append(migrationErrors, err)
		}
	}
...
func migrateControllers(ctx context.Context, logger logr.Logger, controllerCRUD database.ResourceCRUD[api.Controller], resourceDesc string) error {
...
func migrateManagementClusterContents(ctx context.Context, logger logr.Logger, managementClusterContentCRUD database.ManagementClusterContentCRUD, resourceDesc string) error {
...
func replaceWithRetry[T any](ctx context.Context, logger logr.Logger, crud database.ResourceCRUD[T], name string, resourceDesc string) error {
	const maxRetries = 3
	var lastErr error
