...
func (c *nodePoolClusterServiceCreateSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPNodePoolKey) error {
	...
	if existing == nil {
		...
		if c.isOCMErrorBadRequest(err) {
			logger.Error(err, "CS node pool POST returned OCM error with HTTP 4XX status code", "cs_node_pool_href", csNodePoolHREF, "node_pool_resource_id", nodePool.ID.String())
		}
		if err != nil {
			return utils.TrackError(err)
		}
