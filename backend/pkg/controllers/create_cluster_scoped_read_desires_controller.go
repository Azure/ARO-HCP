...
func (c *createClusterScopedReadDesiresSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error {
	...
	// The provision shard's maestro consumer name is the management cluster
	// the HostedCluster is running on. That value also serves as the
	// kube-applier container's Cosmos partition key (the same string the
	// kube-applier binary is started with via --management-cluster).
	...
	if len(managementCluster) == 0 {
		return utils.TrackError(fmt.Errorf("provision shard %q has no management cluster (maestro consumer name)", clusterProvisionShard.ID()))
	}
	...
	crud, err := c.kubeApplierDBClient.KubeApplier(managementCluster).ReadDesires(parent)
	if err != nil {
...
func buildReadDesire(resourceIDString, managementCluster string, target kubeapplier.ResourceReference) *kubeapplier.ReadDesire {
...
func ensureReadDesire(
	...
	if existing.Spec.ManagementCluster == desired.Spec.ManagementCluster &&
