...
func (c *clusterClusterServiceUpdateDispatchSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error {
	...
	if database.IsNotFoundError(err) {
		...
		logger.Info("service provider cluster not found in cache, waiting")
		return nil
	}
	if err != nil {
	...
	// cluster service
	needsUpdate, err := ocm.ClusterUpdateDispatchConfigDiffers(cluster, serviceProviderCluster, clusterServiceCluster)
	if err != nil {
