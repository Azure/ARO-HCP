...
// issuanceObserver is controller #3. A SystemAdminCredentialWatchingController
// that fires per credential; when the credential's mirrored CSR
// ReadDesire reports status.certificate it flips the credential's Phase
// to Issued and copies the cert into Status.SignedCertificate. On
// CertificateDenied / Failed it flips Phase to Failed.
//
// The CSR ReadDesire kubeContent update is not directly wired into this
// controller's queue; we rely on the credential informer's periodic
// relist (resyncDuration) to pick up changes. Latency is bounded by
// that interval, which is acceptable because issuance is the only state
// transition this controller drives.
...
func (c *issuanceObserver) SyncOnce(ctx context.Context, key controllerutils.HCPSystemAdminCredentialKey) error {
	...
	// Only credentials still in Requested are eligible for an issuance
	// transition. Issued/Failed/Revoked are terminal as far as this
	...
	if denied := csrDenied(csr); denied != "" {
		...
		replacement.Status.Conditions = append(replacement.Status.Conditions, metav1.Condition{
			Type:               "CSRDenied",
