...
func TestDNSReservationCleanupController_SyncOnce(t *testing.T) {
	...
	dnsReservationResourceID := metadataapi.Must(coreapihelpers.ToDNSReservationResourceID(subscriptionID, dnsReservationName))
	serviceProviderClusterResourceID := metadataapi.Must(azcorearm.ParseResourceID(
		coreapihelpers.ToServiceProviderClusterResourceIDString(subscriptionID, resourceGroupName, clusterName)))
	...
	tests := []struct {
		...
		},
		{
			...
			serviceProviderCluster: spcWithReservation(coreapihelpers.ToDNSReservationResourceIDString(subscriptionID, "other-dns-name")),
			verify:                 verifyDeleted,
		},
		{
