...
func (c *initialNormalClusterDesiredVersionSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error {
	...
	if !ok {
		logger.Info("Cannot determine requested channel", "channelGroup", cluster.CustomerProperties.Version.ChannelGroup, "requestedVersion", cluster.CustomerProperties.Version.ID)
		return nil
	}
