...
func TestClusterMinor(t *testing.T) {
	...
	serviceProviderCluster := newTestServiceProviderCluster("c1", v("4.22.0"), []coreapi.ServiceProviderClusterActiveVersion{completed("4.21.6")}, nil)
	m, ok := clusterMinor(serviceProviderCluster)
	assert.True(t, ok)
	assert.Equal(t, "4.22", m)

	// falls back to earliest active
	serviceProviderCluster = newTestServiceProviderCluster("c1", nil, []coreapi.ServiceProviderClusterActiveVersion{completed("4.21.6")}, nil)
	m, ok = clusterMinor(serviceProviderCluster)
	assert.True(t, ok)
	...
	_, ok = clusterMinor(serviceProviderCluster)
	assert.False(t, ok)
}
