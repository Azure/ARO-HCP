...
func (c *normalClusterDesiredVersionSyncer) versionCandidateHandlers(queue controllerutils.Enqueuer) (cache.ResourceEventHandlerFuncs, cache.ResourceEventHandlerFuncs) {
	...
	enqueue := func(cluster *coreapi.HCPOpenShiftCluster, reason string) {
		...
		logger.Info("Enqueuing normal assignment for candidate channel", "resourceID", cluster.ID, "ystreamChannel", yStreamChannel, "reason", reason)
	...
	onServiceProviderCluster := func(obj any) {
		serviceProviderCluster, ok := obj.(*coreapi.ServiceProviderCluster)
		if !ok || serviceProviderCluster.Spec.ControlPlaneVersion.DesiredVersion != nil {
			return
		}
		subscription, resourceGroup, name, ok := serviceProviderClusterCoords(serviceProviderCluster)
		if !ok {
			logger.Info("Cannot enqueue normal assignment: ServiceProviderCluster has no cluster resource ID", "resourceID", serviceProviderCluster.ResourceID)
			return
		}
		cluster, err := c.clusterLister.Get(context.Background(), subscription, resourceGroup, name)
		if err != nil {
			logger.Error(err, "Cannot resolve candidate channel for ServiceProviderCluster; cluster events or rollout resync will retry", "resourceID", serviceProviderCluster.ResourceID)
			return
		}
