...
func (c *statusCollectorSyncer) enqueueStatusChannels(queue controllerutils.Enqueuer, minors ...string) {
...
func (c *statusCollectorSyncer) watchStatusInputs(clusters, serviceProviderClusters controllerutils.Notifier, queue controllerutils.Enqueuer) error {
	logger := utils.DefaultLogger().WithValues(utils.LogValues{}.AddControllerName(StatusCollectorControllerName)...)
	...
	_, clusterErr := clusters.AddEventHandlerWithOptions(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(oldObj, newObj any) {
			...
			if oldChannel != newChannel || oldCluster.CustomerProperties.Version.ChannelGroup != newCluster.CustomerProperties.Version.ChannelGroup ||
				(oldCluster.ServiceProviderProperties.DeletionTimestamp == nil) != (newCluster.ServiceProviderProperties.DeletionTimestamp == nil) {
				c.enqueueStatusChannels(queue, "")
			}
		},
	}, options)
