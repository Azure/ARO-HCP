...
func serviceProviderClustersForChannel(ctx context.Context, serviceProviderClusterLister corelisters.ServiceProviderClusterLister, clusterLister corelisters.ClusterLister, yStreamChannel string) ([]*coreapi.ServiceProviderCluster, error) {
	...
	for _, cluster := range clusters {
		if cluster.ID == nil || cluster.ServiceProviderProperties.DeletionTimestamp != nil {
			continue
		}
	...
	for _, serviceProviderCluster := range serviceProviderClusters {
		...
		if !ok || clusterMinorVersion != minor {
			logger.V(1).Info("Excluding cluster from rollout: minor version unknown or does not match", "resourceID", serviceProviderCluster.ResourceID, "minorKnown", ok, "clusterMinor", clusterMinorVersion, "channelMinor", minor, "desiredVersion", versionString(serviceProviderCluster.Spec.ControlPlaneVersion.DesiredVersion), "activeVersion", versionString(earliestActiveVersion(serviceProviderCluster.Status.ControlPlaneVersion.ActiveVersions)))
			continue
		}
		if serviceProviderCluster.ResourceID == nil || serviceProviderCluster.ResourceID.Parent == nil {
			logger.V(1).Info("Excluding cluster from rollout: missing cluster resource ID", "resourceID", serviceProviderCluster.ResourceID)
			continue
		}
		...
		if !ok || clusterChannelGroup != channelGroup {
			logger.V(1).Info("Excluding cluster from rollout: backing cluster missing, deleting, or channel group does not match", "resourceID", serviceProviderCluster.ResourceID, "clusterFound", ok, "clusterChannelGroup", clusterChannelGroup, "channelGroup", channelGroup)
			continue
		}
		logger.V(1).Info("Including cluster in rollout channel", "resourceID", serviceProviderCluster.ResourceID)
