...
func (c *minorUpgradeNormalClusterDesiredVersionSyncer) NeedsWork(cluster *coreapi.Cluster, serviceProviderCluster *coreapi.ServiceProviderCluster) bool {
...
func (c *minorUpgradeNormalClusterDesiredVersionSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error {
	...
	if cosmosstorageutils.IsNotFoundError(err) {
		logger.Info("Waiting for rollout", "ystreamChannel", yStreamChannel, "retryAfter", 10*time.Second)
		c.enqueueAfter.EnqueueAfter(key, 10*time.Second)
		return nil
	}
	if err != nil {
		return utils.TrackError(fmt.Errorf("failed to get rollout %q: %w", yStreamChannel, err))
	}
	if rollout.Spec.BestExactVersion == nil {
		logger.Info("Waiting for best version", "ystreamChannel", yStreamChannel, "retryAfter", 10*time.Second)
		c.enqueueAfter.EnqueueAfter(key, 10*time.Second)
		return nil
	}
