...
// marshal serializes the graph to Cincinnati JSON format with channel
// membership metadata. channelMembership maps version strings to the
// set of channels they appear in across all server channels.
func (g *Graph) marshal(channelMembership map[string][]string) ([]byte, error) {
	nodes := make([]graphNode, len(g.nodes))
	copy(nodes, g.nodes)
	for i := range nodes {
		if channels, ok := channelMembership[nodes[i].Version]; ok {
			if nodes[i].Metadata == nil {
				nodes[i].Metadata = make(map[string]string)
			}
			nodes[i].Metadata["io.openshift.upgrades.graph.release.channels"] = strings.Join(channels, ",")
		}
	}
	edges := g.edges
	if edges == nil {
...
// NodeVersions returns all version strings in the graph.
func (g *Graph) NodeVersions() []string {
	versions := make([]string, len(g.nodes))
