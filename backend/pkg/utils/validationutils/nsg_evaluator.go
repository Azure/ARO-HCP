...
// not blocked on the given ports (typically TCP 443/6443). Fails when an
...
func sourceMatchesSubnet(prefixes []string, subnet netip.Prefix) bool {
	...
	for _, p := range prefixes {
		p = strings.TrimSpace(p)
		if isAnyAddressPrefix(p) || strings.EqualFold(p, "VirtualNetwork") {
			return true
		}
...
func sourceSpecificallyFromWorker(prefixes []string, workerSubnet netip.Prefix) bool {
	for _, p := range prefixes {
		p = strings.TrimSpace(p)
		if isAnyAddressPrefix(p) || p == "" {
			continue
		}
		if strings.EqualFold(p, "VirtualNetwork") {
			return true
		}
		rulePrefix, err := parsePrefixOrAddr(p)
		if err != nil {
...
func destinationIsVNetOrSubnet(prefixes []string, subnet netip.Prefix) bool {
	for _, p := range prefixes {
		...
		if strings.EqualFold(p, "Internet") {
			continue
		}
		if strings.EqualFold(p, "VirtualNetwork") {
...
func sourcesOverlap(allowPrefixes, denyPrefixes []string, subnet netip.Prefix) bool {
	...
	for _, denyP := range denyPrefixes {
		...
		if strings.EqualFold(denyP, "VirtualNetwork") {
...
func allowDestinationCoversDeny(allowPrefixes, denyPrefixes []string) bool {
	...
	for _, denyP := range denyPrefixes {
		denyP = strings.TrimSpace(denyP)
		if strings.EqualFold(denyP, "Internet") {
			if !allowHasTag(allowPrefixes, "Internet") {
				return false
			}
			continue
		}
		if strings.EqualFold(denyP, "VirtualNetwork") {
			if !allowHasTag(allowPrefixes, "VirtualNetwork") {
				return false
			}
			continue
		}
...
func allowCoversPrefix(allowPrefixes []string, denyPrefix netip.Prefix) bool {
	for _, p := range allowPrefixes {
		...
		if strings.EqualFold(p, "VirtualNetwork") && isPrivateAddr(denyPrefix.Addr()) {
			return true
		}
	...
	for _, r := range ranges {
		...
		if strings.Contains(r, ",") {
			for _, part := range strings.Split(r, ",") {
				if portRangesMatch([]string{strings.TrimSpace(part)}, port) {
					return true
				}
			}
			continue
		...
		if strings.Contains(r, "-") {
			...
			lo, errLo := strconv.ParseInt(parts[0], 10, 32)
			hi, errHi := strconv.ParseInt(parts[1], 10, 32)
			if errLo != nil || errHi != nil {
				continue
			}
			if int32(lo) <= port && port <= int32(hi) {
				return true
			}
			continue
		}
