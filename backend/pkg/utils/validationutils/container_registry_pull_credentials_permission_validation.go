...
func (v *ContainerRegistryPullCredentialsPermissionValidation) Validate(ctx context.Context, clusterSubscription *coreapi.Subscription, cluster *coreapi.HCPOpenShiftCluster) error {
	containerRegistryPullMI := cluster.CustomerProperties.Platform.ContainerRegistryPullManagedIdentity
	...
	if err != nil {
		return utils.TrackError(fmt.Errorf("failed to check CAPZ assign/action permission on container registry pull MI: %w", err))
	}
