...
func (v *ContainerRegistryPullCredentialsPermissionValidation) Validate(ctx context.Context, clusterSubscription *coreapi.Subscription, cluster *coreapi.Cluster) ValidationResult {
	...
	if !strings.EqualFold(containerRegistryPullMI.SubscriptionID, cluster.ID.SubscriptionID) {
	...
	if clusterSubscription.Properties == nil || clusterSubscription.Properties.TenantId == nil {
		return UnknownValidation(
			"InternalError",
			"Unable to verify container registry pull credentials permissions.",
			"cluster subscription has no tenant ID",
			ControllerReportingPolicyTypeError,
		)
	}
	...
	if resp == nil {
		return UnknownValidation(
			"InternalError",
			"Unable to verify container registry pull credentials permissions.",
			fmt.Sprintf("CheckAccess response for container registry pull MI %s is unexpectedly nil, indicating a possible internal failure", containerRegistryPullMI),
			ControllerReportingPolicyTypeError,
		)
	}
