...
func (v *AzureCustomerNSGValidation) Validate(ctx context.Context, cluster *api.HCPOpenShiftCluster, _ *arm.Subscription, nodePool *api.HCPOpenShiftClusterNodePool) error {
	...
	err = v.validateInboundNSGRules(inbound, workerPrefixes, vnetIntegrationPrefixes, KASPorts)
	if err != nil {
		return utils.TrackError(err)
	}
	return nil
