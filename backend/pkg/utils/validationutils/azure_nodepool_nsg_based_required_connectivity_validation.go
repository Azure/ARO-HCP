...
func (v *AzureNodePoolNSGBasedRequiredConnectivityValidation) Validate(ctx context.Context, cluster *coreapi.Cluster, _ *coreapi.Subscription, nodePool *coreapi.ClusterNodePool) ValidationResult {
	...
	// If the worker subnet has no nsg attached, then Azure does not block any incoming or outgoing traffic for resources in that subnet.
	...
	// If the vnet-integration subnet has no NSG attached, then Azure does not block any incoming or outgoing traffic for resources in that subnet.
