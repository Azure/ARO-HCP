...
type ManagementClusterLister interface {
	...
	Get(ctx context.Context, subscriptionID, resourceGroupName, name string) (*api.ManagementCluster, error)
...
// Get retrieves a single management cluster by subscription, resource group, and name.
// The store key is the lowercased resource ID string derived from the AKS cluster identity.
func (l *informerBasedManagementClusterLister) Get(ctx context.Context, subscriptionID, resourceGroupName, name string) (*api.ManagementCluster, error) {
	key := api.ToManagementClusterResourceIDString(subscriptionID, resourceGroupName, name)
...
func (l *informerBasedManagementClusterLister) GetByCSProvisionShardID(ctx context.Context, shardID string) (*api.ManagementCluster, error) {
	results, err := listFromIndex[api.ManagementCluster](l.indexer, ByCSProvisionShard, shardID)
	if err != nil {
		return nil, err
	}
	switch len(results) {
