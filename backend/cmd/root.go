...
func (f *BackendRootCmdFlags) AddFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringVar(&f.BackupScheduleState, "backup-schedule-state", f.BackupScheduleState,
		"Backup schedule state. Accepted values: enabled, paused")
...
func (f *BackendRootCmdFlags) validate() error {
	...
	if f.BackupScheduleState != "enabled" && f.BackupScheduleState != "paused" {
		return utils.TrackError(fmt.Errorf("--backup-schedule-state must be \"enabled\" or \"paused\""))
	}
