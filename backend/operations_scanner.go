...
func (s *OperationsScanner) pollClusterOperation(ctx context.Context, op operation) {
	...
	if err != nil {
		...
		if errors.As(err, &ocmError) && ocmError.Status() == http.StatusNotFound && op.doc.Request == database.OperationRequestDelete {
			err = s.deleteOperationCompleted(ctx, op)
			if err != nil {
...
// deleteOperationCompleted updates Cosmos DB to reflect a completed resource deletion.
func (s *OperationsScanner) deleteOperationCompleted(ctx context.Context, op operation) error {
