...
requires all three valid tags, including explicit zero-capacity baselines for
unused roles. Missing tags are a normal provisioning wait: it requeues after two
minutes without listing pools, resolving SKUs/quotas, or issuing pool or tag writes.
It never initializes its own baseline. Malformed tags remain errors. Once the
baseline is complete, incomplete ARM/SKU capacity data or a tier that allocates
no pools stops reconciliation, including an optional tier. Partial nonzero
allocation remains valid only if its desired capacity meets every role's baseline.
...
1. **Gather** — read the AKS cluster and baseline tags. If any role's baseline is
   missing, schedule a retry and stop here. Otherwise require successful cluster
   provisioning and list live agent pools. Only pools carrying a nonempty
   `aro-hcp.azure.com/role` label are managed; unknown roles fail capacity validation.
2. **Resolve** (`compute.ResolveDesiredPools`) — fetch cached SKU metadata and
   ...
   SKUs with an Azure `Location` restriction matching the controller's region
   are excluded from both regional and zonal allocations, regardless of the
   restriction reason. Their resource metadata remains cached for existing-pool
   capacity and quota accounting. Restrictions in other regions do not exclude
   a SKU; zone restrictions continue to be evaluated per tier.
