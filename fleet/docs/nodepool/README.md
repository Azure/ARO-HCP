...
worker pools. It is selected per environment via the `fleet.workerNodePool.profile`
...
### Why freeze exists, and the drain lifecycle

An undesired pool (wrong family/size, or no longer in the plan) cannot simply be
deleted — its nodes may be running workloads, and its autoscaler would fight any
scale-down. Removal is therefore a gentle, multi-reconcile lifecycle:
...
The result: undesired capacity disappears without ever force-evicting a pod, and
desired capacity is always in place first.
...
- Ignore the per-family default quota?
- Bootstrapping when no quota exists (quotaizer reacts to node pools, but the
  controller only creates pools when quota is available — chicken/egg).
- Should a single zone mix SKUs?
