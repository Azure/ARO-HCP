...
func TestResolveDesiredPools(t *testing.T) {
	t.Run("happy path allocates pools and passes tier families to fetchQuotaUsage", func(t *testing.T) {
		skuCache := newResolveTestCache(t, []*armcompute.ResourceSKU{
			resolveTestSKU("Standard_E16ds_v6", "standardEDSv6Family", 16),
		}, nil)

		var gotFamilies sets.Set[VMFamily]
		fetchQuotaUsage := func(families sets.Set[VMFamily]) (map[VMFamily]QuotaUsage, error) {
		...
		assert.True(t, gotFamilies.Equal(sets.New[VMFamily]("standardEDSv6Family")), "expected fetchQuotaUsage to receive the tier's families, got %v", gotFamilies)
		assert.NotEmpty(t, result.Pools, "expected at least one pool to be allocated")
		assert.Empty(t, result.Failures)
		assert.Equal(t, map[VMFamily]int64{"standardEDSv6Family": 1000}, result.FamilyBudgets)
		assert.Contains(t, result.SKUMetadata, "Standard_E16ds_v6")
	})

	t.Run("SKU metadata fetch error is wrapped", func(t *testing.T) {
		skuCache := newResolveTestCache(t, nil, errors.New("resource SKUs API unavailable"))

		_, err := ResolveDesiredPools(utils.ContextWithLogger(context.Background(), logr.Discard()), skuCache, resolveTestSubscriptionID, testProfile(), allZones,
			func(sets.Set[VMFamily]) (map[VMFamily]QuotaUsage, error) {
				t.Fatal("fetchQuotaUsage should not be called when SKU metadata fetch fails")
				return nil, nil
			})

		require.Error(t, err)
		assert.Contains(t, err.Error(), "fetching SKU metadata")
		assert.Contains(t, err.Error(), "resource SKUs API unavailable")
	})

	t.Run("budget computation error is wrapped", func(t *testing.T) {
		skuCache := newResolveTestCache(t, []*armcompute.ResourceSKU{
			resolveTestSKU("Standard_E16ds_v6", "standardEDSv6Family", 16),
		}, nil)
		...
		_, err := ResolveDesiredPools(utils.ContextWithLogger(context.Background(), logr.Discard()), skuCache, resolveTestSubscriptionID, testProfile(), allZones,
			func(sets.Set[VMFamily]) (map[VMFamily]QuotaUsage, error) {
				return nil, errors.New("quota API unavailable")
			})

		require.Error(t, err)
		assert.Contains(t, err.Error(), "computing family budgets")
		assert.Contains(t, err.Error(), "quota API unavailable")
	})
}
...
// set, per-family budgets, and allocation failures.
func TestResolveDesiredPools_ProductionScenario(t *testing.T) {
	...
	fetchQuotaUsage := func(families sets.Set[VMFamily]) (map[VMFamily]QuotaUsage, error) {
		usage := map[VMFamily]QuotaUsage{
			...
			"standardESv3Family":  {Limit: 100, CurrentValue: 0},
		}
	...
	golden := struct {
		FamilyBudgets map[VMFamily]int64  `json:"familyBudgets"`
		Pools         []Pool              `json:"pools"`
		Failures      []AllocationFailure `json:"failures,omitempty"`
	}{
		FamilyBudgets: result.FamilyBudgets,
		Pools:         result.Pools,
