...
// UnlimitedVCPUs is the per-family vCPU limit reported by UnlimitedBudget. It is
// a large-but-comprehensible number rather than math.MaxInt64: high enough to
// exceed any realistic per-family demand, yet low enough that headroom
// arithmetic — which adds freed running vCPUs back onto the limit (see
// liveBudget) — can never overflow int64 and the projection trace stays
// human-readable.
const UnlimitedVCPUs int64 = 1_000_000
...
func UnlimitedBudget(_ context.Context, families sets.Set[VMFamily], _ FetchQuotaUsageFunc) (map[VMFamily]QuotaUsage, error) {
	...
	for _, family := range families.UnsortedList() {
		budgets[family] = QuotaUsage{Limit: UnlimitedVCPUs}
	}
	return budgets, nil
}
