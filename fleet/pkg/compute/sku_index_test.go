...
func TestBuildEligibleSKUIndex(t *testing.T) {
	tests := []struct {
		...
		{
			name: "zone filtering excludes SKU missing required zone",
			skuMetadata: map[string]*skucache.SKUMetadata{
		...
		{
			name: "zone filtering includes SKU with all required zones",
			skuMetadata: map[string]*skucache.SKUMetadata{
				"Standard_E32ds_v6": {VCPUs: 32, Family: "standardEDSv6Family", EphemeralOSDiskSupported: true, EphemeralDiskSizeGB: 1792, Zones: []string{"1", "2", "3"}},
			},
			requiredZones: []string{"1", "2", "3"},
			lookupFamily:  "standardEDSv6Family",
			lookupCores:   32,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			index := BuildEligibleSKUIndex(test.skuMetadata, test.requiredZones)
