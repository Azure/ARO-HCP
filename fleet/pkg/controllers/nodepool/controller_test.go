...
type nodePoolEnqueueAfterFunc func(any, time.Duration)

func (f nodePoolEnqueueAfterFunc) EnqueueAfter(key any, delay time.Duration) {
	f(key, delay)
}

func TestSyncOnceWaitsForCapacityBaseline(t *testing.T) {
	key := fleetcontrollers.ManagementClusterKey{StampIdentifier: syncOnceTestStampID}
	var tags map[string]*string
	server := armcontainerservicefake.ManagedClustersServer{
		Get: func(context.Context, string, string, *armcontainerservice.ManagedClustersClientGetOptions) (resp azfake.Responder[armcontainerservice.ManagedClustersClientGetResponse], errResp azfake.ErrorResponder) {
			resp.SetResponse(http.StatusOK, armcontainerservice.ManagedClustersClientGetResponse{ManagedCluster: armcontainerservice.ManagedCluster{
				Tags: tags, Properties: &armcontainerservice.ManagedClusterProperties{ProvisioningState: ptr.To("Succeeded")},
			}}, nil)
			return
		},
		// No write handlers: the controller must not create its own baseline.
	}
	discoveryError := errors.New("pool discovery unavailable")
	poolDiscoveryCalls, requeues := 0, 0
	syncer := &nodePoolSyncer{
		managementClusterLister: &fakeManagementClusterLister{mc: syncOnceTestManagementCluster(syncOnceTestAKSResourceID)},
		credential:              &azfake.TokenCredential{},
		...
		enqueueAfter: nodePoolEnqueueAfterFunc(func(got any, delay time.Duration) {
			require.Equal(t, key, got)
			require.Positive(t, delay)
			requeues++
		}),
		// No SKU cache or allocation profile: waiting must not require either.
	}
	require.NoError(t, syncer.SyncOnce(testSyncOnceContext(), key), "an absent baseline is a normal provisioning wait")
	require.Zero(t, poolDiscoveryCalls)
	require.Equal(t, 1, requeues)

	// An absent infra role still blocks reconciliation even though its expected
	// capacity is zero. Every role must have an explicitly recorded baseline.
	tags = map[string]*string{
		"arohcp-capacity-system": ptr.To(`{"vcpus":12,"memoryGiB":48,"swiftNICs":0}`),
		"arohcp-capacity-worker": ptr.To(`{"vcpus":8,"memoryGiB":32,"swiftNICs":0}`),
	}
	require.NoError(t, syncer.SyncOnce(testSyncOnceContext(), key))
	require.Zero(t, poolDiscoveryCalls)
	require.Equal(t, 2, requeues)

	// Once provisioning records the final role, the next queued sync enters
	// normal reconciliation and exposes downstream errors instead of waiting.
	tags["AROHCP-Capacity-infra"] = ptr.To(`{"vcpus":0,"memoryGiB":0,"swiftNICs":0}`)
	require.ErrorIs(t, syncer.SyncOnce(testSyncOnceContext(), key), discoveryError)
	require.Equal(t, 1, poolDiscoveryCalls)
	require.Equal(t, 2, requeues)
}
