...
func (s *nodePoolSyncer) SyncOnce(ctx context.Context, key fleetcontrollers.ManagementClusterKey) error {
	...
	// Baselines are the provisioning handoff. Do not inspect or plan pools
	// until every role has been recorded, including roles with zero capacity.
	if len(capacityBaseline) != len(compute.CapacityRoles) {
		logger.Info("waiting for cluster capacity baselines")
		s.enqueueAfter.EnqueueAfter(key, requeueAfterCreate)
		return nil
	}
