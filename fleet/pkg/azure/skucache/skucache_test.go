	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for vmSize, wantMem := range test.wantMemoryGB {
				meta, ok := metadata[vmSize]
				require.True(t, ok, "expected %q in result", vmSize)
				assert.Equal(t, wantMem<<30, meta.MemoryBytes, "%s memory bytes", vmSize)
			}
...
func TestExtractSKUMetadata(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			meta := extractSKUMetadata(test.sku, testRegion)
			assert.Equal(t, test.wantVCPUs, meta.VCPUs, "VCPUs")
			assert.Equal(t, test.wantMemoryGB<<30, meta.MemoryBytes, "memory bytes")
...
func TestSKUCachePreservesFractionalMemory(t *testing.T) {
	cache := newTestCache(t, []*armcompute.ResourceSKU{
		makeVMResourceSKU("Standard_Half", map[string]string{"MemoryGB": "3.5", "vCPUs": "2"}),
	}, nil)
	metadata, err := cache.SKUMetadataByVMSize(context.Background(), testSubscriptionID)
	require.NoError(t, err)
	require.Contains(t, metadata, "Standard_Half")

	resources := metadata["Standard_Half"].ResourceList()
	require.Zero(t, resources.Memory().Cmp(resource.MustParse("3.5Gi")))
}
