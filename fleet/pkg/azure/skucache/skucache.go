...
func (m *SKUMetadata) ResourceList() corev1.ResourceList {
	...
	if m.MemoryBytes > 0 {
		rl[corev1.ResourceMemory] = *resource.NewQuantity(m.MemoryBytes, resource.BinarySI)
	}
...
func (c *SKUCache) fetchSKUMetadata(ctx context.Context, subscriptionID string) (map[string]*SKUMetadata, error) {
	...
	for pager.More() {
		...
		for _, sku := range page.Value {
			...
			meta := extractSKUMetadata(sku, c.region)
...
func extractSKUMetadata(sku *armcompute.ResourceSKU, region string) *SKUMetadata {
	meta := &SKUMetadata{
		Name:               *sku.Name,
		LocationRestricted: isLocationRestricted(sku, region),
	}
	...
	if raw, ok := lookupCapability(sku, capabilityNameMemoryGB); ok {
		// Azure names this capability MemoryGB, but reports GiB.
		if memory, err := resource.ParseQuantity(raw + "Gi"); err == nil {
			meta.MemoryBytes = memory.Value()
		}
	}
...
func (m *SKUMetadata) HasResources() bool {
	return m.VCPUs > 0 || m.MemoryBytes > 0 || m.SecondaryNICs > 0
}

func isLocationRestricted(sku *armcompute.ResourceSKU, region string) bool {
	for _, restriction := range sku.Restrictions {
		...
		// Both fields describe restricted locations. The reason explains the
		// restriction; it does not change whether new allocations may use it.
		for _, location := range restriction.Values {
