...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "nodepool-zones has non-single-digit value",
			modify:  func(opts *RawControllerOptions) { opts.NodePoolZones = "1,2,10" },
			wantErr: true,
		},
		{
			name:    "nodepool-zones has non-digit value",
			modify:  func(opts *RawControllerOptions) { opts.NodePoolZones = "1,2,a" },
			wantErr: true,
		},
		{
		...
		{
			name:    "nodepool-zones has too few zones",
			modify:  func(opts *RawControllerOptions) { opts.NodePoolZones = "1,2" },
			wantErr: true,
		},
		{
			name:    "nodepool-zones has too many zones",
			modify:  func(opts *RawControllerOptions) { opts.NodePoolZones = "1,2,3,4" },
			wantErr: true,
		},
	}
