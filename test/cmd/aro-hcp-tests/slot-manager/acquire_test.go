...
type fakeAcquireAssetHandler struct {
	calls      *[]string
	prepareErr error
}

func (h *fakeAcquireAssetHandler) Kind() assets.Kind {
	return assets.KindE2EIdentities
}

func (h *fakeAcquireAssetHandler) Declared(slots.Pool) bool {
	return true
}

func (h *fakeAcquireAssetHandler) AcquireLease(context.Context, assets.LeaseRequest) error {
	*h.calls = append(*h.calls, "resolve")
	return nil
}

func (h *fakeAcquireAssetHandler) ReleaseLease(context.Context, assets.LeaseRequest) error {
	return nil
}

func (h *fakeAcquireAssetHandler) ApplyPools(context.Context, assets.PoolRequest) error {
	return nil
}

func (h *fakeAcquireAssetHandler) ValidatePools(context.Context, assets.PoolRequest) error {
	return nil
}

func (h *fakeAcquireAssetHandler) PrepareLease(context.Context, assets.LeaseRequest) error {
	*h.calls = append(*h.calls, "prepare")
	return h.prepareErr
}

func (h *fakeAcquireAssetHandler) ValidateLease(context.Context, assets.LeaseRequest) error {
	*h.calls = append(*h.calls, "validate")
	return nil
}

func (h *fakeAcquireAssetHandler) PublishLease(_ context.Context, _ assets.LeaseRequest, contract *slots.RuntimeContractBuilder) error {
	*h.calls = append(*h.calls, "publish")
	return contract.Add(string(h.Kind()), "LEASED_MSI_CONTAINERS", "identity-rg")
}

func TestAcquireV2InvalidRuntimeStateRollsBackBeforePreparation(t *testing.T) {
	t.Parallel()
	...
	defer server.Close()
	calls := []string{}
	registry, err := assets.NewRegistry(&fakeAcquireAssetHandler{calls: &calls})
	if err != nil {
		t.Fatal(err)
	}
	options := lifecycleOptions(t, `version: 2
environments:
  dev:
...
`, server.URL, registry)
	options.ResolveSubscriptions = func(context.Context, string, string, string, string) (slots.ResolvedSubscriptions, error) {
		...
		if err != nil || state.Leases.Primary.ResourceName != "aro-hcp-dev-shard0-slot-00" {
			t.Fatalf("primary journal must precede resolution: %+v, %v", state, err)
		}
		return slots.ResolvedSubscriptions{}, nil
	}
	err = Acquire(context.Background(), options)
	if err == nil || !strings.Contains(err.Error(), "unresolved E2E subscription") {
		t.Fatalf("expected full runtime validation failure, got %v", err)
	}
	if !equalStrings(calls, []string{"resolve"}) {
		t.Fatalf("invalid runtime state reached asset preparation: %v", calls)
	}
	if !equalStrings(*released, []string{"aro-hcp-dev-shard0-slot-00"}) {
		t.Fatalf("invalid runtime state blocked rollback: %v", *released)
	}
	if _, err := slots.LoadAcquiredSlotState(options.SharedDir); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("successful rollback did not remove journal: %v", err)
	}
	envFile, err := slots.EnvFile(options.SharedDir)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(envFile); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("invalid runtime state published an env file: %v", err)
	}
}
...
func TestAcquireV2PersistsStateBeforeAssetAdmissionFailure(t *testing.T) {
	t.Parallel()
...
environments:
  dev:
    deployment_environment:
      name: ci01
      infrastructure_subscription: dev-infra
    pools:
    - name: shard0
      region: westus3
      ...
      slot_assets:
        e2e_identities:
          allocation: dedicated
          resource_group_prefix: identity-rg
          resource_group_count: 1
`)
	server, _, releaseCalls := newTestLeaseProxyServer(t, map[string][]leaseProxyReply{
		"aro-hcp-dev-shard0-slot": {successAcquireReply("aro-hcp-dev-shard0-slot-00")},
	})
	defer server.Close()

	expectedErr := errors.New("asset is dirty")
	calls := []string{}
	registry, err := assets.NewRegistry(&fakeAcquireAssetHandler{calls: &calls, prepareErr: expectedErr})
	if err != nil {
		t.Fatalf("expected fake asset registry construction to succeed: %v", err)
	}
	sharedDir := t.TempDir()
	...
	if !errors.Is(err, expectedErr) {
		t.Fatalf("expected asset preparation failure, got %v", err)
	}
	if got, want := calls, []string{"resolve", "prepare"}; !equalStrings(got, want) {
		t.Fatalf("unexpected asset lifecycle calls: got %v want %v", got, want)
	}
	if _, err := slots.LoadAcquiredSlotState(sharedDir); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("expected successful rollback to remove state, got %v", err)
	}
	envPath, err := slots.EnvFile(sharedDir)
	if err != nil {
		t.Fatalf("expected env path resolution to succeed: %v", err)
	}
	if _, err := os.Stat(envPath); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("expected no runtime contract after admission failure, got %v", err)
	}
	if len(*releaseCalls) != 1 {
		t.Fatalf("expected immediate rollback after admission failure, got release calls %v", *releaseCalls)
	}
}
...
func TestParseLocationWeights(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "leading separator", raw: ",westus3=1,centralus=1,canadacentral=1", wantError: "location=weight format"},
		{name: "trailing separator", raw: "westus3=1,centralus=1,canadacentral=1,", wantError: "location=weight format"},
		{name: "repeated separator", raw: "westus3=1,,centralus=1,canadacentral=1", wantError: "location=weight format"},
		{name: "repeated newline separator", raw: "westus3=1\n\ncentralus=1\ncanadacentral=1", wantError: "location=weight format"},
