...
import (
	...
	testartifacts "github.com/Azure/ARO-HCP/test/e2e/test-artifacts"
...
func BindApplyOptions(opts *RawApplyOptions, cmd *cobra.Command) {
	cmd.Flags().StringVar(&opts.Environment, "environment", opts.Environment, "Environment suffix used in resourceGroupBaseName (e.g. int, stg, dev)")
	cmd.Flags().IntVar(&opts.PoolSize, "pool-size", opts.PoolSize, "poolSize parameter passed to the bicep template")
}
...
func (o *ValidatedApplyOptions) Complete(ctx context.Context) (*ApplyOptions, error) {
	...
	template, err := testartifacts.TestArtifactsFS.ReadFile("generated-test-artifacts/msi-pools.json")
	if err != nil {
