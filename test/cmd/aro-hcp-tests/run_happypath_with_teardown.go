...
func NewRunHappyPathWithValidationsCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, err = integration.FallbackCreateClusterWithBicep(
				ctx,
				"e2e/test-artifacts/e2e-setup.json",
				*resourceGroup.Name,
			...
			// Ensure SETUP_FILEPATH is set before running happy-path specs
			specsHappyPath.AddBeforeAll(func() {
				if _, found := os.LookupEnv("SETUP_FILEPATH"); !found {
					// Default to repo-local test artifacts path if not provided externally
					_ = os.Setenv("SETUP_FILEPATH", "e2e/test-artifacts/e2e-setup.json")
				}
			})

			// Ensure SETUP_FILEPATH is set before running teardown validation specs
			...
			// To Do Cleanup Cluster

			// Run teardown validation specs
