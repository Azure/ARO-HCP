...
func newCleanupCommand() *cobra.Command {
	...
	cmd.PersistentFlags().DurationVar(&opt.InterruptTimeout, "interrupt-timeout", opt.InterruptTimeout, "Grace period after first interrupt")

	cmd.AddCommand(newCleanupResourceGroupsCommand())

	cmd.PersistentPreRun = func(cmd *cobra.Command, args []string) {
		ctx := logr.NewContext(newCleanupContext(opt.InterruptTimeout), createLogger(opt.Verbosity))
		cmd.SetContext(ctx)
	}
	return cmd
}
...
func newDeleteExpiredResourceGroupsCommand() *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		return opt.Run(logr.NewContext(newCleanupContext(30*time.Minute), createLogger(0)))
	}
...
func newCleanupContext(timeout time.Duration) context.Context {
	...
	go func() {
		...
		<-abortCh
		fmt.Fprintf(os.Stderr, "Interrupted, terminating cleanup\n")
