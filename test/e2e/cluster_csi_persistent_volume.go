...
var _ = Describe("Customer", func() {
	It("should provision Azure Disk and Azure File CSI volumes and run a pod against each",
		...
		func(ctx context.Context) {
			...
			clusterName := "cluster-csi-" + suffix
			...
			resourceGroup, err := tc.NewResourceGroup(ctx, "rg-csi-", tc.Location())
			...
			clusterParams, err = tc.CreateClusterCustomerResources20251223(ctx,
				...
				map[string]interface{}{
					"customerNsgName":        "customer-nsg-csi-" + suffix,
					"customerVnetName":       "customer-vnet-csi-" + suffix,
					"customerVnetSubnetName": "customer-vnet-subnet-csi-" + suffix,
				},
				TestArtifactsFS,
			...
			err = verifiers.VerifyNodesReady().Verify(ctx, adminRESTConfig)
			Expect(err).NotTo(HaveOccurred(), "failed to verify worker nodes are ready on cluster %q", clusterName)

			const csiVolumeTimeout = 10 * time.Minute

			By("verifying Azure Disk and Azure File CSI can provision volumes and run pods against them")
			err = verifiers.VerifyHCPCluster(ctx, adminRESTConfig,
				verifiers.VerifyAzureDiskCSIVolume(csiVolumeTimeout),
				verifiers.VerifyAzureFileCSIVolume(csiVolumeTimeout),
			)
			Expect(err).NotTo(HaveOccurred(), "failed to verify CSI volumes on cluster %q", clusterName)
		},
	)
})
