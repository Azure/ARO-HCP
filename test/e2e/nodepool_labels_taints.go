...
var _ = Describe("Customer", func() {
	...
	It("should be able to update node pool labels and taints",
		...
		func(ctx context.Context) {
			...
			nodePool := framework.BuildNodePoolFromParams20260901(nodePoolParams, tc.Location())

			nodePool.Properties.Labels = []*hcpsdk20260901preview.Label{
				{
					Key:   to.Ptr("key1"),
					Value: to.Ptr("value1"),
				},
			}
			nodePool.Properties.Taints = []*hcpsdk20260901preview.Taint{
				{
					Key:    to.Ptr("key1"),
					Value:  to.Ptr("value1"),
			...
			_, err = framework.CreateNodePoolAndWait20260901(
				ctx,
			...
			updateTaints := hcpsdk20260901preview.NodePoolUpdate{
					...
					Taints: []*hcpsdk20260901preview.Taint{
						{
							Key:    to.Ptr("key2"),
							Value:  to.Ptr("value2"),
							Effect: to.Ptr(hcpsdk20260901preview.EffectPreferNoSchedule),
						},
					},
				},
			}
