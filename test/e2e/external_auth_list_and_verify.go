...
var _ = Describe("Customer", func() {
	...
	It("should be able to lifecycle and confirm external auth on a cluster",
		...
		func(ctx context.Context) {
			...
			expectedExternalAuth := hcpsdk20251223preview.ExternalAuth{
			...
			result, err := tc.Get20251223ClientFactoryOrDie(ctx).NewExternalAuthsClient().Get(
				ctx,
				*resourceGroup.Name,
				clusterName,
				*expectedExternalAuth.Name,
			...
			Expect(*result.Properties.ProvisioningState).To(Equal(hcpsdk20251223preview.ExternalAuthProvisioningStateSucceeded), "external auth provisioning state should be Succeeded")
			...
			externalAuthClient := tc.Get20251223ClientFactoryOrDie(ctx).NewExternalAuthsClient()
			pager := externalAuthClient.NewListByParentPager(*resourceGroup.Name, clusterName, nil)
			var extAuthResult []hcpsdk20251223preview.ExternalAuth
			for pager.More() {
			...
			updatedResult, err := tc.Get20251223ClientFactoryOrDie(ctx).NewExternalAuthsClient().Get(
				ctx,
				*resourceGroup.Name,
				clusterName,
				*expectedExternalAuth.Name,
			...
			Expect(*updatedResult.Properties.ProvisioningState).To(Equal(hcpsdk20251223preview.ExternalAuthProvisioningStateSucceeded), "updated external auth provisioning state should be Succeeded")
			...
			err = framework.DeleteExternalAuthAndWait20251223(
				ctx,
				externalAuthClient,
				*resourceGroup.Name,
				clusterName,
