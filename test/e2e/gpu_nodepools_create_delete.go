...
var _ = Describe("HCP Nodepools GPU instances", func() {
	It("creates and deletes a GPU nodepool in a single cluster",
		...
		func(ctx context.Context) {
			...
			defaultNodePoolParams := framework.NewDefaultNodePoolParams20260901()
			...
			gpuNodePoolParams := framework.NewDefaultNodePoolParams20260901()
			...
			Expect(*created.Properties.ProvisioningState).To(Equal(hcpsdk20260901preview.ProvisioningStateSucceeded), "GPU nodepool %s provisioning state should be Succeeded", gpuNodePoolName)
			...
			Expect(framework.DeleteNodePool20260901(
				ctx,
				tc.Get20260901ClientFactoryOrDie(ctx).NewNodePoolsClient(),
				*resourceGroup.Name,
				customerClusterName,
			...
			_, getErr := framework.GetNodePool20260901(ctx,
				tc.Get20260901ClientFactoryOrDie(ctx).NewNodePoolsClient(),
				*resourceGroup.Name,
				customerClusterName,
				gpuNodePoolName,
