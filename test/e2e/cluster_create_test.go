...
var _ = Describe("Put HCPOpenShiftCluster", func() {
	It("Attempts to put HCPOpenshiftCluster with non-existent Resource Group and cluster resource as nil", labels.CreateCluster, labels.RequireNothing, labels.Medium, labels.Negative, func(ctx context.Context) {
		...
		var (
			clusterResource hcpsdk20251223preview.HcpOpenShiftCluster
			clusterOptions  *hcpsdk20251223preview.HcpOpenShiftClustersClientBeginCreateOrUpdateOptions
		)

		By("Sending put request to create HCPOpenshiftCluster")
		_, err := tc.Get20251223ClientFactoryOrDie(ctx).NewHcpOpenShiftClustersClient().BeginCreateOrUpdate(ctx, customerRGName, clusterName, clusterResource, clusterOptions)
