...
var _ = Describe("Customer", func() {
	It("should create a cluster using the 2026-05-31-preview API version",
		...
		func(ctx context.Context) {
			const (
				clusterName = "v20260531preview-cluster"
			...
			if !framework.IsDevelopmentEnvironment() {
				...
				if !available {
					if time.Now().After(framework.Must(time.Parse(time.RFC3339, "2026-06-30T00:00:00Z"))) {
						Fail(fmt.Sprintf("API version %s should be available for Microsoft.RedHatOpenShift/hcpOpenShiftClusters by 2026-06-30 00:00 UTC", apiVersion))
					}
			...
			err = tc.CreateHCPCluster20260531FromParam(ctx,
				GinkgoLogr,
				*resourceGroup.Name,
				clusterParams,
				nil,
