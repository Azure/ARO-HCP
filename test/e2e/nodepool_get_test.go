...
var _ = Describe("Get HCPOpenShiftCluster nodepool", func() {
	var (
		...
		nodePoolOptions *hcpsdk20251223preview.NodePoolsClientGetOptions
		...
		It("Get each nodepool from HCPOpenShiftCluster", labels.RequireHappyPathInfra, labels.Medium, labels.Positive, labels.SetupValidation, func(ctx context.Context) {
			...
			if nodePools != nil {
				...
				for np := range nps {
					...
					clusterNodePool, err := tc.Get20251223ClientFactoryOrDie(ctx).NewNodePoolsClient().Get(ctx, customerEnv.CustomerRGName, clusterEnv.Name, nps[np].Name, nodePoolOptions)
					Expect(err).To(BeNil(), "failed to get nodepool %q for cluster %q", nps[np].Name, clusterEnv.Name)
					...
					Expect(string(*clusterNodePool.Name)).To(Equal(nps[np].Name), "expected nodepool name to equal %s", nps[np].Name)
					Expect(string(*clusterNodePool.Properties.ProvisioningState)).To(Equal("Succeeded"), "expected nodepool %s provisioning state to be Succeeded", nps[np].Name)
				}
			}
		})
	})
