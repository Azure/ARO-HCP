	...
	It("should persist nodeSshPublicKeys set at cluster creation and return them via ARM GET",
		...
		func(ctx context.Context) {
			...
			for i, key := range sshPublicKeys {
				...
				Expect(cluster.Properties.NodeSSHPublicKeys[i].Key).ToNot(BeNil(),
					"cluster %q Properties.NodeSSHPublicKeys[%d].Key was nil", customerClusterName, i)
				Expect(*cluster.Properties.NodeSSHPublicKeys[i].Key).To(Equal(*key),
