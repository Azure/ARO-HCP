...
var _ = Describe("Customer", func() {
	It("should be able to create a cluster with ACR pull via managed identity and pull from a private ACR",
		...
		func(ctx context.Context) {
			...
			if !apiAvailable {
				if time.Now().After(framework.V20261001PreviewDeploymentDeadline) {
					Fail(fmt.Sprintf("API version %s should be fully available by %s", apiVersion, framework.V20261001PreviewDeploymentDeadline.Format(time.RFC3339)))
				}
			...
			updateResp, err := framework.UpdateHCPCluster20261001(
				ctx,
				hcpClient,
				*resourceGroup.Name,
				customerClusterName,
				hcpsdk20261001preview.HcpOpenShiftClusterUpdate{
					Properties: &hcpsdk20261001preview.HcpOpenShiftClusterPropertiesUpdate{
						Platform: &hcpsdk20261001preview.PlatformProfileUpdate{
			...
			Expect(*updateResp.Properties.ProvisioningState).To(
				Equal(hcpsdk20261001preview.ProvisioningStateSucceeded),
			...
			By("deleting the first pull test pod so the next check can only observe the post-update pull")
			err = kubeClient.CoreV1().Pods(pullTestNamespace).Delete(ctx, "acr-pull-test", metav1.DeleteOptions{})
			Expect(err).NotTo(HaveOccurred(), "failed to delete pod acr-pull-test")
