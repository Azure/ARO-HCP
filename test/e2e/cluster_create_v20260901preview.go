...
var _ = Describe("Customer", func() {
	...
	It("should create a cluster using v20260901preview API and verify cluster health",
		...
		func(ctx context.Context) {
			...
			resourceGroup, err := tc.NewResourceGroup(ctx, "v20260901preview", tc.Location())
			Expect(err).NotTo(HaveOccurred(), "failed to create resource group for v20260901preview test")
			...
			Expect(err).NotTo(HaveOccurred(), "failed to create customer resources for v20260901preview cluster")

			By("creating the HCP cluster via v20260901preview")
			...
			Expect(err).NotTo(HaveOccurred(), "failed to create node pool %q for v20260901preview cluster %q",
			...
			Expect(err).NotTo(HaveOccurred(), "failed to get admin REST config for v20260901preview cluster %q", customerClusterName)

			By("verifying the cluster is viable")
			err = verifiers.VerifyHCPCluster(ctx, adminRESTConfig)
			Expect(err).NotTo(HaveOccurred(), "failed to verify cluster health for v20260901preview cluster %q", customerClusterName)

			By("verifying a simple web app can run")
			err = verifiers.VerifySimpleWebApp().Verify(ctx, adminRESTConfig)
			Expect(err).NotTo(HaveOccurred(), "failed to verify simple web app runs on v20260901preview cluster %q", customerClusterName)
		},
	)
})
