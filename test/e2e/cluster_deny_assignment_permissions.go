...
var _ = Describe("Deny Assignment Permissions", func() {
	...
	It("should allow all expected customer permissions on the managed resource group",
		...
		func(ctx context.Context) {
			...
			// Use the framework helper rather than calling the SDK directly: it retries the
			// transient 409 OperationNotAllowed Azure returns while the VM is busy, while
			// still surfacing a deny-assignment 403 immediately.
			consoleLog, err := framework.GetVirtualMachineConsoleLog(ctx, computeFactory, managedResourceGroupName, *vms[0].Name)
			...
			Expect(consoleLog.Close()).To(Succeed(), "failed to close console log reader for VM %q", *vms[0].Name)
