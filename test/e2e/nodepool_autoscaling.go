...
var _ = Describe("Customer", func() {
	...
	It("should be able to create a cluster with default autoscaling and a nodepool with autoscaling enabled up to replica limits",
		...
		func(ctx context.Context) {
			...
			clusterResp, err := tc.Get20251223ClientFactoryOrDie(ctx).NewHcpOpenShiftClustersClient().Get(ctx,
				*resourceGroup.Name,
				customerClusterName,
				nil)
			...
			if !hasAZ {
				...
				azNodePoolParams := framework.NewDefaultNodePoolParams20251223()
				...
				azNodePoolResp, err := framework.GetNodePool20251223(ctx,
					nodePoolsClient,
					*resourceGroup.Name,
					customerClusterName,
					azNodePoolName)
				...
				_, err = framework.UpdateNodePoolAndWait20251223(ctx,
					...
					*resourceGroup.Name,
					customerClusterName,
					azNodePoolName,
					hcpsdk20251223preview.NodePoolUpdate{
				...
				azNodePoolUpdatedResp, err := framework.GetNodePool20251223(ctx,
					nodePoolsClient,
					*resourceGroup.Name,
					customerClusterName,
					azNodePoolName)
				Expect(err).NotTo(HaveOccurred(), "failed to get updated AZ nodepool %s", azNodePoolName)
			...
			noAZNodePoolParams := framework.NewDefaultNodePoolParams20251223()
			...
			noAZNodePoolResp, err := framework.GetNodePool20251223(ctx,
				nodePoolsClient,
				*resourceGroup.Name,
				customerClusterName,
				noAZNodePoolName)
			...
			nodes, err := kubeClient.CoreV1().Nodes().List(ctx, metav1.ListOptions{})
			Expect(err).NotTo(HaveOccurred(), "failed to list nodes after no-AZ nodepool creation")
