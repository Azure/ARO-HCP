...
var _ = Describe("Nodepool OS Disk Encryption", func() {
	It("should create a nodepool with customer-managed disk encryption via DES",
		...
		func(ctx context.Context) {
			...
			registered, err := tc.IsSubscriptionFeatureRegistered(ctx, metadataapi.FeatureExperimentalReleaseFeatures)
			Expect(err).NotTo(HaveOccurred(), "failed to check ExperimentalReleaseFeatures AFEC registration")
			if !registered {
				Skip("skipping: ExperimentalReleaseFeatures AFEC not registered on subscription")
			}
