...
var _ = Describe("Customer", func() {
	...
	It("should be able to create several HCP clusters in their customer resource group, but not in the same managed resource group",
		...
		func(ctx context.Context) {
			...
			By("creating the first cluster")
			err = tc.CreateHCPClusterFromParam(ctx, *customerResourceGroup.Name, clusterParams1, 45*time.Minute)
			Expect(err).NotTo(HaveOccurred())
			...
			By("creating the second cluster with a different managed resource group")
			err = tc.CreateHCPClusterFromParam(ctx, *customerResourceGroup.Name, clusterParams2, 45*time.Minute)
			Expect(err).NotTo(HaveOccurred())
			...
			err = tc.CreateHCPClusterFromParam(ctx, *customerResourceGroup.Name, clusterParams3, 45*time.Minute)
