...
func toIdentityUpdate(identity *hcpsdk20251223preview.ManagedServiceIdentity) *hcpsdk20251223preview.AzureResourceManagerCommonTypesManagedServiceIdentityUpdate {
	if identity == nil {
		return nil
	}
	return &hcpsdk20251223preview.AzureResourceManagerCommonTypesManagedServiceIdentityUpdate{
		Type:                   identity.Type,
	...
	Context("Negative", func() {
		It("creates a cluster and fails to update its name with a PATCH request",
			...
			func(ctx context.Context) {
				...
				_, err = framework.UpdateHCPCluster20251223(
					ctx,
	...
	Context("Positive", func() {
		It("creates a cluster and updates tags with a PATCH request",
			...
			func(ctx context.Context) {
				...
				resp, err := framework.UpdateHCPCluster20251223(
					ctx,
					tc.Get20251223ClientFactoryOrDie(ctx).NewHcpOpenShiftClustersClient(),
					*resourceGroup.Name,
					clusterName,
				...
				respGet, err := tc.Get20251223ClientFactoryOrDie(ctx).NewHcpOpenShiftClustersClient().Get(ctx, *resourceGroup.Name, clusterName, nil)
