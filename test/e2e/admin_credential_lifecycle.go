		...
		func(ctx context.Context) {
			...
			Eventually(func() error {
				...
				if cluster.Properties.ProvisioningState != nil {
					...
					GinkgoWriter.Printf("Cluster found with provisioning state: %s\n", state)

					// If cluster is still deploying and we haven't tested yet, test admin credentials
					if !testedWhileDeploying && (state == "Accepted" || state == "InProgress" || state == "Creating") {
						...
						return fmt.Errorf("cluster still deploying (state: %s), continuing to wait", state)
					}
