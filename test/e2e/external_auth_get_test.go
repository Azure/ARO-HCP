...
var _ = Describe("Get HCPOpenShiftCluster ExternalAuth", func() {
	var (
	...
	Context("Positive", func() {
		It("Get each nodepool from HCPOpenShiftCluster",
			labels.RequireHappyPathInfra, labels.Medium, labels.Positive, labels.SetupValidation,
			func(ctx context.Context) {
				if nodePools != nil {
					nps := *nodePools
					for _, np := range nps {
						By("Send get request for nodepool")
						clusterNodePool, err := NodePoolsClient.Get(ctx, customerEnv.CustomerRGName, clusterEnv.Name, np.Name, nodePoolOptions)
						Expect(err).To(BeNil())
						Expect(clusterNodePool).ToNot(BeNil())

						By("Check to see nodepool exists and is successfully provisioned")
						Expect(string(*clusterNodePool.Name)).To(Equal(np.Name))
