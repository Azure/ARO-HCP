...
var _ = Describe("Customer", func() {
	...
	It("should be able to create an HCP cluster with a customer managed KMS key for etcd",
		...
		func(ctx context.Context) {
			...
			By("creating the hcp cluster with a customer managed kms key for etcd")
			managedResourceGroupName := framework.SuffixName(*resourceGroup.Name, "-managed", 64)
			_, err = framework.CreateBicepTemplateAndWait(ctx,
				tc.GetARMResourcesClientFactoryOrDie(ctx).NewDeploymentsClient(),
				*resourceGroup.Name,
				"hcp-cluster",
				framework.Must(TestArtifactsFS.ReadFile("test-artifacts/generated-test-artifacts/image-registry/cmk-etcd-cluster.json")),
				map[string]interface{}{
