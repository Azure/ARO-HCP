...
var _ = Describe("ARO HCP Service", func() {
	It("should successfully provision cluster when MI role assignments are added after cluster creation",
		...
		func(ctx context.Context) {
			...
			clusterParams, err = framework.PopulateClusterParamsFromCustomerInfraDeployment(clusterParams, customerInfraResult)
			...
			Eventually(func(g Gomega) {
				...
				if err != nil {
					...
					g.Expect(err).NotTo(HaveOccurred(), "GET cluster returned unexpected non-404 error")
				}
			...
			Consistently(func(g Gomega) {
				...
				if err != nil {
					var respErr *azcore.ResponseError
					if errors.As(err, &respErr) && respErr.StatusCode == http.StatusNotFound {
						g.Expect(err).NotTo(HaveOccurred(), "cluster returned 404 — disappeared unexpectedly")
					}
					GinkgoLogr.Info("GET cluster returned non-404 error, skipping poll iteration", "error", err)
					return
				}
			...
			Eventually(func(g Gomega) {
				...
				if err != nil {
					var respErr *azcore.ResponseError
					if errors.As(err, &respErr) && respErr.StatusCode == http.StatusNotFound {
						g.Expect(err).NotTo(HaveOccurred(), "cluster returned 404 — disappeared after role assignment deployment")
					}
					GinkgoLogr.Info("GET cluster returned non-404 error, retrying", "error", err)
					g.Expect(err).NotTo(HaveOccurred(), "failed to GET the HCP cluster resource")
				}
				g.Expect(resp.Properties).NotTo(BeNil(), "cluster response has nil Properties")
				g.Expect(resp.Properties.ProvisioningState).NotTo(BeNil(), "cluster response has nil ProvisioningState")
				state := *resp.Properties.ProvisioningState
