...
// Proves that customer NSG Deny rules that block worker bootstrap paths leave a
// node pool stuck Provisioning (never Succeeded):
//  1. outbound Any → worker subnet (kubelet → KAS ILB path)
//  2. inbound worker → vnet-integration/Swift subnet (private-router / KAS / ignition / konnectivity)
//
// After both Deny rules are removed, a new node pool create should enter
// Provisioning (positive control that the cluster can still accept node pools).
var _ = Describe("Customer", func() {
	It("should leave node pool stuck provisioning when NSG denies Any→worker or worker→Swift paths",
		...
		func(ctx context.Context) {
			...
			By("Case 1: removing outbound Deny rule before Case 2")
			...
			By("removing inbound Deny rule before recovery create")
