...
var _ = Describe("Get HCPOpenShiftCluster", func() {
	...
	Context("Positive", func() {
		It("Confirms cluster has been created successfully", labels.RequireHappyPathInfra, labels.Medium, labels.Positive, labels.SetupValidation, func(ctx context.Context) {
			...
			out, err := tc.Get20260901ClientFactoryOrDie(ctx).NewHcpOpenShiftClustersClient().Get(ctx, customerEnv.CustomerRGName, clusterInfo.Name, nil)
			Expect(err).To(BeNil(), "failed to get HCP cluster %q", clusterInfo.Name)
			Expect(string(*out.Properties.ProvisioningState)).To(Equal(("Succeeded")), "cluster %q provisioning state should be Succeeded", clusterInfo.Name)
		})
	})
	...
	Context("Negative", func() {
		It("Fails to get a nonexistent cluster with a Not Found error", labels.RequireHappyPathInfra, labels.Medium, labels.Negative, func(ctx context.Context) {
			...
			_, err := tc.Get20260901ClientFactoryOrDie(ctx).NewHcpOpenShiftClustersClient().Get(ctx, customerEnv.CustomerRGName, clusterName, nil)
			Expect(err).ToNot(BeNil(), "expected error when getting nonexistent cluster %q", clusterName)
			...
			Expect(strings.ToLower(err.Error())).To(ContainSubstring(strings.ToLower(errMessage)), "error should indicate cluster %q was not found", clusterName)
		})
	})
})
