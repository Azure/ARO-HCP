...
* **Polling:** Pass `verifiers.WithPolling(timeout, interval)` to verifier constructors when a condition may take time; omit poll options for a single-shot `Verify`. This is the only poll option for tests — do not call internal `maybePoll`. Use `verifiers.DefaultPollInterval` when the test does not need a custom interval. Reuse `verifiers.VerifyDaemonSetReady(namespace, name, ...)` for any DaemonSet readiness check; `VerifyGlobalPullSecretSyncer` is a thin alias for the syncer in kube-system.
...
5. **Polling inside verifiers:** Use `verifiers.WithPolling(timeout, interval)` on verifier constructors so delta-only logging and elapsed-time reporting live in `test/util/verifiers/poll.go`. When polling completes, verifiers must log actual wall-clock duration (success and timeout) via `GinkgoLogr` (`elapsed` field) and `GinkgoWriter` (`[VerifierName] succeeded after 1m23s`). Tests should call `Expect(verifier.Verify(ctx, cfg)).NotTo(HaveOccurred(), "intent message")` — the annotation describes what the test was trying to do; the returned error carries the proximal failure (see **Descriptive error checks** above). Do not add test-level `EventuallyVerify`-style wrappers that bypass `VerifyHCPCluster` parallelism.
