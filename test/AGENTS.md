...
* **Polling:** Pass `verifiers.WithWait(timeout, interval)` to verifier constructors when a condition may take time; omit wait options for a single-shot `Verify`. Polling runs inside each verifier's `Verify` method via shared helpers in `poll.go`. Use `verifiers.DefaultPollInterval` when the test does not need a custom interval. Reuse `verifiers.VerifyDaemonSetReady(namespace, name, ...)` for any DaemonSet readiness check; `VerifyGlobalPullSecretSyncer` is a thin alias for the syncer in kube-system.
