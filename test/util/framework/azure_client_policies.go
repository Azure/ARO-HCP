...
func (p *lroPollerRetryPolicy) Do(req *policy.Request) (*http.Response, error) {
	...
	err := wait.ExponentialBackoffWithContext(req.Raw().Context(), p.deploymentNotFoundBackoff, func(ctx context.Context) (bool, error) {
		...
		resp, requestErr = p.doWithUnauthorizedRetry(ctx, req, &authRetried)
		if retryReasonForResponse(resp, requestErr) != retryDeploymentNotFound {
			if requestErr != nil {
...
func (p *lroPollerRetryPolicy) doWithUnauthorizedRetry(ctx context.Context, req *policy.Request, authRetried *bool) (*http.Response, error) {
	var resp *http.Response
	var lastRetryableErr error

	err := wait.ExponentialBackoffWithContext(ctx, p.unauthorizedBackoff, func(ctx context.Context) (bool, error) {
		if resp != nil && resp.Body != nil {
			if err := resp.Body.Close(); err != nil {
				ginkgo.GinkgoLogr.Error(err, "failed to close response body")
			}
		...
		if retryReasonForResponse(resp, requestErr) != retryUnauthorized || *authRetried {
			if requestErr != nil {
				return false, requestErr
			}
			return true, nil
		...
		*authRetried = true
		lastRetryableErr = requestErr
		ginkgo.GinkgoLogr.Info("transient 401 Unauthorized on operationStatuses, retrying with a refreshed token",
			"url", req.Raw().URL.String())
		return false, nil
	})
	if err == nil {
