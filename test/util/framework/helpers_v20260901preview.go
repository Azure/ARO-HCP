...
// CSR-based 2026-09-01-preview API version. Used when the legacy 20260630
// admin credential path fails, so tests keep working once break-glass
// issuance is no longer served for the stable 20260630 API version. Both the
// originating 20260630 error and any 2026-09-01-preview error are wrapped so a
...
func (tc *perItOrDescribeTestContext) fallbackAdminRESTConfigTo20260901(
	...
	if fallbackErr != nil {
		return nil, fmt.Errorf("20260630 credential request failed: %w; fallback client factory error: %w", causeErr, fallbackErr)
	}
	...
	if fallbackErr != nil {
		return nil, fmt.Errorf("20260630 credential request failed: %w; 2026-09-01-preview fallback failed: %w", causeErr, fallbackErr)
	}
