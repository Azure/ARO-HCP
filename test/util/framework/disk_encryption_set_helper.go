...
func (tc *perItOrDescribeTestContext) CreateDiskEncryptionSet(ctx context.Context, resourceGroupName, keyVaultName, clusterName, serviceMIPrincipalID, location string) (string, error) {
	startTime := time.Now()
	defer func() {
	...
	if serviceMIPrincipalID != "" {
		...
		desReaderAssignmentName := guid(desResourceID, serviceMIPrincipalID, readerRoleDefID)
		...
		if err != nil {
			return "", fmt.Errorf("failed to assign Reader to SMI on DES: %w", err)
		}
		tc.trackRoleAssignment(*desReaderResult.ID)
		ginkgo.GinkgoLogr.Info("Assigned Reader to SMI on DES", "scope", desResourceID, "principalID", serviceMIPrincipalID)
	}
