...
type NodePoolParams20261001 struct {
	...
	VMSize                 string
	OSDiskSizeGiB          int32
	DiskType               hcpsdk20261001.OsDiskType
...
func ConvertToUserAssignedIdentitiesProfile20261001(value interface{}) (*hcpsdk20261001.UserAssignedIdentitiesProfile, error) {
	if value == nil {
		return nil, nil
	}
	...
	var uamis hcpsdk20261001.UserAssignedIdentitiesProfile
...
func ConvertToManagedServiceIdentity20261001(value interface{}) (*hcpsdk20261001.ManagedServiceIdentity, error) {
	if value == nil {
		return nil, nil
	}
	b, err := json.Marshal(value)
	...
	var msi hcpsdk20261001.ManagedServiceIdentity
...
func (tc *perItOrDescribeTestContext) CreateHCPClusterFromParam20261001(
	...
	imageDigestMirrors []*hcpsdk20261001.ImageDigestMirror,
	...
	defer func() {
		finishTime := time.Now()
		tc.RecordTestStep(fmt.Sprintf("Deploy HCP cluster %s/%s (v20261001)", resourceGroupName, clusterName), startTime, finishTime)
	}()
...
func BuildHCPClusterFromParams20261001(
	...
	location string,
	imageDigestMirrors []*hcpsdk20261001.ImageDigestMirror,
) (hcpsdk20261001.HcpOpenShiftCluster, error) {
	// Convert identity types from old SDK to new SDK via JSON round-trip
	var identity *hcpsdk20261001.ManagedServiceIdentity
	if parameters.Identity != nil {
		var err error
		identity, err = convertViaJSON[hcpsdk20261001.ManagedServiceIdentity](parameters.Identity)
		if err != nil {
			return hcpsdk20261001.HcpOpenShiftCluster{}, fmt.Errorf("failed to convert Identity: %w", err)
		}
	}
	...
	if parameters.UserAssignedIdentitiesProfile != nil {
		var err error
		uamis, err = convertViaJSON[hcpsdk20261001.UserAssignedIdentitiesProfile](parameters.UserAssignedIdentitiesProfile)
		if err != nil {
			return hcpsdk20261001.HcpOpenShiftCluster{}, fmt.Errorf("failed to convert UserAssignedIdentitiesProfile: %w", err)
		}
	}
	...
	return hcpsdk20261001.HcpOpenShiftCluster{
		...
		Properties: &hcpsdk20261001.HcpOpenShiftClusterProperties{
			...
			Network: &hcpsdk20261001.NetworkProfile{
				NetworkType: to.Ptr(hcpsdk20261001.NetworkType(parameters.Network.NetworkType)),
			...
			API: &hcpsdk20261001.APIProfile{
				Visibility:      to.Ptr(hcpsdk20261001.Visibility(parameters.APIVisibility)),
				AuthorizedCIDRs: parameters.AuthorizedCIDRs,
			},
			Ingress: &hcpsdk20261001.IngressProfile{
				Type: to.Ptr(hcpsdk20261001.IngressType(parameters.IngressType)),
			},
			ClusterImageRegistry: &hcpsdk20261001.ClusterImageRegistryProfile{
				State: to.Ptr(hcpsdk20261001.ClusterImageRegistryState(parameters.ImageRegistryState)),
			},
...
func CreateHCPClusterAndWait20261001(
	ctx context.Context,
	logger logr.Logger,
	hcpClient *hcpsdk20261001.HcpOpenShiftClustersClient,
	resourceGroupName string,
	hcpClusterName string,
	...
	logger.Info("Starting HCP cluster creation (v20261001)", "clusterName", hcpClusterName, "resourceGroup", resourceGroupName)
	...
	if timeout > 0*time.Second {
		...
		case hcpsdk20261001.HcpOpenShiftClustersClientCreateOrUpdateResponse:
...
func UpdateHCPCluster20261001(
	...
	resourceGroupName string,
	hcpClusterName string,
	update hcpsdk20261001.HcpOpenShiftCluster,
	timeout time.Duration,
) (*hcpsdk20261001.HcpOpenShiftCluster, error) {
	...
	var hcpOpenShiftCluster *hcpsdk20261001.HcpOpenShiftCluster
...
func BuildNodePoolFromParams20261001(
	parameters NodePoolParams20261001,
	location string,
) hcpsdk20261001.NodePool {

	nodePool := hcpsdk20261001.NodePool{
		Location: to.Ptr(location),
		Tags:     parameters.Tags,
		Properties: &hcpsdk20261001.NodePoolProperties{
			Version: &hcpsdk20261001.NodePoolVersionProfile{
				ID:           to.Ptr(parameters.OpenshiftVersionId),
				ChannelGroup: to.Ptr(parameters.ChannelGroup),
			},
			NodeDrainTimeoutMinutes: parameters.NodeDrainTimeoutMinutes,
			Platform: &hcpsdk20261001.NodePoolPlatformProfile{
				VMSize: to.Ptr(parameters.VMSize),
				OSDisk: &hcpsdk20261001.OsDiskProfile{
					SizeGiB:                to.Ptr(parameters.OSDiskSizeGiB),
					DiskStorageAccountType: to.Ptr(hcpsdk20261001.DiskStorageAccountType(parameters.DiskStorageAccountType)),
...
func CreateNodePoolAndWait20261001(
	ctx context.Context,
	nodePoolsClient *hcpsdk20261001.NodePoolsClient,
	resourceGroupName string,
	hcpClusterName string,
	nodePoolName string,
	nodePool hcpsdk20261001.NodePool,
	timeout time.Duration,
) (*hcpsdk20261001.NodePool, error) {
...
func (tc *perItOrDescribeTestContext) Get20261001ClientFactory(ctx context.Context) (*hcpsdk20261001.ClientFactory, error) {
...
func (tc *perItOrDescribeTestContext) Get20261001ClientFactoryOrDie(ctx context.Context) *hcpsdk20261001.ClientFactory {
	return Must(tc.Get20261001ClientFactory(ctx))
}

func (tc *perItOrDescribeTestContext) get20261001ClientFactoryUnlocked(ctx context.Context) (*hcpsdk20261001.ClientFactory, error) {
	if tc.clientFactory20261001 != nil {
		return tc.clientFactory20261001, nil
	}
	...
	clientFactory, err := hcpsdk20261001.NewClientFactory(subscriptionID, creds, tc.perBinaryInvocationTestContext.getHCPClientFactoryOptions())
	if err != nil {
		return nil, err
	}
...
func (tc *perItOrDescribeTestContext) GetAdminRESTConfigForHCPCluster20261001(
	...
	adminCredentialRequestPoller, err := hcpClient.BeginRequestAdminCredential(
		...
		resourceGroupName,
		hcpClusterName,
		hcpsdk20261001.HcpOpenShiftClusterAdminCredentialRequest{
