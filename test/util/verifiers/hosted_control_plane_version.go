...
func (v verifyHostedControlPlaneZStreamUpgradeOnly) Verify(ctx context.Context, adminRESTConfig *rest.Config) error {
	...
	if len(clusterVersion.Status.History) < 2 {
		// TEMP: testing to see if StopTrying works (should terminate the test early without retrying)
		return gomega.StopTrying(fmt.Sprintf("unexpected number of items in cluster version history. Expected 1 or greater but got %d", len(clusterVersion.Status.History)))
	}
	...
	// note that clusterVersion.Status.History has the latest version first and the install version last
	for i, history := range clusterVersion.Status.History {
		...
		if historyVersion.Major != initialSemver.Major || historyVersion.Minor != initialSemver.Minor {
			return fmt.Errorf("version %q in clusterversion history has different major.minor than initial %q (expected %d.%d.x)",
				history.Version, v.initialVersion, initialSemver.Major, initialSemver.Minor)
		}

		if i > 0 {
			// "prev" here means previously visited indexed
			prevHistoryVersion, err := semver.ParseTolerant(clusterVersion.Status.History[i-1].Version)
			if err != nil {
				return fmt.Errorf("parse version %q in history: %w", history.Version, err)
			}

			// versions at earlier indices must be larger than versions at later indices
			if prevHistoryVersion.LTE(historyVersion) {
				historyVersionStr := updateHistoryToString(clusterVersion.Status.History)

				return fmt.Errorf(
					"downgrade unexpected: version %q is less than or equal to %q. Cluster history: %s. Error at history index %d and %d",
					history.Version,
...
func updateHistoryToString(updateHistory []configv1.UpdateHistory) string {
	var historyVersionSb strings.Builder
	historyVersionSb.WriteString("[")
	for i, history := range updateHistory {
		historyVersionSb.WriteString(history.Version)

		if i != len(updateHistory)-1 {
			historyVersionSb.WriteString(", ")
		}
	}
	historyVersionSb.WriteString("]")

	return historyVersionSb.String()
}
