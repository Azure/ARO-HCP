...
func (v verifyHostedControlPlaneZStreamUpgradeOnly) Verify(ctx context.Context, adminRESTConfig *rest.Config) error {
	...
	if len(clusterVersion.Status.History) < 1 {
		return fmt.Errorf("unexpected number of items in cluster version history. Expected 1 or greater but got %d",
			len(clusterVersion.Status.History),
		)
	}
	...
	for i, history := range clusterVersion.Status.History {
		...
		if i > 0 {
			...
			if prevHistoryVersion.LTE(historyVersion) {
				...
				return gomega.StopTrying(
					fmt.Sprintf(
						"downgrade unexpected: cluster went from version %q and unexpectedly downgraded to version %q. clusterVersion history: %s. Error when version at index %d downgraded to version at index %d",
						history.Version,
