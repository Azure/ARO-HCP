...
3. Open a PR. See [PR preview](../../../../docs/grafana-dashboards.md#pr-preview).
4. This folder is registered in [`observability/observability.yaml`](../../../observability.yaml) as `SRE Cluster Health`.
5. The `GrafanaDashboards` EV2 step deploys all JSON files in this folder to the `SRE Cluster Health` Grafana folder in INT, STG, and PROD.
6. No manual upload is needed — merge to `main` and the pipeline handles deployment.
