...
get_existing_cluster_payload() {
  EXISTING_CLUSTER_PAYLOAD=$(curl --silent --show-error "${FRONTEND_HOST}:${FRONTEND_PORT}${CLUSTER_RESOURCE_ID}?${FRONTEND_API_VERSION_QUERY_PARAM}")
}
...
delete_cluster() {
  ...
  correlation_headers | curl --silent --show-error --include --header @- --request DELETE "${FRONTEND_HOST}:${FRONTEND_PORT}${CLUSTER_RESOURCE_ID}?${FRONTEND_API_VERSION_QUERY_PARAM}"
  ...
  while true ; do
    CLUSTER_GET_RESP=$(curl --silent --show-error "${FRONTEND_HOST}:${FRONTEND_PORT}${CLUSTER_RESOURCE_ID}?${FRONTEND_API_VERSION_QUERY_PARAM}")
