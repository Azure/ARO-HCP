...
// TestAdmitCluster_PrivateKASVersion covers the admission check that rejects
// cluster creation with private KAS (API server) visibility when the requested
// OpenShift version is below 4.22 (the minimum version whose HyperShift
// supports a private API server). Public KAS clusters are unaffected, and
// the check only fires on CREATE.
func TestAdmitCluster_PrivateKASVersion(t *testing.T) {
	t.Parallel()
	...
	makeCluster := func(versionID string, apiVisibility metadataapi.Visibility) *coreapi.HCPOpenShiftCluster {
		resourceID := metadataapi.Must(azcorearm.ParseResourceID(fmt.Sprintf(
			"/subscriptions/%s/resourceGroups/rg/providers/Microsoft.RedHatOpenShift/hcpOpenShiftClusters/test-cluster",
			subscriptionID)))
		return &coreapi.HCPOpenShiftCluster{
			TrackedResource: coreapi.NewTrackedResource(resourceID, "eastus"),
			CustomerProperties: coreapi.HCPOpenShiftClusterCustomerProperties{
				Version: coreapi.VersionProfile{ID: versionID},
				API:     coreapi.CustomerAPIProfile{Visibility: apiVisibility},
				Platform: coreapi.CustomerPlatformProfile{
					ManagedResourceGroup:   "mrg-test",
					SubnetID:               subnetID,
	...
	tests := []struct {
		...
		{
			name:         "private KAS at 4.22 is allowed",
			cluster:      makeCluster("4.22", metadataapi.VisibilityPrivate),
			expectErrors: []utils.ExpectedError{},
		},
		{
			name:         "private KAS at 4.23 is allowed",
			cluster:      makeCluster("4.23", metadataapi.VisibilityPrivate),
			expectErrors: []utils.ExpectedError{},
		},
		{
			name:         "private KAS at 5.0 is allowed",
			cluster:      makeCluster("5.0", metadataapi.VisibilityPrivate),
			expectErrors: []utils.ExpectedError{},
		},
		{
			name:    "private KAS at 4.21 is rejected",
			cluster: makeCluster("4.21", metadataapi.VisibilityPrivate),
			expectErrors: []utils.ExpectedError{
				{FieldPath: "properties.version.id", Message: "private KAS (API server) visibility requires OpenShift version 4.22.0 or above"},
			},
		},
		{
			name:    "private KAS at 4.17 is rejected",
			cluster: makeCluster("4.17", metadataapi.VisibilityPrivate),
			expectErrors: []utils.ExpectedError{
				{FieldPath: "properties.version.id", Message: "private KAS (API server) visibility requires OpenShift version 4.22.0 or above"},
			},
		},
		{
			name:         "public KAS at 4.17 is allowed",
			cluster:      makeCluster("4.17", metadataapi.VisibilityPublic),
			expectErrors: []utils.ExpectedError{},
		},
		{
			name:         "public KAS at 4.21 is allowed",
			cluster:      makeCluster("4.21", metadataapi.VisibilityPublic),
			expectErrors: []utils.ExpectedError{},
		},
		{
			name:         "public KAS at 4.22 is allowed",
			cluster:      makeCluster("4.22", metadataapi.VisibilityPublic),
			expectErrors: []utils.ExpectedError{},
		},
		{
			name:         "private KAS with exact version 4.22.3 is allowed",
			cluster:      makeCluster("4.22.3", metadataapi.VisibilityPrivate),
			expectErrors: []utils.ExpectedError{},
		},
		{
			name:    "private KAS with exact version 4.21.5 is rejected",
			cluster: makeCluster("4.21.5", metadataapi.VisibilityPrivate),
			expectErrors: []utils.ExpectedError{
				{FieldPath: "properties.version.id", Message: "private KAS (API server) visibility requires OpenShift version 4.22.0 or above"},
			},
		},
		{
			name:         "private KAS with nightly 4.22 version is allowed",
			cluster:      makeCluster("4.22.0-0.nightly-multi-2026-06-15-123456", metadataapi.VisibilityPrivate),
			expectErrors: []utils.ExpectedError{},
		},
		{
			name:         "private KAS with empty version skips check",
			cluster:      makeCluster("", metadataapi.VisibilityPrivate),
			expectErrors: []utils.ExpectedError{},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			errs := AdmitCluster(ctx, admissionContext, operation.Operation{Type: operation.Create}, tt.cluster, nil)
...
// TestAdmitClusterPrivateKASVersion_UnitLevel covers the per-function behavior
// of admitClusterPrivateKASVersion in isolation. It verifies that the function
// is a no-op on UPDATE and handles edge cases like malformed version IDs.
func TestAdmitClusterPrivateKASVersion_UnitLevel(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		name         string
		op           operation.Operation
		props        coreapi.HCPOpenShiftClusterCustomerProperties
		expectErrors []utils.ExpectedError
	}{
		{
			name: "no-op on UPDATE",
			op:   operation.Operation{Type: operation.Update},
			props: coreapi.HCPOpenShiftClusterCustomerProperties{
				Version: coreapi.VersionProfile{ID: "4.17"},
				API:     coreapi.CustomerAPIProfile{Visibility: metadataapi.VisibilityPrivate},
			},
			expectErrors: []utils.ExpectedError{},
		},
		{
			name: "no-op when visibility is not Private",
			op:   operation.Operation{Type: operation.Create},
			props: coreapi.HCPOpenShiftClusterCustomerProperties{
				Version: coreapi.VersionProfile{ID: "4.17"},
				API:     coreapi.CustomerAPIProfile{Visibility: metadataapi.VisibilityPublic},
			},
			expectErrors: []utils.ExpectedError{},
		},
		{
			name: "no-op when visibility is empty",
			op:   operation.Operation{Type: operation.Create},
			props: coreapi.HCPOpenShiftClusterCustomerProperties{
				Version: coreapi.VersionProfile{ID: "4.17"},
			},
		...
		{
			name: "malformed version ID skips check",
			op:   operation.Operation{Type: operation.Create},
			props: coreapi.HCPOpenShiftClusterCustomerProperties{
				Version: coreapi.VersionProfile{ID: "not-a-version"},
				API:     coreapi.CustomerAPIProfile{Visibility: metadataapi.VisibilityPrivate},
			},
			expectErrors: []utils.ExpectedError{},
		},
		{
			name: "private KAS at exactly 4.22 is allowed",
			op:   operation.Operation{Type: operation.Create},
			props: coreapi.HCPOpenShiftClusterCustomerProperties{
		...
		},
		{
			...
			op:   operation.Operation{Type: operation.Create},
			props: coreapi.HCPOpenShiftClusterCustomerProperties{
				Version: coreapi.VersionProfile{ID: "4.21"},
				API:     coreapi.CustomerAPIProfile{Visibility: metadataapi.VisibilityPrivate},
			},
			expectErrors: []utils.ExpectedError{
				{FieldPath: "properties.version.id", Message: "private KAS (API server) visibility requires OpenShift version 4.22.0 or above"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			errs := admitClusterPrivateKASVersion(context.Background(), nil, tt.op, fldPath, &tt.props)
