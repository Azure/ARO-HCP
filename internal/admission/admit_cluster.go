...
// cluster's active control plane version is too old. For nightly channel
// clusters, only major.minor is checked (nightlies always carry the latest
// code). For GA channels (stable, fast, candidate), the full version
// including patch must be >= 4.22.4 where the CPO change landed.
