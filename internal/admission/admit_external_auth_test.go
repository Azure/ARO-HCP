...
func TestAdmitExternalAuth(t *testing.T) {
	...
	externalAuthWithResourceID := func(resourceID string) *coreapi.ClusterExternalAuth {
		return coreapi.NewDefaultClusterExternalAuth(metadataapi.Must(azcorearm.ParseResourceID(resourceID)))
	}
	...
	tests := []struct {
		...
		},
		{
			...
			op:   operation.Create,
			admissionContext: &ExternalAuthAdmissionContext{
				ClusterExternalAuths: []*coreapi.ClusterExternalAuth{existingExternalAuth},
			},
			newObj: newExternalAuth,
			expectErrors: []utils.ExpectedError{
				{
		...
		},
		{
			...
			op:   operation.Create,
			admissionContext: &ExternalAuthAdmissionContext{
				ClusterExternalAuths: []*coreapi.ClusterExternalAuth{existingExternalAuth, otherExternalAuth},
			},
			newObj: newExternalAuth,
			expectErrors: []utils.ExpectedError{
				{
