...
func TestCopyReadOnlyValues_NodePool(t *testing.T) {
	seed := rand.Int63()
	t.Logf("seed: %d", seed)
	...
	for i := 0; i < 2000; i++ {
		...
		copyReadOnlyNodePool(t, original)
	}
}

func copyReadOnlyNodePool(t *testing.T, originalInternal *api.HCPOpenShiftClusterNodePool) {
	originalExternal := v20240610preview.NewVersion().NewHCPOpenShiftClusterNodePool(originalInternal)
	oldDestinationExternal := v20240610preview.NewVersion().NewHCPOpenShiftClusterNodePool(&api.HCPOpenShiftClusterNodePool{})
	api.CopyReadOnlyValues(originalExternal, oldDestinationExternal)

	newDestinationInternal := &api.HCPOpenShiftClusterNodePool{}
	CopyReadOnlyNodePoolValues(newDestinationInternal, originalInternal)
	newDestinationExternal := v20240610preview.NewVersion().NewHCPOpenShiftClusterNodePool(newDestinationInternal)
