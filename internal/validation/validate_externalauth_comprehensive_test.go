...
func TestValidateExternalAuth(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			newObj: func() *coreapi.ExternalAuth {
		...
		},
		{
			...
			oldObj: func() *coreapi.ExternalAuth {
				obj := createValidExternalAuth()
...
func TestValidateExternalAuthDiscriminatedUnions(t *testing.T) {
	...
	tests := []struct {
		name         string
		setupObject  func() *coreapi.ExternalAuth
		expectErrors []utils.ExpectedError
	}{
		{
...
func testExternalAuthWithClients(audiences []string, clients ...coreapi.ExternalAuthClientProfile) *coreapi.ExternalAuth {
	obj := createValidExternalAuth()
...
func createMinimalExternalAuth() *coreapi.ExternalAuth {
	...
	obj := coreapi.NewDefaultExternalAuth(resourceID)
...
func createValidExternalAuth() *coreapi.ExternalAuth {
	createdAt := time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC)
