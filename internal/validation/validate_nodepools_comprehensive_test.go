...
func TestValidateNodePoolCreate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			nodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.AutoScaling = &resourcesapi.NodePoolAutoScaling{
					Min: 1,
					Max: 5,
				}
		...
		},
		{
			...
			nodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.Taints = []resourcesapi.Taint{
					{
						Key:    "dedicated",
						Value:  "gpu",
						Effect: resourcesapi.EffectNoSchedule,
					},
					{
						Key:    "environment",
						Value:  "test",
						Effect: resourcesapi.EffectPreferNoSchedule,
					},
				}
				return np
			}(),
		...
		},
		{
			...
			nodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.Platform.OSDisk.EncryptionSetID = resourcesapi.Must(azcorearm.ParseResourceID("/subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/test-rg/providers/Microsoft.Compute/diskEncryptionSets/test-des"))
				return np
			}(),
			expectErrors: []expectedError{},
		},
		...
		},
		{
			...
			nodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.Platform.OSDisk.DiskStorageAccountType = resourcesapi.DiskStorageAccountTypeStandardSSD_LRS
				return np
			}(),
			expectErrors: []expectedError{},
		},
		...
		},
		{
			...
			nodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.Platform.SubnetID = resourcesapi.Must(azcorearm.ParseResourceID("/subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/test-rg/providers/Microsoft.Network/virtualNetworks/test-vnet"))
				return np
			}(),
			expectErrors: []expectedError{
		...
		},
		{
			...
			nodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.Platform.OSDisk.DiskType = resourcesapi.OsDiskTypeEphemeral
		...
		},
		{
			...
			nodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.Platform.OSDisk.EncryptionSetID = resourcesapi.Must(azcorearm.ParseResourceID("/subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/test-rg/providers/Microsoft.Network/virtualNetworks/test-vnet"))
				return np
			}(),
			expectErrors: []expectedError{
...
func TestValidateNodePoolUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			newNodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.Replicas = 5
				return np
			}(),
			oldNodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.Replicas = 3
				return np
			}(),
		...
		},
		{
			...
			newNodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.ProvisioningState = armresourcesapi.ProvisioningStateProvisioning
				return np
			}(),
			oldNodePool: func() *resourcesapi.HCPOpenShiftClusterNodePool {
				np := createValidNodePool()
				np.Properties.ProvisioningState = armresourcesapi.ProvisioningStateSucceeded
				return np
			}(),
			expectErrors: []expectedError{
...
func createValidNodePool() *resourcesapi.HCPOpenShiftClusterNodePool {
	nodePool := resourcesapi.NewDefaultHCPOpenShiftClusterNodePool(
		resourcesapi.Must(azcorearm.ParseResourceID("/subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/test-rg/providers/Microsoft.RedHatOpenShift/hcpOpenShiftClusters/test-cluster/nodePools/test-nodepool")),
	...
	nodePool.Properties.Platform.SubnetID = resourcesapi.Must(azcorearm.ParseResourceID("/subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/test-rg/providers/Microsoft.Network/virtualNetworks/test-vnet/subnets/test-subnet"))
	nodePool.Properties.Platform.VMSize = "Standard_D2s_v3"
	nodePool.Properties.Replicas = 3

	// Add required systemData fields
	createdAt := time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC)
	nodePool.SystemData = &armresourcesapi.SystemData{
		CreatedBy:     "test-user",
...
func TestValidateNodePoolVersionWithFeatureFlags(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			opOptions: testNodePoolFeatureOptions(resourcesapi.FeatureExperimentalReleaseFeatures),
			expectErrors: []expectedError{
				{message: "No Major.Minor.Patch elements found", fieldPath: "properties.version.id"},
			},
		},
