...
func TestValidateNodePoolUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			newNodePool: func() *coreapi.ClusterNodePool {
				np := createValidNodePool()
				np.Properties.Replicas = 5
				return np
			}(),
			oldNodePool: func() *coreapi.ClusterNodePool {
				np := createValidNodePool()
				np.Properties.Replicas = 3
				return np
			}(),
...
func createValidNodePool() *coreapi.ClusterNodePool {
	nodePool := coreapi.NewDefaultClusterNodePool(
