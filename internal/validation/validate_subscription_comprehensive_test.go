...
func TestValidateSubscriptionCreate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.State = coreapi.SubscriptionStateRegistered
				return s
			}(),
			expectErrors: []utils.ExpectedError{},
		},
		...
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				...
				s.ResourceID = metadataapi.Must(azcorearm.ParseResourceID("/subscriptions/test-sub/resourceGroups/test-rg/providers/Microsoft.RedHatOpenShift/hcpOpenShiftClusters/test-cluster"))
				return s
			}(),
			expectErrors: []utils.ExpectedError{
		...
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.ResourceID = metadataapi.Must(azcorearm.ParseResourceID("/subscriptions/invalid-uuid"))
				return s
			}(),
			expectErrors: []utils.ExpectedError{
		...
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.State = coreapi.SubscriptionState("InvalidState")
				return s
			}(),
			expectErrors: []utils.ExpectedError{
		...
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.State = coreapi.SubscriptionState("")
				return s
			}(),
			expectErrors: []utils.ExpectedError{
				{Message: "supported values:", FieldPath: "required"},
		...
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.State = coreapi.SubscriptionStateUnregistered
				return s
			}(),
			expectErrors: []utils.ExpectedError{},
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.State = coreapi.SubscriptionStateWarned
				return s
			}(),
			expectErrors: []utils.ExpectedError{},
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.State = coreapi.SubscriptionStateDeleted
				return s
			}(),
			expectErrors: []utils.ExpectedError{},
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.State = coreapi.SubscriptionStateSuspended
				return s
			}(),
			expectErrors: []utils.ExpectedError{},
		},
		// Resource naming validation tests (covering middleware_validatestatic_test.go patterns)
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.ResourceID = metadataapi.Must(azcorearm.ParseResourceID("/subscriptions/12345678-1234-1234-1234-123456789012"))
				return s
			}(),
			expectErrors: []utils.ExpectedError{},
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.ResourceID = metadataapi.Must(azcorearm.ParseResourceID("/subscriptions/123"))
				return s
			}(),
			expectErrors: []utils.ExpectedError{
				{Message: "invalid UUID length", FieldPath: "id"},
		...
		},
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.ResourceID = metadataapi.Must(azcorearm.ParseResourceID("/subscriptions/not-a-uuid-at-all"))
				return s
			}(),
			expectErrors: []utils.ExpectedError{
				{Message: "invalid UUID length", FieldPath: "id"},
		...
		// Test that resource group validation works properly
		{
			...
			subscription: func() *coreapi.Subscription {
				s := createValidSubscription()
				s.ResourceID = metadataapi.Must(azcorearm.ParseResourceID("/subscriptions/12345678-1234-1234-1234-123456789012/resourceGroups/should-not-exist"))
				return s
			}(),
			expectErrors: []utils.ExpectedError{
				{Message: "resource ID must reference an instance of type \"Microsoft.Resources/subscriptions\"", FieldPath: "id"},
