...
func TestValidateClusterCreate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectErrors: []utils.ExpectedError{
				{Message: "must be the key version only, not a full key URL", FieldPath: "customerProperties.etcd.dataEncryption.customerManaged.kms.activeKey.version"},
			},
		},
		{
			name: "invalid kms key version - https URL - create",
			cluster: func() *coreapi.HCPOpenShiftCluster {
				c := createValidCluster()
				c.CustomerProperties.Etcd.DataEncryption.KeyManagementMode = metadataapi.EtcdDataEncryptionKeyManagementModeTypeCustomerManaged
				c.CustomerProperties.Etcd.DataEncryption.CustomerManaged = &coreapi.CustomerManagedEncryptionProfile{
					...
					Kms: &coreapi.KmsEncryptionProfile{
						...
						ActiveKey: coreapi.KmsKey{
							Name:      "test-key",
							VaultName: "test-vault",
							Version:   "https://vault.azure.net/keys/name/version",
						},
					},
				}
				return c
		...
		{
			name: "invalid kms key version - http URL - create",
			cluster: func() *coreapi.HCPOpenShiftCluster {
				c := createValidCluster()
				c.CustomerProperties.Etcd.DataEncryption.KeyManagementMode = metadataapi.EtcdDataEncryptionKeyManagementModeTypeCustomerManaged
				c.CustomerProperties.Etcd.DataEncryption.CustomerManaged = &coreapi.CustomerManagedEncryptionProfile{
					...
					Kms: &coreapi.KmsEncryptionProfile{
						...
						ActiveKey: coreapi.KmsKey{
							Name:      "test-key",
							VaultName: "test-vault",
							Version:   "http://example.com/version",
						},
					},
				}
				return c
