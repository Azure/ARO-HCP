...
func TestValidateContainerRegistryProfile(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectErrors: []utils.ExpectedError{
				{FieldPath: "platform.containerRegistry.credentials.managedIdentity.resourceId", Message: "must not be the same resource group name"},
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			errs := validateContainerRegistryProfile(ctx, op, fldPath, tt.newObj, nil, "managedRg")
			utils.VerifyErrorsMatch(t, tt.expectErrors, errs)
		})
	}
}
