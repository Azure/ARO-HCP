...
func TestExternalAuthRequired(t *testing.T) {
	tests := []struct {
		name         string
		resource     *coreapi.ClusterExternalAuth
		...
		},
		{
			...
			resource: coreapi.NewDefaultClusterExternalAuth(metadataapi.Must(azcorearm.ParseResourceID("/subscriptions/test-sub/resourceGroups/test-rg/providers/Microsoft.RedHatOpenShift/hcpOpenShiftClusters/test-cluster/externalAuths/test-auth"))),
			expectErrors: []utils.ExpectedError{
				{
					Message:   "Required value",
					FieldPath: "trackedResource.resource.systemData",
...
func TestExternalAuthValidate(t *testing.T) {
	...
	tests := []struct {
		name         string
		tweaks       *coreapi.ClusterExternalAuth
		expectErrors []utils.ExpectedError
	}{
		{
			name:   "Minimum valid external auth",
