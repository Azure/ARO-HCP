...
func TestRetriableStatusCodes(t *testing.T) {
	tests := []struct {
		...
		{"400 on POST retries (eventual consistency)", http.MethodPost, http.StatusBadRequest, true},
		{"400 on GET does not retry", http.MethodGet, http.StatusBadRequest, false},
