...
func FuzzArmResourceID(resourceType string, genName func() string) *azcorearm.ResourceID {
	...
	return api.Must(azcorearm.ParseResourceID(b.String()))
}
...
func FuzzInternalID(genName func() string) api.InternalID {
	return api.Must(api.NewInternalID("/api/clusters_mgmt/v1/clusters/" + sanitizeFuzzName(genName())))
}
...
func CommonRoundTripFuzzFuncs() []interface{} {
	return append(CommonDeepCopyFuzzFuncs(),
		...
		func(j *api.CosmosMetadata, c randfill.Continue) {
			c.FillNoCustom(j)
			// ResourceID is synchronized with arm.Resource.ID after Fill.
			j.ResourceID = nil
			j.ExistingCosmosUID = ""
			j.CosmosETag = ""
			j.InstanceVersion = 0
...
func CommonDeepCopyFuzzFuncs() []interface{} {
	return []interface{}{
		...
		func(j *api.InternalID, c randfill.Continue) {
		...
		func(j *api.NodePoolPlatformProfile, c randfill.Continue) {
			c.FillNoCustom(j)
			if j.SubnetID != nil {
				j.SubnetID = FuzzArmResourceID("Microsoft.Network/virtualNetworks/subnets", GenName(c))
			}
		...
		func(j *api.HCPOpenShiftClusterServiceProviderProperties, c randfill.Continue) {
			...
			j.ClusterServiceID = api.Ptr(FuzzInternalID(GenName(c)))
			j.PendingClusterServiceID = api.Ptr(FuzzInternalID(GenName(c)))
		},
		func(j *api.HCPOpenShiftClusterNodePoolServiceProviderProperties, c randfill.Continue) {
			c.FillNoCustom(j)
			if j == nil {
		...
		func(j *api.Operation, c randfill.Continue) {
			c.FillNoCustom(j)
			if j == nil {
				return
			}
