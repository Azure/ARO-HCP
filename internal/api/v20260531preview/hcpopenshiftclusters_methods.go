...
func newConditions(from []api.Condition) []*generated.Condition {
	if from == nil {
		return nil
	}
	out := make([]*generated.Condition, 0, len(from))
	for _, c := range from {
		out = append(out, &generated.Condition{
			Type:               api.PtrOrNil(generated.ConditionType(c.Type)),
			Status:             api.PtrOrNil(generated.StatusType(c.Status)),
			LastTransitionTime: api.PtrOrNil(c.LastTransitionTime),
			Reason:             api.PtrOrNil(c.Reason),
			Message:            api.PtrOrNil(c.Message),
		})
	}
	return out
}
	...
	out := &HcpOpenShiftCluster{
		generated.HcpOpenShiftCluster{
			...
			Properties: &generated.HcpOpenShiftClusterProperties{
				...
				Conditions:              newConditions(from.ServiceProviderProperties.Conditions),
			},
...
func (c *HcpOpenShiftCluster) ConvertToInternal(existing *api.HCPOpenShiftCluster) (*api.HCPOpenShiftCluster, error) {
	...
	if c.Properties != nil {
		...
		// They are preserved via CopyReadOnlyClusterValues.
	}
