...
type ServiceProviderClusterStatus struct {
	...
	// marks identities that have left that data-plane set as PendingDeconfigure
	// (stamping DeconfigureTimestamp when that was requested so Azure FIC deletes
	// wait 24 hours on a live cluster). DataPlaneOIDCFederation then creates or
	// deletes federated identity credentials in Azure (one per data-plane operator
