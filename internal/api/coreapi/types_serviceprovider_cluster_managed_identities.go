...
// the slot/generation/instance/binding model and the configure/replace/deconfigure
// flow.
//
// Managed identity condition types used on ServiceProviderCluster.Status.ManagedIdentities
// (subsystem, slot, generation, instance, and binding). Top-level
...
const (
	...
	// On a control-plane or data-plane instance: the generation is Configured
	// and every type-particular configure step has succeeded (Key Vault later
	// on control-plane; none today). Operator-specific steps live on bindings.
	// On an SMI instance: the generation is Configured plus Cluster Service
	// dispatch and customer-scope roles (Key Vault later).
	// On a binding: every operator-specific configure step for that binding
	// has succeeded.
	// On a control-plane or data-plane slot: ActiveInstanceID equals
	// Spec.InstanceID, that instance is Configured, and this slot's binding
	// on that instance is Configured.
	// On the SMI slot: ActiveInstanceID equals Spec.InstanceID and that
	// instance is Configured.
	// On the subsystem: every spec slot is Configured and no generation,
	// instance, or binding is still deconfiguring.
	...
	// Service has been told to use this identity. On a control-plane or
	// data-plane binding: for that operator. On an SMI instance: for the
	// cluster SMI.
	...
	// identity credentials for this data-plane binding's operator subjects exist.
	...
	// done and no instance still names it. On a control-plane or data-plane
	// instance: type-particular teardown is done and no bindings remain. On an
	// SMI instance: type-particular teardown is done, including customer-scope
	// roles. On a binding: operator-specific teardown is done and the binding
	// can be dropped from the instance.
...
// # Instance
//
// An instance is type-particular progress plus, for control-plane and
// data-plane, bindings. Control-plane operators, SMI, and data-plane each
// have their own instance map. Two control-plane operators share one
// control-plane instance. A control-plane slot and the SMI slot never share
// an instance; they share a generation. A control-plane slot and a data-plane
// slot share only a generation.
//
// Each instance's GenerationID is assigned at create and is never rewritten.
// Replacement opens a new instance and moves the slot's InstanceID.
//
// # Binding
//
// A binding is one operator slot's use of one instance. It holds
// operator-specific configure/deconfigure progress (OIDC federation subjects,
// Cluster Service dispatch). Identity-level work stays on the generation.
// SMI has no binding: there is one SMI slot per cluster, so Cluster Service
// dispatch and customer-scope roles live on the SMI instance. During
// replacement a control-plane or data-plane slot can have a binding on both
// the Active instance and the Spec-desired instance.
//
...
// Configured. Control-plane and data-plane also wait for this slot's binding
// on that instance.
//
// # Flow
//
//  1. Frontend writes new desired ResourceIDs on the Cluster and starts an
//     Update operation. It does not write this Spec, generations, instances,
//     or bindings.
...
//     a new type instance pointing at that generation, and points Spec.InstanceID
//     at the instance. Two control-plane operators share one instance.
//     Control-plane and SMI share a generation, not an instance. Control-plane
//     and data-plane share a generation, not an instance.
...
//     the coordinator opens a new generation and new type instances for
//     every type that Spec'd the old generation.
...
//     (generation plus type-particular steps). Control-plane and data-plane
//     also wait for this slot's binding on that instance. Until then the
//     previous instance stays Active.
//  5. A control-plane or data-plane binding with no slot Spec and no slot
//     Active still naming that instance starts deconfiguring. When
//     Deconfigured it is dropped from the instance.
//  6. An instance with no slot Spec and no slot Active starts type-particular
//     deconfiguring. When Deconfigured (type-particular teardown done and,
//     for control-plane and data-plane, no bindings remain) it is dropped
//     from its type map.
//  7. A generation with no remaining instance starts identity-level
...
// type instance maps, bindings, nested conditions, and the two fetch recheck
// times (MSI dataplane vs data-plane ARM). See
// ServiceProviderClusterManagedIdentitiesSpec for the
// slot/generation/instance/binding model.
type ServiceProviderClusterManagedIdentitiesStatus struct {
	...
	// ControlPlaneInstances is the durable set of control-plane type instances
	// we have started configuring and have not finished deconfiguring. The key
	// is ControlPlaneManagedIdentityInstance.InstanceID (the UUID assigned when
	// the instance is created). It is not an operator name, not a ResourceID,
	// and is not lowercased. Several control-plane operators can share one
	// instance, so the operator is not on this key: start from Spec/Status
	// ControlPlaneOperators slots or from this instance's Bindings, which are
	// keyed by operator name as on Cluster. Each instance points at a GenerationID.
	// Written by: ManagedIdentitiesCoordinator (membership, Trigger, DeconfigurationStarted, bindings)
	...
	// DataPlaneInstances is the durable set of data-plane type instances
	// we have started configuring and have not finished deconfiguring. The key
	// is DataPlaneManagedIdentityInstance.InstanceID (the UUID assigned when the
	// instance is created). It is not an operator name, not a ResourceID, and
	// is not lowercased. Several operators can share one instance, so the
	// operator is not on this key: start from Spec/Status DataPlaneOperators
	// slots or from this instance's Bindings, which are keyed by operator name
	// as on Cluster. Each instance points at a GenerationID.
	// Written by: ManagedIdentitiesCoordinator (membership, Trigger, DeconfigurationStarted, bindings)
...
// instance is Configured (generation plus type-particular steps).
// Control-plane and data-plane also wait for this slot's binding on that
// instance. See ServiceProviderClusterManagedIdentitiesSpec.
type ManagedIdentitySlotStatus struct {
	// Conditions:
	// - "Configured": ActiveInstanceID equals Spec.InstanceID and that instance
	//   is Configured. Control-plane and data-plane also require this slot's
	//   binding on that instance to be Configured.
	// Written by: ManagedIdentitiesStatusAggregator
	// +optional
	// +patchMergeKey=type
	// +patchStrategy=merge
...
// deny-assignment exclude are True. Type-particular progress lives on
// instances; operator-specific progress lives on bindings.
...
// ControlPlaneManagedIdentityInstance is one control-plane type instance plus
// type-particular progress and per-operator bindings. Several control-plane
// slots may share one instance when they share a ResourceID. The Azure
...
type ControlPlaneManagedIdentityInstance struct {
	...
	// Trigger is why this type instance was created. See
	// ManagedIdentityReplacementTrigger. Distinct from the generation Trigger
	// when a later type starts using an already-created generation.
	// Written by: ManagedIdentitiesCoordinator
	Trigger ManagedIdentityReplacementTrigger         `json:"trigger,omitempty"`
	Status  ControlPlaneManagedIdentityInstanceStatus `json:"status,omitempty"`
}

// ServiceManagedIdentityInstance is one SMI type instance plus SMI
// configure/deconfigure progress. There is no binding: the SMI is
// cluster-scoped, not an operator. The Azure generation lives on
...
// DataPlaneManagedIdentityInstance is one data-plane type instance plus
// type-particular progress and per-operator bindings. Several data-plane slots
// may share one instance when they share a ResourceID. The Azure generation
// lives on Status.ManagedIdentities.Generations[GenerationID]. See
// ServiceProviderClusterManagedIdentitiesSpec.
...
// ControlPlaneManagedIdentityInstanceStatus is type-particular control-plane
// configure/deconfigure progress. Configured means the generation is
// Configured and every type-particular step has succeeded (Key Vault later;
// none today). Operator-specific progress lives on bindings.
type ControlPlaneManagedIdentityInstanceStatus struct {
	// Conditions:
	// - Configured (generation Configured and all type-particular configure steps True)
	// - Deconfigured (type-particular retirement complete and no bindings remain)
	// Written by: ManagedIdentitiesStatusAggregator
	// +optional
	// +patchMergeKey=type
	// +patchStrategy=merge
	...
	// Bindings is operator-specific progress for control-plane slots that Spec
	// or Active this instance. Keyed by operator name as on Cluster (not lowercased).
	// Written by: ManagedIdentitiesCoordinator (membership, DeconfigurationStarted),
	// ManagedIdentitiesStatusAggregator (Conditions)
	Bindings map[string]*ControlPlaneManagedIdentityBinding `json:"bindings,omitempty"`
}
...
// DataPlaneManagedIdentityInstanceStatus is type-particular data-plane
// configure/deconfigure progress. Configured means the generation is
// Configured and every type-particular step has succeeded (none today).
// Operator-specific progress (OIDC federation, Cluster Service dispatch)
// lives on bindings.
type DataPlaneManagedIdentityInstanceStatus struct {
	...
	// Bindings is operator-specific progress for data-plane slots that Spec or
	// Active this instance. Keyed by operator name as on Cluster (not lowercased).
	// Written by: ManagedIdentitiesCoordinator (membership, DeconfigurationStarted),
	// ManagedIdentitiesStatusAggregator (Conditions)
	Bindings map[string]*DataPlaneManagedIdentityBinding `json:"bindings,omitempty"`
}

// ControlPlaneManagedIdentityBinding is one control-plane operator's use of
// one control-plane instance. Operator-specific progress (Cluster Service
// dispatch) lives here; identity-level progress stays on the generation.
type ControlPlaneManagedIdentityBinding struct {
	// Conditions:
	// - ClusterServiceDispatched
	// - Configured (all operator-specific configure steps True)
	// - Deconfigured (operator-specific retirement complete)
	// Written by: ManagedIdentitiesStatusAggregator
	// +optional
	// +patchMergeKey=type
	// +patchStrategy=merge
	...
	// DeconfigurationStarted is set when no control-plane slot Spec or Active
	// for this operator still names the parent instance.
	// Written by: ManagedIdentitiesCoordinator
	DeconfigurationStarted *metav1.Time `json:"deconfigurationStarted,omitempty"`
}

// DataPlaneManagedIdentityBinding is one data-plane operator's use of one
// data-plane instance. OIDC federation subjects come from this operator's
// KubernetesServiceAccounts, not from the UAMI.
type DataPlaneManagedIdentityBinding struct {
	// Conditions:
	...
	// DeconfigurationStarted is set when no data-plane slot Spec or Active for
	// this operator still names the parent instance.
	// Written by: ManagedIdentitiesCoordinator
	DeconfigurationStarted *metav1.Time `json:"deconfigurationStarted,omitempty"`

	// OIDCFederation tracks federated identity credentials for this operator's
	// Kubernetes service account subjects.
	// Written by: (not yet; OIDC federation controller will write per binding)
...
// binding. Subjects are that operator's KubernetesServiceAccounts.
