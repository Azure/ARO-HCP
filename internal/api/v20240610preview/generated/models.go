...
type HcpOpenShiftClusterProperties struct {
	...
	// nodeDrainGracePeriodMinutes is the grace period for how long Pod Disruption Budget-protected workloads will be respected
	// during upgrades. After this grace period, any workloads protected by Pod
	// Disruption Budgets that have not been successfully drained from a node will be forcibly evicted.
	// Valid values are in minutes and from 0 to 10080 minutes (1 week) . 0 means that the MachinePool can be drained without
	// any time limitation.
	// This value acts as a global default for all nodePools but can be overridden but setting nodeDrainGracePeriodMinutes for
	// a specific nodePool.
...
type NodePoolProperties struct {
	...
	// during upgrades. After this grace period, any workloads protected by Pod
	// Disruption Budgets that have not been successfully drained from a node will be forcibly evicted.
	// Valid values are from 0 to 10080 minutes (1 week) . 0 means that the MachinePool can be drained without any time limitation.
	// If unset the cluster nodeDrainGracePeriodMinutes value is used as a default.
	NodeDrainGracePeriodMinutes *int32
