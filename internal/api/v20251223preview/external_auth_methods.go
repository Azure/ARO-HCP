...
func normalizeExternalAuthClientProfile(p *generated.ExternalAuthClientProfile, out *resourcesapi.ExternalAuthClientProfile) {
	if p.Component != nil {
		...
		out.Component = resourcesapi.ExternalAuthClientComponentProfile{}
	}
	out.ClientID = resourcesapi.Deref(p.ClientID)
	...
	out.Type = resourcesapi.ExternalAuthClientType(resourcesapi.Deref(p.Type))
}
...
func normalizeTokenIssuerProfile(p *generated.TokenIssuerProfile, out *resourcesapi.TokenIssuerProfile) {
	...
	out.CA = resourcesapi.Deref(p.CA)
}
...
func normalizeExternalAuthClaimProfile(p *generated.ExternalAuthClaimProfile, out *resourcesapi.ExternalAuthClaimProfile) {
	if p.Mappings != nil {
		...
		out.Mappings = resourcesapi.TokenClaimMappingsProfile{}
	}
...
func normalizeTokenClaimMappingsProfile(p *generated.TokenClaimMappingsProfile, out *resourcesapi.TokenClaimMappingsProfile) {
	if p.Username != nil {
		out.Username.Claim = resourcesapi.Deref(p.Username.Claim)
		out.Username.Prefix = resourcesapi.Deref(p.Username.Prefix)
		out.Username.PrefixPolicy = resourcesapi.UsernameClaimPrefixPolicy(resourcesapi.Deref(p.Username.PrefixPolicy))
	} else {
		out.Username = resourcesapi.UsernameClaimProfile{}
	}
	if p.Groups != nil {
		out.Groups = &resourcesapi.GroupClaimProfile{
			Claim:  resourcesapi.Deref(p.Groups.Claim),
			Prefix: resourcesapi.Deref(p.Groups.Prefix),
		}
	} else {
		out.Groups = nil
	}
...
func normalizeTokenClaimValidationRule(p *generated.TokenClaimValidationRule, out *resourcesapi.TokenClaimValidationRule) {
	out.Type = resourcesapi.TokenValidationRuleType(resourcesapi.Deref(p.Type))
	if p.RequiredClaim != nil {
		out.RequiredClaim.Claim = resourcesapi.Deref(p.RequiredClaim.Claim)
		out.RequiredClaim.RequiredValue = resourcesapi.Deref(p.RequiredClaim.RequiredValue)
	} else {
		out.RequiredClaim = resourcesapi.TokenRequiredClaim{}
	}
}
