...
// "stable-4.21"), which is the rollout's top-level resource name. All rollouts
// share a Cosmos partition keyed by the provider namespace.
...
func (l *informerBasedControlPlaneVersionRolloutLister) Get(ctx context.Context, yStreamChannel string) (*fleetapi.ControlPlaneVersionRollout, error) {
	key := fleetapihelpers.ToControlPlaneVersionRolloutResourceIDString(yStreamChannel)
