...
var _ cosmosstorageutils.DBTransaction = &cosmosDBTransaction{}

type cosmosDBTransaction struct {
	pk           string
	client       *azcosmos.ContainerClient
	steps        []cosmosstorageutils.CosmosDBTransactionStep
	stepsDetails []cosmosstorageutils.CosmosDBTransactionStepDetails
...
func (t *cosmosDBTransaction) AddStep(details cosmosstorageutils.CosmosDBTransactionStepDetails, stepFn cosmosstorageutils.CosmosDBTransactionStep) {
	t.steps = append(t.steps, stepFn)
	t.stepsDetails = append(t.stepsDetails, details)
}

func (t *cosmosDBTransaction) OnSuccess(callback cosmosstorageutils.DBTransactionCallback) {
	if callback != nil {
		t.onSuccess = append(t.onSuccess, callback)
	}
}

func (t *cosmosDBTransaction) Execute(ctx context.Context, o *azcosmos.TransactionalBatchOptions) (cosmosstorageutils.DBTransactionResult, error) {
		...
		if !response.Success {
			for step, result := range response.OperationResults {
				if result.StatusCode != http.StatusFailedDependency {
					...
					return nil, cosmosstorageutils.NewTransactionStepError(step+1, len(response.OperationResults), int(result.StatusCode), t.stepsDetails[step])
				}
			}
		}
...
func (t *cosmosDBTransaction) ToJSONStructRendering() cosmosstorageutils.CosmosDBTransactionDetails {
	return cosmosstorageutils.CosmosDBTransactionDetails{
		PartitionKey: t.pk,
		Steps:        t.stepsDetails,
	}
}

var _ cosmosstorageutils.DBTransactionResult = &cosmosDBTransactionResult{}
