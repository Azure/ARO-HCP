...
// types. Each union lister fans every read out to a configurable set of
// per-management-cluster sublisters and merges the results, satisfying the
// same listers.<Type>DesireLister interface that any single-MC sublister
// satisfies. Add/Remove maintain the sublister set under a mutex; lookups
// take a snapshot under RLock so reads never block writes for the full
// duration of a Cosmos call.
//
...
// UnionApplyDesireLister fans every read out to a set of ApplyDesireLister
...
// call.
type UnionApplyDesireLister struct {
	mu         sync.RWMutex
	sublisters map[string]listers.ApplyDesireLister // key = lowercased(rid.String())
}

var _ listers.ApplyDesireLister = &UnionApplyDesireLister{}

// NewUnionApplyDesireLister returns an empty union; call Add to register
// per-management-cluster sublisters.
func NewUnionApplyDesireLister() *UnionApplyDesireLister {
	return &UnionApplyDesireLister{sublisters: map[string]listers.ApplyDesireLister{}}
}
...
func (u *UnionApplyDesireLister) Add(managementClusterResourceID *azcorearm.ResourceID, sublister listers.ApplyDesireLister) {
...
func (u *UnionApplyDesireLister) Remove(managementClusterResourceID *azcorearm.ResourceID) {
	if managementClusterResourceID == nil {
		return
	}
	u.mu.Lock()
...
func (u *UnionApplyDesireLister) snapshot() []listers.ApplyDesireLister {
	u.mu.RLock()
	defer u.mu.RUnlock()
	out := make([]listers.ApplyDesireLister, 0, len(u.sublisters))
...
func (u *UnionApplyDesireLister) lookup(rid *azcorearm.ResourceID) listers.ApplyDesireLister {
	if rid == nil {
		return nil
	}
	u.mu.RLock()
...
func (u *UnionApplyDesireLister) List(ctx context.Context) ([]*kubeapplier.ApplyDesire, error) {
...
func (u *UnionApplyDesireLister) GetForCluster(
...
func (u *UnionApplyDesireLister) GetForNodePool(
...
func (u *UnionApplyDesireLister) ListForManagementCluster(
...
func (u *UnionApplyDesireLister) ListForCluster(
...
func (u *UnionApplyDesireLister) ListForNodePool(
