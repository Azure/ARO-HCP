...
type ManagementClusterCRUD interface {
	ResourceCRUD[fleet.ManagementCluster]
}

func NewManagementClusterCRUD(containerClient *azcosmos.ContainerClient, subscriptionID, resourceGroupName string) ManagementClusterCRUD {
	parentResourceID := api.Must(api.ToResourceGroupResourceID(subscriptionID, resourceGroupName))
	return NewCosmosResourceCRUD[fleet.ManagementCluster, GenericDocument[fleet.ManagementCluster]](
		containerClient, parentResourceID, fleet.ManagementClusterResourceType)
}
