...
func TestMockKubeApplier_IsolatedFromMockResourcesDBClient(t *testing.T) {
	...
	dbMock := corecosmosstoragetesting.NewMockResourcesDBClient()
	desire := newClusterApplyDesire(t)
	...
	if got := len(dbMock.GetAllDocuments()); got != 0 {
		t.Errorf("corecosmosstoragetesting.MockResourcesDBClient saw %d documents from a kube-applier write; expected 0", got)
	}
